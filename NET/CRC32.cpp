...
#define xDO1(buf) crc = crc_32_table[((int)crc ^ (*buf++)) & 0xff] ^ (crc >> 8);
#define xDO2(buf) xDO1(buf); xDO1(buf);
#define xDO4(buf) xDO2(buf); xDO2(buf);
#define xDO8(buf) xDO4(buf); xDO4(buf);

dword CRC32::GetCRC32(const char * pBuffer, dword dwlen)
{
	if (!pBuffer) return 0;
	dword crc = 0;
	while (dwlen >= 8)
