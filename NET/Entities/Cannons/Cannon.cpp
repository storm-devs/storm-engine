...
void NetCannon::RealFire()
{
    ...
    api->Event("NetServer_OnCannonFire", "afffffff", GetAIObjPointer()->GetACharacter(), vPosTemp.x, vPosTemp.y,
               vPosTemp.z, fSpeedV0, fFireDirection, fFireHeightAngle, fDirY);
...
bool NetCannon::Fire(float _fSpeedV0, CVECTOR &_vFirePos)
{
    ...
    ATTRIBUTES *pAFireTime = GetACharacter()->FindAClass(GetACharacter(), "Ship.Cannons.FireTime");
    Assert(pAFireTime);
...
void NetCannon::Unload()
{
    ...
    {
        ATTRIBUTES *pACharge = GetACharacter()->FindAClass(GetACharacter(), "Ship.Cannons.Charge");
        Assert(pACharge);
        ATTRIBUTES *pAGoods = GetACharacter()->FindAClass(GetACharacter(), "Ship.Goods");
        Assert(pAGoods);
...
void NetCannon::Recharge()
{
    ...
    ATTRIBUTES *pARechargeTime = GetACharacter()->FindAClass(GetACharacter(), "Ship.Cannons.ReloadTime");
    Assert(pARechargeTime);
