...
dword NetWeather::AttributeChanged(ATTRIBUTES *pAttribute)
{
    ...
    if (*pParent == "fog")
    {
        ...
        if (*pAttribute == "Start")
        {
            fFogStart = pAttribute->GetAttributeAsFloat();
            return 0;
        }
        if (*pAttribute == "Density")
        {
            fFogDensity = pAttribute->GetAttributeAsFloat();
            return 0;
        }
        if (*pAttribute == "Color")
        {
            dwFogColor = pAttribute->GetAttributeAsDword();
            return 0;
        }
    }
    ...
    if (*pParent == "sun")
    {
        if (*pAttribute == "Color")
        {
            dwSunColor = pAttribute->GetAttributeAsDword();
            return 0;
        }
        if (*pAttribute == "Ambient")
        {
            dwSunAmbient = pAttribute->GetAttributeAsDword();
            return 0;
        }
        if (*pAttribute == "x")
        {
            fSunX = pAttribute->GetAttributeAsFloat();
            return 0;
        }
        if (*pAttribute == "y")
        {
            fSunY = pAttribute->GetAttributeAsFloat();
            return 0;
        }
        if (*pAttribute == "z")
        {
            fSunZ = pAttribute->GetAttributeAsFloat();
            return 0;
        }
    }
