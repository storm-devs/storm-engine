...
void NetSunGlow::GenerateSunGlow()
{
    ...
    if (Flares.sTexture.Len())
        iFlareTex = pRS->TextureCreate(Flares.sTexture);
    if (Glow.sTexture.Len())
        iGlowTex = pRS->TextureCreate(Glow.sTexture);
    if (Overflow.sTexture.Len())
        iOverflowTex = pRS->TextureCreate(Overflow.sTexture);
...
dword NetSunGlow::AttributeChanged(ATTRIBUTES *pAttribute)
{
    if (*pAttribute == "isDone")
    {
        ...
        Assert(pSVWeather);
        pAWeather = pSVWeather->GetAClass();
        Assert(pAWeather);
    ...
    if (*pParent == "Glow")
    {
        ...
        if (*pAttribute == "Size")
        {
            Glow.fSize = pAttribute->GetAttributeAsFloat();
            return 0;
        }
        ...
        {
            Glow.sTexture = pAttribute->GetThisAttr();
            return 0;
        }
