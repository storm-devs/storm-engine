...
NetDeckCamera::~NetDeckCamera()
{
    Render().TextureRelease(iCrosshairTex);
    iCrosshairTex = -1;
    Render().TextureRelease(iTelescopeTex);
    iTelescopeTex = -1;
}
...
void NetDeckCamera::Execute(dword Delta_Time)
{
    ...
    if (fCurrentPerspective < 0.0f)
        fCurrentPerspective = fStandartPerspective;
...
void NetDeckCamera::Realize(dword Delta_Time)
{
    ...
    Render().GetTransform(D3DTS_VIEW, m);
    m.Transposition();
    CVECTOR vZ = m.Vz();

    ATTRIBUTES *pACamPos = AttributesPointer->FindAClass(AttributesPointer, "Pos");
    Assert(pACamPos);
    ATTRIBUTES *pACamDir = AttributesPointer->FindAClass(AttributesPointer, "Dir");
    Assert(pACamDir);
    ...
    if (IsZoomed())
    {
        ...
        if (pBestShip)
        {
            ...
            Render().ExtPrint(FONT_DEFAULT, 0xFFFFFFFF, 0x00000000, ALIGN_CENTER, 0, 0.85f, 0, 0, long(vp.Width) / 2,
        long(vp.Height) / 2 - vp.Height / 6, "%s, HP: %.1f, Crew: %.1f", sName.GetBuffer(), fHP, fCrew);
        }*/
...
dword NetDeckCamera::AttributeChanged(ATTRIBUTES *pAttr)
{
    ...
    if (pAParent && *pAParent == "Telescope")
    {
        if (*pAttr == "Technique")
        {
            sTelescopeTechnique = pAttr->GetThisAttr();
            return 0;
        }
        if (*pAttr == "Texture")
        {
    ...
    if (pAParent && *pAParent == "Crosshair")
    {
        if (*pAttr == "Technique")
        {
            sCrosshairTechnique = pAttr->GetThisAttr();
            return 0;
        }
        if (*pAttr == "SubTexX")
        {
            dwCrosshairSubTexX = pAttr->GetAttributeAsDword();
            return 0;
        }
        if (*pAttr == "SubTexY")
        {
            dwCrosshairSubTexY = pAttr->GetAttributeAsDword();
            return 0;
        }
        if (*pAttr == "Size")
        {
            fCrosshairSize = pAttr->GetAttributeAsFloat();
            return 0;
        }
        if (*pAttr == "Texture")
        {
    ...
    if (pAParent && *pAParent == "Colors")
    {
        if (*pAttr == "Default")
        {
            Colors[0] = pAttr->GetAttributeAsDword();
            return 0;
        }
        if (*pAttr == "Friend")
        {
            Colors[1] = pAttr->GetAttributeAsDword();
            return 0;
        }
        if (*pAttr == "Enemy")
        {
            Colors[2] = pAttr->GetAttributeAsDword();
            return 0;
        }
    }
    ...
    if (*pAttr == "On")
        SetOn(pAttr->GetAttributeAsDword() != 0);
    if (*pAttr == "Active")
        SetActive(pAttr->GetAttributeAsDword() != 0);
