...
void FirePlace::Run(const char *pParticleSmokeName, const char *pParticleFireName, const char *pSoundName,
                    float _fRunTime)
{
    if (isActive())
        return;
...
{
    if (_iSoundID != 0)
        api->Send_Message(eidSound, "lll", MSG_SOUND_STOP, _iSoundID, 0);
}
