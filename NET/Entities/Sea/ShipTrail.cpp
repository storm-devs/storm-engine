...
void NetSea::GetLineABC(const CVECTOR &v1, const CVECTOR &v2, float &A, float &B, float &C)
{
    A = (v2.z - v1.z);
    B = (v1.x - v2.x);
...
void NetSea::NewShipInfo(ENTITY_ID *eid, float fSpeed, CVECTOR *vNewPos, float fAngY)
{
    dword i;

    for (i = 0; i < aShips.Size(); i++)
        if (eid->pointer == aShips[i].eID.pointer)
            break;
    if (i == aShips.Size())
        return;
...
void NetSea::ExecuteShipsTrails(dword DeltaTime)
{
    // return;
    dword i, j;
...
void NetSea::GetTrailUV(shiptrail_t *pST, CVECTOR *vPos, float *fU, float *fV)
{
    long iIDX = 0;
...
void NetSea::CheckPointForShipTrail(SEAPENAVERTEX *pV, seaship_t *pS)
{
    float sum = 0.0f;
    CVECTOR &vPos = pV->pos;
...
void NetSea::CalculateShipTrailTrapezium(seaship_t *pS)
{
    dword i, j;
    float fMaxX = -100000.0f, fMaxZ = -100000.0f, fMinX = 100000.0f, fMinZ = 100000.0f;
    if (pS->dwNumTrails)
        for (i = 0; i < pS->dwNumTrails - 1; i++)
        {
