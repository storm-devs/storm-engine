...
void NetSay::Realize(DWORD __DeltaTime)
{
    ...
    for (long i = iFirst; i < iLast; i++)
    {
        ...
        if (S.Type == CHAT_MESSAGE)
        {
            ...
            Render().ExtPrint(FONT_DEFAULT, dwAlpha | 0x62AD00, 0x00000000, ALIGN_LEFT, 0, fFontScale, 0, 0, x, y + dy,
                              (char *)sRes.GetBuffer());
            Render().ExtPrint(FONT_DEFAULT, dwAlpha | S.dwColor, 0x00000000, ALIGN_LEFT, 0, fFontScale, 0, 0,
                              x + iWidth, y + dy, (char *)S.sMessage.GetBuffer());
        }
        if (S.Type == GAME_MESSAGE)
        {
            Render().ExtPrint(FONT_DEFAULT, dwAlpha | S.dwColor, 0x00000000, ALIGN_LEFT, 0, fFontScale, 0, 0, x, y + dy,
                              (char *)S.sMessage.c_str());
        }
    }
}
