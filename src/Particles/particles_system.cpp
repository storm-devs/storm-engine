...
// node ----------------------------------------------------------------------------
PARTICLE_SYSTEM *PARTICLE_SYSTEM::GetLeftNode()
{
    return l_PTR;
}
PARTICLE_SYSTEM *PARTICLE_SYSTEM::GetRightNode()
{
    return r_PTR;
}
void PARTICLE_SYSTEM::SetLeftNode(PARTICLE_SYSTEM *node)
{
    l_PTR = node;
}
void PARTICLE_SYSTEM::SetRightNode(PARTICLE_SYSTEM *node)
{
    r_PTR = node;
}
...
bool PARTICLE_SYSTEM::Init(INIFILE **pInifiles, DWORD nIniNum, char *psname)
{
    ...
    if (ini->TestKey(psname, PSKEY_COLORINVERSE, 0))
        bColorInverse = true;
    else
        bColorInverse = false;

    if (ini->TestKey(psname, PSKEY_UNIFORMEMIT, 0))
        bUniformEmit = true;
    else
        bUniformEmit = false;

    if (ini->TestKey(psname, PSKEY_RANDOMDIRECTION, 0))
        bRandomDirection = true;
    else
        bRandomDirection = false;

    if (ini->TestKey(psname, PSKEY_NONSTOPEMIT, 0))
        bRepeat = true;
    else
        bRepeat = false;
    ...
    RenderService->CreateVertexBuffer(sizeof(PARTICLE_VERTEX) * VERTEXS_ON_PARTICLE * ParticlesNum, D3DUSAGE_WRITEONLY,
                                      PARTICLE_FVF, D3DPOOL_MANAGED, &VBuffer);
    if (VBuffer == 0)
        _THROW(vbuffer error);
...
void PARTICLE_SYSTEM::UpdateVertexBuffer()
{
    ...
    RenderService->VBLock(VBuffer, 0, sizeof(PARTICLE_VERTEX) * VERTEXS_ON_PARTICLE * ParticlesNum, (byte **)&pVertex,
                          0);
    for (n = 0; n < ParticlesNum; n++)
    {
