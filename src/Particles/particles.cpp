...
PARTICLES::~PARTICLES()
{
    ...
    if (pInifiles)
    {
        for (DWORD n = 0; n < nFilesCounter; n++)
            delete pInifiles[n];
...
void PARTICLES::Realize(dword Delta_Time)
{
    ...
    while (psStart)
    {
        ...
        while (ps)
        {
            if (ps->TextureID[0] != psStart->TextureID[0])
                break;
            if (ps->TexturesNum != psStart->TexturesNum)
                break;
            if (stricmp(ps->TechniqueName, psStart->TechniqueName) != 0)
                break;
            ps = ps->GetRightNode();
        }
...
dword PARTICLES::AttributeChanged(ATTRIBUTES *pA)
{
    ...
    else if (stricmp(aname, "x") == 0 || stricmp(aname, "y") == 0 || stricmp(aname, "z") == 0)
    {
        pA = pA->GetParent();
        if (pA == null)
            return 0;
