...
void ISLAND::SetDevice()
{
    ...
    Assert(pGS);
    pIslandTraceWalker = api->LayerGetWalker("island_trace");
    Assert(pIslandTraceWalker);
}
...
void ISLAND::CalcBoxParameters(CVECTOR &_vBoxCenter, CVECTOR &_vBoxSize)
{
    ...
    if (pEID)
        do
        {
            MODEL *pM = (MODEL *)api->GetEntityPointer(pEID);
            Assert(pM);
            dword i = 0;
            while (true)
            {
...
void ISLAND::CreateDirectories(char *pDir)
{
    ...
    while (true)
    {
        ...
        if (!pStr)
            break;
        strncpy(sTemp, pLast, pStr - pLast);
        sTemp[pStr - pLast] = '\0';
        pLast = ++pStr;
        strcat(sCurDir, sTemp);
        strcat(sCurDir, "\\");
...
bool ISLAND::CreateShadowMap(char *pDir, char *pName)
{
    ...
    sDir.Format("resource\\foam\\%s\\%s\\", pDir, AttributesPointer->GetAttribute("LightingPath"));
    sDir.CheckPath();
    ...
    if (mzShadow.Load(string(fname) + ".zap"))
        return true;
...
bool ISLAND::CreateHeightMap(char *pDir, char *pName)
{
    ...
    sDir.Format("resource\\\\foam\\\\%s\\\\", pDir);
    sDir.CheckPath();
    sprintf(fname, "%s%s.tga", (const char *)sDir, pName);
    ...
    if (bLoad)
    {
        ...
        for (iDMapSizeShift = 0; iDMapSizeShift < 30; iDMapSizeShift++)
            if (dword(1 << iDMapSizeShift) == iDMapSize)
                break;
        ...
        {
            api->Trace("Island: vBoxCenter not equal, foam error: %s, distance = %.3f", iname,
    ...
    for (fZ = 0; fZ < float(iDMapSize); fZ += 1.0f)
    {
        ...
        for (fX = 0; fX < float(iDMapSize); fX += 1.0f)
        {
            ...
            if (pDepthMap[iIdx] == BYTE(HMAP_START))
                fEarthPercent += 1.0f;
        }
    }
...
bool ISLAND::Mount(char *fname, char *fdir, ENTITY_ID *eID)
{
    ...
    sRealFileName.Format("%s\\%s", fdir, fname);
    sRealFileName.CheckPath();
