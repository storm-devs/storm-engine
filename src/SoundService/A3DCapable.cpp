...
//--------------------------------------------------------------------
A3DCapable::A3DCapable() : ds3DBuffer(0)
{
}
...
//--------------------------------------------------------------------
bool A3DCapable::Play(bool _looped, const CVECTOR *_startPosition, float _minDistance, float _maxDistance,
                      float _volume)
{
    GUARD(A3DCapable::Unload)
...
bool A3DCapable::Set3DParameter(eSoundMessage _message, const void *_op)
{
    ...
    case SM_POSITION:
        return (ds3DBuffer->SetPosition(((const CVECTOR *)_op)->x, ((const CVECTOR *)_op)->y, ((const CVECTOR *)_op)->z,
                                        DS3D_DEFERRED) == DS_OK);
    }
#else
    if (!core->dsBuffer)
    ...
    case SM_POSITION:
        return (core->dsBuffer->SetPosition(((const CVECTOR *)_op)->x, ((const CVECTOR *)_op)->y,
                                            ((const CVECTOR *)_op)->z, DS3D_DEFERRED) == DS_OK);
    }
#endif
    return false;
    UNGUARD
