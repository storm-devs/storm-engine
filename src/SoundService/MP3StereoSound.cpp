...
//--------------------------------------------------------------------
bool MP3StereoSound::Play(const char *_name, bool _simpleCache /*= false*/, long _time /* = 0*/,
...
#ifndef _XBOX
    if (!CreateBuffer(DSBCAPS_CTRLPAN | DSBCAPS_CTRLVOLUME | DSBCAPS_CTRLPOSITIONNOTIFY | DSBCAPS_CTRLFREQUENCY |
                          DSBCAPS_GETCURRENTPOSITION2 | DSBCAPS_LOCSOFTWARE,
                      chunkInfo.cksize, waveFormat))
        return false;
#else
    if (!CreateBuffer(DSBCAPS_CTRLVOLUME | DSBCAPS_CTRLPOSITIONNOTIFY | DSBCAPS_CTRLFREQUENCY, chunkInfo.cksize,
                      waveFormat))
        return false;
#endif
