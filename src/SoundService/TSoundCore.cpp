...
TSoundCore::TSoundCore(eVolumeType _volumeType)
    : ds8(0), looped(false), volume(1.0f), playing(false), fade(FADE_DISABLED), dsBuffer(0),
      loopPauseTime(LOOP_DISABLED), maxLoopPauseTime(0), volumeType(_volumeType), isOriginalBuffer(true),
      onUnload(false)
{
    dataSize = 0;
}
...
bool TSoundCore::CreateBuffer(DWORD _flags, int _size, const WAVEFORMATEX &_waveFormat, tCachedData *_cachedData)
{
    ...
    DSBUFFERDESC bufferDescriptor = {sizeof(bufferDescriptor), _flags, finalSize, 0, &myWF};
    ...
    DSBUFFERDESC bufferDescriptor = {sizeof(bufferDescriptor), _flags, 0, (LPWAVEFORMATEX)&xWF, 0, 0};
    ...
    if (_cachedData)
    {
        ...
        Assert(buf1Size == _cachedData->size) memcpy(buf1, _cachedData->pcmData, _cachedData->size);
