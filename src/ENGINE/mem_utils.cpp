...
void MEM_USAGE_INFO::Register(char *pFileName, DWORD nLine, DWORD nMemSize)
{
    ...
    if (pFileName == 0)
    {
        ...
        if (pMemBL[n].nMaxAllocated < pMemBL[n].nCurrentAllocated)
            pMemBL[n].nMaxAllocated = pMemBL[n].nCurrentAllocated;
        if (pMemBL[n].nMaxBlocks < pMemBL[n].nCurrentBlocks)
            pMemBL[n].nMaxBlocks = pMemBL[n].nCurrentBlocks;
    ...
    for (n = 0; n < nMemBLNum; n++)
    {
        if (pMemBL[n].nLine != nLine)
            continue;
        if (strcmp(pMemBL[n].pFileName, pFileName) != 0)
            continue;
        pMemBL[n].nTotalBlocks++;
        pMemBL[n].nTotalAllocated += nMemSize;
        pMemBL[n].nCurrentAllocated += nMemSize;
...
void MEM_USAGE_INFO::Allocate(char *pFileName, DWORD nLine, DWORD nMemSize)
{
    ...
    nCurrentBlocks++;
    if (nCurrentBlocks > nMaxSimBlocks)
        nMaxSimBlocks = nCurrentBlocks;

    nCurrentAllocated += nMemSize;
    if (nCurrentAllocated > nMaxSimAllocated)
        nMaxSimAllocated = nCurrentAllocated;
...
void MEM_USAGE_INFO::Resize(char *pFileName, DWORD nLine, DWORD nMemSize, char *pOldFileName, DWORD nOldLine,
                            DWORD nOldMemSize)
{
    if (nMemSize < SMALL_BLOCKS_LIMIT)
    {
    ...
    if (nOldMemSize < SMALL_BLOCKS_LIMIT)
        nBlockTableSBS[nOldMemSize].nCurrent--;
...
void MEM_USAGE_INFO::Report()
{
    ...
    for (n = 0; n < SMALL_BLOCKS_LIMIT; n++)
    {
        ...
        trace("    [%d] : %d  summ[ %d ]  b2tr : %d", n, nBlockTableSBS[n].nMax, nBlockTableSBS[n].nMax * n,
              nBlockTableSBS[n].nB2TRatio);
    }
    ...
    for (n = 0; n < MEDIUM_BLOCKS_LIMIT; n++)
    {
        ...
        trace("    [%d - %d] : %d  summ[ %d ] ", n * 256, n * 256 + 255, nBlockTableMBS[n].nMax,
              nBlockTableMBS[n].nMax * n * 255);
    }
    ...
    for (i = 0; i < nMemBLNum; i++)
    {
        ...
        trace("%d in %d blocks  : %s  line %d", pMemBL[mn].nMaxAllocated, pMemBL[mn].nMaxBlocks, pMemBL[mn].pFileName,
              pMemBL[mn].nLine);
...
void MEM_USAGE_INFO::UpdateMemoryProfile(char *pFileName)
{
    ...
    for (n = 0; n < MEM_PROFILE_BLOCKS_NUM; n++)
    {
        if (n < nProfileBlocks)
        {
            if (nBlockTableSBS[n].nMax > pTempProfileInfo[n])
                nProfileValue = nBlockTableSBS[n].nMax;
            else
                nProfileValue = pTempProfileInfo[n];
        }
        else
            nProfileValue = nBlockTableSBS[n].nMax;
...
MEM_ALLOC_ADDRESS::~MEM_ALLOC_ADDRESS()
{
    ...
    if (pData)
    {
        ...
        {
            if (pData[n].pFileName)
                free(pData[n].pFileName);
        }
        free(pData);
    }
    pData = 0;
...
DWORD MEM_ALLOC_ADDRESS::GetAddress(const char *pFileName, DWORD dwLine)
{
    ...
    for (n = 0; n < dwRecordsNum; n++)
    {
        if (pData[n].dwLine != dwLine)
            continue;
        if (stricmp(pData[n].pFileName, pFileName) != 0)
            continue;
        return n;
    }
    while ((dwRecordsNum + 1) >= dwBufferSize)
    {
        ...
        if (dwBufferSize == 0)
        {
            ...
            if (!pData)
                throw "MEM_ALLOC_ADDRESS::GetAddress";
        }
        else
        {
    ...
    if (!pData[dwRecordsNum].pFileName)
        throw "MEM_ALLOC_ADDRESS::GetAddress";
