...
void FillCharactersScroll()
{
    ...
    GameInterface.CHARACTERS_SCROLL.(attributeName).tex1 =
        FindFaceGroupNum("CHARACTERS_SCROLL.ImagesGroup", "FACE128_" + pchar.FaceID);
    ...
    for (i = 0; i < nOffSize; i++)
    {
        ...
        if (_curCharIdx != -1)
        {
            ...
            GameInterface.CHARACTERS_SCROLL.(attributeName).tex1 =
                FindFaceGroupNum("CHARACTERS_SCROLL.ImagesGroup", "FACE128_" + Characters[_curCharIdx].FaceID);
        ...
        else
        {
            ...
            GameInterface.CHARACTERS_SCROLL.(attributeName).tex1 =
                FindFaceGroupNum("CHARACTERS_SCROLL.ImagesGroup", "FACE128_" + PsgAttrName);
            pRef2.str2 = "#" + " ";
            pRef2.str3 = "#" + " ";
    ...
    for (i = 0; i < GetPassengersQuantity(pchar); i++)
    {
        ...
        if (_curCharIdx != -1 &&
            !CheckAttribute(&characters[_curCharIdx],
                            "isfree")) // нужно ставить isfree для офицеров, чтоб два раза не попадали.
        {
            makearef(pRef2, GameInterface.CHARACTERS_SCROLL.(attributeName));
            GameInterface.CHARACTERS_SCROLL.(attributeName).character = _curCharIdx;
            GameInterface.CHARACTERS_SCROLL.(attributeName).img1 = GetFacePicName(GetCharacter(_curCharIdx));
