...
void CONTROL_STACK::Clear()
{
    used_blocks = 0;
};
dword CONTROL_STACK::DataNum()
{
    return used_blocks;
};
...
bool CONTROL_STACK::Read(CONTROL_BLOCK *cb_PTR)
{
    char *mem_PTR;
    if (cb_PTR)
        memset(cb_PTR, 0, sizeof(CONTROL_BLOCK));
    if (used_blocks == 0)
        return false;
    if (Data_PTR == null)
        return false;
    ...
    if (cb_PTR)
        memcpy(cb_PTR, mem_PTR, sizeof(CONTROL_BLOCK));
    return true;
};
