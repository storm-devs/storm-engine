...
void _cdecl ENTITY_STATE_GEN_R::SetState(char *Format, ...)
{
    ...
    if (!Format)
        SE_THROW_MSG(empty format string);
    if (Format_string == null)
    {
    ...
    for (n = 0; Format[n]; n++)
    {
        ...
        case 'v':
            ...
            break;
        default:
            SE_THROW_MSG(invalid format string specificator);
            break;
        }
    }
...
void ENTITY_STATE_R::LoadStateBlock()
{
    ...
    if (dwR != sizeofstruct)
        SE_THROW;
    if (memcmp(Signature, Signature_Buff, strlen(Signature)) != 0)
        SE_THROW_MSG(invalid signature);

    // read size of format string
    fio->_ReadFile(file_handle, &format_size, sizeof(format_size), &dwR);
    if (dwR != sizeof(format_size))
        SE_THROW;
    if (format_size == 0)
        SE_THROW_MSG(empty block);
...
void ENTITY_STATE_R::ValidateFormat(char c)
{
    ...
    if (Format_string == null)
        LoadStateBlock();
    if (Format_string == null)
        SE_THROW_MSG(no state data);
    if (Format_string[Format_index] != c)
        SE_THROW_MSG(incorrect state data);
    Format_index++;
    UNGUARD
}

//#define RETURN_DATA(t) {Data_index += sizeof(t);if(Data_index > Data_size) SE_THROW_MSG(no data);return (*(t
//*)(Buffer[Data_index - sizeof(t)]));}
#define RETURN_DATA(t)                                                                                                 \
    {                                                                                                                  \
        Data_PTR += sizeof(t);                                                                                         \
        if (Data_PTR > (Buffer + Data_size))                                                                           \
            SE_THROW_MSG(no data);                                                                                     \
...
void ENTITY_STATE_R::String(dword dest_buffer_size, char *buffer)
{
    ...
    if (size > dest_buffer_size)
        SE_THROW_MSG(insufficient string buffer);
...
void ENTITY_STATE_R::MemoryBlock(dword memsize, char *buffer)
{
    ...
    size = (*(dword *)(Data_PTR - sizeof(dword)));
    if (size != memsize)
        SE_THROW_MSG(invalid buffer size);
    Data_PTR += size;
    if (Data_PTR > (Buffer + Data_size))
        SE_THROW_MSG(no data);
    memcpy(buffer, (char *)(Data_PTR - size), size);
...
void ENTITY_STATE_R::Struct(dword memsize, char *buffer)
{
    ...
    size = (*(dword *)(Data_PTR - sizeof(dword)));
    if (size != memsize)
        SE_THROW_MSG(invalid structure size);
    Data_PTR += size;
    if (Data_PTR > (Buffer + Data_size))
        SE_THROW_MSG(no data);
    memcpy(buffer, (char *)(Data_PTR - size), size);
