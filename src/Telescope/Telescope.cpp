...
//--------------------------------------------------------------------
TELESCOPE::TELESCOPE()
    : status(INACTIVE), mainTextureID(0), zoomK(0.0f), changeStatusTime(0), updateTime(0), shipWalker(0), shipsCount(0)
{
...
void TELESCOPE::HandleTargetTest(dword _dTime)
{
    ...
    { // really a ship?
        ...
        if (mainAttributes->GetAttribute("MainCharacter"))
        {
            if (idleTime > SHOW_OBJECT_TIME)
                api->Event("ReleaseTelescopeInfo", NULL);
            return;
        }
        /*
