...
subprocess_create_ex(const char *const command_line[], int options,
                     ...
                     struct subprocess_s *const out_process, const char *const cd = nullptr);
...
int subprocess_create_ex(const char *const commandLine[], int options,
                         const char *const environment[],
                         struct subprocess_s *const out_process, const char * const cd)
{
#if defined(_MSC_VER)
  int fd;
  void *rd, *wr;
  ...
  if (!CreateProcessA(commandLineCombined,
                      SUBPROCESS_NULL,  // command line
