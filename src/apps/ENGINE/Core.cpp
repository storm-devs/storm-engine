...
#define VALIDATE_API_CALLS                                                                                             \
    {                                                                                                                  \
        if (Constructor_counter)                                                                                       \
            SE_THROW_MSG(api call from constructor);                                                                   \
    } // develop version
...
bool CORE::Run()
{
...
#ifndef _XBOX
    if (bDebugWindow && api->Controls && api->Controls->GetDebugAsyncKeyState(VK_F9) < 0)
        DumpEntitiesInfo(); // boal
    ...
    if (Exit_flag)
        return false; // exit
    if (Reset_flag)
        ResetCore(); // reset to initial state
    ...
    if (pVCTime)
        pVCTime->Set((long)GetRDeltaTime());
    ...
    // setup current real computer time
    {
        ...
        if (pVYear)
            pVYear->Set(long(st.wYear));
        if (pVMonth)
            pVMonth->Set(long(st.wMonth));
        if (pVDay)
...
void CORE::ProcessSystemMessage(UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (SystemMessagesNum >= SYSTEM_MESSAGE_STACK_SIZE)
    {
        trace("SMS overflaw");
        return;
    }
...
bool __declspec(noinline) __cdecl CORE::Initialize()
{
    ...
    engine_ini = File_Service.OpenIniFile(ENGINE_INI_FILE_NAME);
    if (engine_ini == null)
        SE_THROW_MSG(no 'engine.ini' file);
    ...
    // create atoms space
    if (!CreateAtomsTable(CORE_DEFAULT_ATOMS_SPACE))
        return false;
...
bool CORE::LoadCoreState(CORE_STATE cs)
{
    ...
    // create atoms space
    if (!CreateAtomsTable(cs.Atoms_space))
        SE_THROW;
...
void __declspec(noinline) __cdecl CORE::ProcessEngineIniFile()
{
    ...
    if (res)
    {
        ...
        if (_CORE_API->Controls == 0)
            _CORE_API->Controls = (CONTROLS *)MakeClass("controls");
    }
    else
    {
        if (Controls)
...
#else
    if (!Controls)
        Controls = (CONTROLS *)MakeClass("xbox_controls");
    if (!Controls)
        SE_THROW("no xbox_controls");
#endif

    res = engine_ini->ReadString(0, "run", String, sizeof(String), "");
    if (res)
    {
        // if(!Program.RunProgram(String)) SE_THROW_MSG(fail to run program);
        if (!Compiler.CreateProgram(String))
            SE_THROW_MSG(fail to create program);
        if (!Compiler.Run())
            SE_THROW_MSG(fail to run program);
        // Тест версии скрипта
    ...
    {
        // delete engine_ini;
        // SE_THROW_MSG(no class for loading);
    }
    while (res)
    {
...
void CORE::CheckAutoExceptions(dword xflag)
{
    ...
    case _X_NO_MEM:
        SE_THROW_MSG(no mem);
    case _X_NO_FILE:
        SE_THROW_MSG(no file);
    case _X_NO_FILE_READ:
        SE_THROW_MSG(cant read from file);
    case _X_NO_FILE_WRITE:
        SE_THROW_MSG(cant write to file);
    case _X_NO_CREATE_ENTITY:
        SE_THROW_MSG(cant create object);
    case _X_NO_CLASS:
        SE_THROW_MSG(cant find class);
    case _X_NO_SERVICE:
        SE_THROW_MSG(cant create service);
    case _X_NO_ENTITY:
        SE_THROW_MSG(cant find entity);
    case _X_NO_LAYER:
        SE_THROW_MSG(no layer);
    default:
        SE_THROW_MSG(invalid exceptions flag);
    }
}
...
bool CORE::CreateEntity(ENTITY_ID *id_PTR, char *class_name, ATTRIBUTES *attributesPTR)
{
    ...
    // if(State_loading) SE_THROW_MSG(attempt to create on load state);

    VMA *pClass;
    long hash;
    bool bClassFound;

    if (_pModuleClassRoot == 0)
        return false;
    ...
    //	trace("( can't create module api class ) ? %s :
    //%s",Modules_Table.GetModuleName(class_search_data.module_code),class_name); 	DeleteAtom(atom_PTR);
    //	CheckAutoExceptions(_X_NO_CREATE_ENTITY);
    //	return false;
    //}
    ...
    catch (...)
    {
        ...
        POP_CONTROL(0)
        System_Api.entityID_PTR = null;
        SE_THROW_MSG(CreateEntity(Constructor));
    }
#endif
    ...
    catch (...)
    {
        ...
        POP_CONTROL(0)
        System_Api.entityID_PTR = null;
        SE_THROW_MSG(CreateEntity(Init));
    }
#endif
...
void CORE::CreationTimeInc()
{
    ...
    do
    {
        if (CoreState.Creation_Time.time[n] == 0xffffffff)
            transfer = true;
        else
            transfer = false;
    ...
    if (transfer && n >= CRTM_DWORDS)
        SE_THROW_MSG(Id time overflaw);
    UNGUARD
}
...
C_ATOM *CORE::CreateAtom(dword class_code)
{
    ...
    if (CoreState.Atoms_number >= CoreState.Atoms_space)
    {
        ...
        if (Atoms_PTR == null)
            SE_THROW_MSG(Resizing Atoms Space Error);
        memset((char *)Atoms_PTR + CoreState.Atoms_space * sizeof(C_ATOM *), 0,
               CoreState.Atoms_space * sizeof(C_ATOM *));
    ...
    for (n = CoreState.Atoms_min_free_orbit; n < CoreState.Atoms_space; n++)
    {
        ...
        if (CoreState.Atoms_max_orbit < n)
            CoreState.Atoms_max_orbit = n;
        ...
        if (CoreState.Atoms_min_free_orbit == n)
            ...
            {
                if (Atoms_PTR[CoreState.Atoms_min_free_orbit] == null)
                    break;
            }
...
C_ATOM *CORE::FitAtom(ENTITY_ID entity_id, ATOM_STATE atom_state)
{
    ...
    if (Atoms_PTR == null)
        SE_THROW;
    if (entity_id.atom_position > CoreState.Atoms_max_orbit)
        SE_THROW;
    if (Atoms_PTR[entity_id.atom_position] != null)
        SE_THROW;
...
bool CORE::DeleteAtom(C_ATOM *atom_PTR)
{
    ...
    dword ap;
    if (Atoms_PTR == null || atom_PTR == null)
        return false;
    ap = atom_PTR->atom_id.atom_position;
    if (ap >= CoreState.Atoms_space)
        SE_THROW;
    if (Atoms_PTR[ap] != atom_PTR)
        SE_THROW;
    delete Atoms_PTR[ap];
    Atoms_PTR[ap] = null;
    CoreState.Atoms_number--;
    if (CoreState.Atoms_min_free_orbit > ap)
        CoreState.Atoms_min_free_orbit = ap;
    UNGUARD
    return true;
}
...
bool CORE::FindClass(ENTITY_ID *id_PTR, char *class_name, dword class_code)
{
    ...
    for (n = 0; n <= CoreState.Atoms_max_orbit; n++)
    {
        ...
        if (Atoms_PTR[n]->atom_id.class_code == hash)
        {
            ...
            if (id_PTR)
                memcpy(id_PTR, &Atoms_PTR[n]->atom_id, sizeof(ENTITY_ID));
            return true;
        }
    }
...
bool CORE::CompareID(ENTITY_ID *ida_PTR, ENTITY_ID *idb_PTR)
{
    ...
    if (memcmp(ida_PTR, idb_PTR, sizeof(ENTITY_ID)) == 0)
        return true;
    UNGUARD
    return false;
}
...
bool CORE::GetEntity(ENTITY_ID *id_PTR)
{
    ...
    if (Scan_Layer_Code == INVALID_LAYER_CODE)
    {
        ...
        do
        {
            if (Atom_Get_Position > CoreState.Atoms_max_orbit)
                return false;
...
ENTITY *CORE::GetEntityPointer(ENTITY_ID *id_PTR)
{
    ...
    VALIDATE_API_CALLS
    if (!ValidateEntity(id_PTR))
        return null;
    ep = (ENTITY *)Atoms_PTR[id_PTR->atom_position]->atom_id.pointer;
    if (Atoms_PTR[id_PTR->atom_position]->as.Deleted)
        return null;
    UNGUARD
    return ep;
}
...
bool CORE::ValidateEntity(ENTITY_ID *id_PTR)
{
    ...
    pos = id_PTR->atom_position;
    if (pos >= CoreState.Atoms_space)
        return false;
    if (Atoms_PTR[pos] == null)
        return false;
...
bool CORE::MarkEntityAsDeleted(void *entity_PTR)
{
    ...
    ENTITY_ID entity_id;
    if (!Convert_Pointer2ID(entity_PTR, &entity_id))
        return false;
...
void CORE::ProcessDeleteList()
{
    ...
    if (deleted_num != 0)
    {
        ...
        {
            if (!DeleteEntityList.GetData(n, &entity_id))
                continue;
    ...
    if (deleted_num != 0)
    {
        for (n = 0; n < deleted_num; n++)
        {
            if (!DeleteLayerList.GetData(n, &dwcode))
                continue;
    ...
    if (deleted_num != 0)
    {
        for (n = 0; n < deleted_num; n++)
        {
            if (!DeleteServicesList.GetData(n, &data))
                continue;
            service_PTR = (SERVICE *)data.pointer;
            if (!service_PTR)
                SE_THROW_MSG(invalid service);
#ifndef EX_OFF
            try
            {
#endif
...
bool CORE::EraseEntity(ENTITY_ID entity_id)
{
    ...
    {
        if (CommonLayers.Layer_Table[n] == null)
            continue;
        if (Atoms_PTR[entity_id.atom_position]->TstLayerAttribute(n, false))
    ...
    {
        for (CoreState.Atoms_max_orbit = entity_id.atom_position; CoreState.Atoms_max_orbit > 0;
             CoreState.Atoms_max_orbit--)
        {
            if (Atoms_PTR[CoreState.Atoms_max_orbit] == null)
                continue;
            break;
        }
    }
...
bool CORE::Convert_Pointer2ID(void *_entity_pointer, ENTITY_ID *id_PTR)
{
    ...
    if (Atoms_PTR == null)
        SE_THROW;
    if (eid.atom_position > CoreState.Atoms_max_orbit)
        SE_THROW;
    if (Atoms_PTR[eid.atom_position] == null)
        SE_THROW;
    if (Atoms_PTR[eid.atom_position]->atom_id.pointer != _entity_pointer)
        SE_THROW;
    if (id_PTR != null)
        memcpy(id_PTR, &Atoms_PTR[eid.atom_position]->atom_id, sizeof(ENTITY_ID));
    UNGUARD
    return true;
}
    ...
    if (CommonLayers.Verify(layer_name))
    {
        ...
        if (layer_PTR)
        {
            ...
            while (id_PTR)
            {
                ...
                if (atom_PTR)
                    atom_PTR->ClrLayerAttribute(CommonLayers.GetIndex(layer_name), false);
...
bool CORE::LayerAdd(const char *pLayerName, ENTITY_ID eid, dword priority, bool system)
{
    ...
    else
    {
        ...
        if (index == INVALID_LAYER_CODE)
            return false;
        if (!ValidateEntity(&eid))
            return false;
        if (Atoms_PTR[eid.atom_position]->TstLayerAttribute(index, false))
            return true;
        if (!CommonLayers.Add(index, eid, priority))
            return false;
...
{
    if (on)
        LayerSetFlags(layer_name, LRFLAG_EXECUTE);
    else
        LayerClrFlags(layer_name, LRFLAG_EXECUTE);
}
...
{
    if (on)
        LayerSetFlags(layer_name, LRFLAG_REALIZE);
    else
        LayerClrFlags(layer_name, LRFLAG_REALIZE);
}
...
void *CORE::MemAllocate(size_t size, char *pFileName, unsigned long nLine)
{
    ...
    CodeSource.pFileName = 0;
    CodeSource.line = 0xffffffff;
    return pPTR;
    UNGUARD
    return 0;
}
void CORE::MemFree(void *block_ptr)
{
    ...
    if (Control_Stack.ScanPointer(block_ptr)) // if this block in executed objects list
    {
        if (Services_List.FindNode((SERVICE *)block_ptr))
        {
            ...
            MarkEntityAsDeleted(block_ptr); // this is entity
        return;
    }
...
dword _cdecl CORE::Send_Message(ENTITY_ID Destination, char *Format, ...)
{
    ...
    MESSAGE message;
    CONTROL_BLOCK cb;
    if (!ValidateEntity(&Destination))
        return 0;            // check for valid destination
...
void *CORE::CreateService(char *service_name)
{
    ...
    catch (...)
    {
        TraceCurrent();
        POP_CONTROL(0);
        SE_THROW_MSG(CreateService : Init func);
    }
#endif
    POP_CONTROL(0)
...
void _cdecl CORE::BTrace(const char *format, ...)
{
    ...
    va_end(args);
    if (!gdi_display.gdi_off)
        gdi_display.Print(buffer_4k);
    else
        trace(buffer_4k);
    Beep(2000, 150);
    UNGUARD
...
void CORE::ProcessExecute()
{
    ...
    for (n = 0; n <= CommonLayers.lss.Layer_max_index; n++)
    {
        ...
        flags = l_PTR->ls.Flags & LRFLAG_EXECUTE;
        if (flags == 0)
            continue; // skip non execute layers
        eid_PTR = l_PTR->GetID();
        while (eid_PTR)
        {
            if (ValidateEntity(eid_PTR))
            {
                if (!Atoms_PTR[eid_PTR->atom_position]->as.Deleted)
                {
                    ...
                    if (pAs->Execute_ticks_max < ticks)
                        pAs->Execute_ticks_max = ticks;
                }
            }
...
void CORE::ProcessRealize()
{
    ...
    for (n = 0; n <= CommonLayers.lss.Layer_max_index; n++)
    {
        ...
        if (flags != 0)
            continue; // skip frozen layers
        flags = l_PTR->ls.Flags & LRFLAG_REALIZE;
        if (flags == 0)
            continue; // skip non realize layers
        eid_PTR = l_PTR->GetID();
        while (eid_PTR)
        {
            if (ValidateEntity(eid_PTR))
            {
                if (!Atoms_PTR[eid_PTR->atom_position]->as.Deleted)
                {
                    ...
                    if (pAs->Realize_ticks_max < ticks)
                        pAs->Realize_ticks_max = ticks;
                }
            }
            eid_PTR = l_PTR->GetNextID();
        }
...
void CORE::ProcessSystemMessagesBuffer()
{
    ...
    for (n = 0; n <= CommonLayers.lss.Layer_max_index; n++)
    {
        ...
        if (flags != 0)
            continue; // skip frozen layers
        flags = l_PTR->ls.Flags & LRFLAG_SYS_MESSAGES;
        if (flags == 0)
            ...
            if (ValidateEntity(eid_PTR))
            {
                if (!Atoms_PTR[eid_PTR->atom_position]->as.Deleted)
                    for (i = 0; i < SystemMessagesNum; i++)
                    {
                        ...
                        ((ENTITY *)eid_PTR->pointer)
                            ->ProcessMessage(MessageStack[i].iMsg, MessageStack[i].wParam, MessageStack[i].lParam);
                        POP_CONTROL(0)
                    }
            }
            eid_PTR = l_PTR->GetNextID();
...
void CORE::TraceCurrent()
{
    ...
    if (Control_Stack.Read(&cb))
    {
        if (cb.ctp != CTP_CORE)
        {
            ...
            if (pClass)
                strcpy(gstring, pClass->GetName());
            trace("Object: %s -> %s",
                  // Classes_Table.GetString(cb.class_code),
                  gstring, CTP_NAME(cb.ctp));
            return;
        }
    }
    trace("Object: CORE");
...
bool CORE::SaveState(char *file_name)
{
    ...
    */
    ...
    esg.SetState("u", CommonLayers.lss.Layers_number);
    esg.CloseState();
    esg.SetState("m", sizeof(CommonLayers.lss), &CommonLayers.lss);
    for (i = 0; i < _MAX_LAYERS; i++)
    {
        if (CommonLayers.Layer_Table[i] == null)
            continue;
        ...
        for (n = 0; n < layer_objects; n++)
        {
            if (n == 0)
            {
                id_PTR = CommonLayers.Layer_Table[i]->GetID(&Priority);
            }
            else
            {
                id_PTR = CommonLayers.Layer_Table[i]->GetNextID(&Priority);
            }
            if (id_PTR == null)
                SE_THROW;
...
void CORE::ProcessStateLoading()
{
    ...
    if (nPackedSize == INVALID_FILE_SIZE)
    {
        fio->_CloseHandle(pfh);
        return;
    }
    ...
    if (cs.engine_version != CoreState.engine_version)
        SE_THROW_MSG(incorrect state file);

    // validate space size
    if (cs.Atoms_max_orbit >= cs.Atoms_space)
        SE_THROW;
    ...
    //	if(classes_count != Classes_Table.GetStringsCount()) SE_THROW_MSG(incorrect state file);
    ...
    /*	for(n=0;n<Classes_Table.GetStringsCount();n++)
        {
            ...
            if(stricmp(v_Buff.Ptr,char_PTR) != 0) SE_THROW_MSG(incorrect state file);
        }
    */
    ...
    for (n = 0; n < entities_in_state; n++)
    {
        ...
        try
        {
            ...
            if (!entity_PTR->LoadState(&es))
            {
                ...
                TraceCurrent();
                SE_THROW_MSG(state loading error);
            }
            POP_CONTROL(0)
#ifndef EX_OFF
        }
...
void CORE::DumpEntitiesInfo()
{
    ...
    for (n = 0; n <= CoreState.Atoms_max_orbit; n++)
    {
        ...
        {
            trace("Class: %s", ptr);
            trace("     : Realize:  Cur( %d ) Max( %d )", Atoms_PTR[n]->as.Realize_ticks_av,
                  Atoms_PTR[n]->as.Realize_ticks_max);
            trace("     : Execute:  Cur( %d ) Max( %d )", Atoms_PTR[n]->as.Execute_ticks_av,
                  Atoms_PTR[n]->as.Execute_ticks_max);
        }
    }
