...
void C_ATOM::SetLayerAttribute(dword index, bool system)
{
    ...
    if (system)
        as.SysLayers_mask[dword_number] |= layer_flag;
    else
        as.Layers_mask[dword_number] |= layer_flag;
}
...
bool C_ATOM::TstLayerAttribute(dword index, bool system)
{
    ...
    layer_flag = (1 << (index - dword_number * 32));
    if (system)
        layer_flag = layer_flag & as.SysLayers_mask[dword_number];
    else
        layer_flag = layer_flag & as.Layers_mask[dword_number];
    if (layer_flag != 0)
        return true;
    return false;
}
