...
#define PUSH_CONTROL(ptr, code, c)                                                                                     \
    { /*Control_Stack.Push(ptr,code,c);*/                                                                              \
        Control_Block.pointer = ptr;                                                                                   \
        Control_Block.class_code = code;                                                                               \
        Control_Block.ctp = c;                                                                                         \
        Memory_Service.SetState((word)c, code);                                                                        \
        Control_Stack.Push(ptr, code, c);                                                                              \
    }
#define POP_CONTROL(control_block_ptr)                                                                                 \
    {                                                                                                                  \
        Control_Stack.Pop(control_block_ptr);                                                                          \
        Control_Stack.Read(&Control_Block);                                                                            \
        Memory_Service.SetState((word)Control_Block.ctp, Control_Block.class_code);                                    \
    }
