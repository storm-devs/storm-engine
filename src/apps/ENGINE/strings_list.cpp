...
dword STRINGS_LIST::GetStringsCount()
{
    return Strings;
}
...
{
    if (code >= Strings || String_Table_PTR == null)
        return null;
...
bool STRINGS_LIST::AddString(char *_char_PTR)
{
    ...
    if (String_Table_PTR[Strings] == null)
        return false;
...
void STRINGS_LIST::Release()
{
    ...
    dword n;
    if (Strings <= 0 || String_Table_PTR == null)
        return;
    for (n = 0; n < Strings; n++)
...
{
    if (code >= Strings || String_Table_PTR == null || data_PTR == null)
        return false;
