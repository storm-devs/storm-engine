...
void SOURCE_VIEW::DoStep(long iCount)
{
    ...
    if (iNewActiveLine >= nLinesNum)
        iNewActiveLine = int(nLinesNum - 1);
    ...
    if (CDebug.SourceView)
        ...
        case WM_KEYDOWN:
            ...
            case 'C':
                ...
                if (OpenClipboard(0)) // hwnd))
                {
                    ...
                    dwBytes =
                        abs((long)CDebug.SourceView->nEndSelection - (long)CDebug.SourceView->nStartSelection) + 1;
        ...
        case WM_LBUTTONUP:
            ...
            if (CDebug.SourceView->bDrag)
            {
                ...
                if (CDebug.SourceView->sCopyPasteBuffer.Len() &&
                    WindowFromPoint(pnt) == CDebug.WatcherList->GetWindowHandle())
                {
                    ...
                    for (long i = 0; i < iItemCount; i++)
                    {
                        if (ListView_GetItemState(CDebug.WatcherList->GetWindowHandle(), i, LVIS_SELECTED) &
                            LVIS_SELECTED)
                        {
                            CDebug.WatcherList->SetItemText(i, 0,
                                                            (char *)CDebug.SourceView->sCopyPasteBuffer.GetBuffer());
        ...
        case WM_LBUTTONDOWN:
...

            {
                ...
                bool bCursorInSelection = r.top < yPos && r.bottom > yPos && r.left < xPos && r.right > xPos;
        ...
        case WM_VSCROLL:
            ...
            case SB_LINEUP:
                ...
                break;
            case SB_THUMBTRACK:
                if (CDebug.SourceView->nTopLine == (DWORD)nPos)
                    break;
...
SOURCE_VIEW::SOURCE_VIEW(HWND _hMain, HINSTANCE _hInst) : htBookmarks(__FILE__, __LINE__, 512), aStrings(_FL_)
{
    ...
    hOwn = CreateWindowEx(WS_EX_CLIENTEDGE, SVClass, SVClass, WS_CHILD | WS_VISIBLE | WS_VSCROLL, 0, 0,
                          Pos.right - Pos.left, Pos.bottom - Pos.top, hMain, NULL, hInst, NULL);
    if (hOwn == NULL)
...
bool SOURCE_VIEW::OpenSourceFile(const char *_filename)
{
    ...
    if (stricmp(SourceFileName, _filename) == 0)
        return true;
    ...
    if (pSourceFile)
    {
        ...
        for (n = nTopLine; n < nTopLine + nClientLinesSize + 1; n++)
        {
...

            {
                ...
                {
                    TabbedTextOut(dc, x, y, TextW.c_str(), nTextLen, 0, 0, 0);
                }
                else
                {
                    if (nStartSelection < nEndSelection)
                    ...
                    if (nFrom > 0)
                    {
                        w = LOWORD(TabbedTextOut(dc, x, y, TextW.c_str(), nFrom, 0, 0, 0));
                        x += w;
                    }
                    ...
                    w = LOWORD(TabbedTextOut(dc, x, y, TextW.c_str() + nFrom, nTo - nFrom, 0, 0, 0));
                    x += w;
                    ...
                    SetTextColor(dc, WRGB(0, 0, 0));
                    TabbedTextOut(dc, x, y, TextW.c_str() + nTo, nTextLen - nTo, 0, 0, 0);
...
void SOURCE_VIEW::StartSelection(DWORD x_pos)
{
    ...
    while (!(pLine[nSymNum] == 0xd || pLine[nSymNum] == 0xa || pLine[nSymNum] == 0))
    {
        ...
        BeginPath(dc);
        TabbedTextOut(dc, X_OFFSET, 0, LineW.c_str(), nSymNum + 1, 0, 0, 0);
        EndPath(dc);
        r = PathToRegion(dc);
        if (r)
        {
...
void SOURCE_VIEW::InvalidateLineSection(DWORD line, DWORD r1, DWORD r2)
{
    ...
    SelectionRect.bottom = SelectionRect.top + nFontHeight;
    SelectionRect.left = LOWORD(GetTabbedTextExtent(dc, LineW.c_str(), r1 + 1, 0, 0));
    SelectionRect.right = LOWORD(GetTabbedTextExtent(dc, LineW.c_str(), r2 + 2, 0, 0));
...
void SOURCE_VIEW::DetCursorPos(DWORD x_pos, DWORD y_pos)
{
    ...
    while (!(pLine[nSymNum] == 0xd || pLine[nSymNum] == 0xa || pLine[nSymNum] == 0))
    {
        nLen = LOWORD(GetTabbedTextExtent(dc, LineW.c_str(), nSymNum + 1, 0, 0));
...
bool SOURCE_VIEW::SetVariableOnChange(const char *pString, bool bSet)
{
    ...
    if (sToken == "[")
    {
        ...
        VDATA *pV = (VDATA *)_CORE_API->GetScriptVariable(sVarName, null);
        if (!pV)
            return false;
...
INT_PTR CALLBACK VarChangeDialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: {
        ...
        col.fmt = LVCFMT_LEFT;
        col.cx = 378;
        col.pszText = TEXT("Variable name");
