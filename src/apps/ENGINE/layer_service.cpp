...
void LAYER_SERVICE::Fit(dword index, char *layer_name, LAYER_STATE ls)
{
    ...
    if (layer_name == null)
        SE_THROW_MSG(zero name);
    if (index > lss.Layer_max_index)
        SE_THROW_MSG(invalid index);
...
bool LAYER_SERVICE::Create(char *layer_name, bool ordered, bool fail_if_exist)
{
    ...
    for (n = 0; n < _MAX_LAYERS; n++)
    {
        ...
        if (Layer_Table[n] == null)
            SE_THROW;
        if (lss.Layer_max_index < n)
            lss.Layer_max_index = n;
        lss.Layers_number++;
        return true;
    }
    CheckAutoExceptions();
...
void LAYER_SERVICE::Erase(dword index)
{
    ...
    {
        SE_THROW_SEVERITY_MSG(NON_FATAL, ghost layer);
    }
    l_PTR = Layer_Table[index];
    if (l_PTR == null)
    {
        SE_THROW_SEVERITY_MSG(NON_FATAL, ghost layer);
    }
...
void LAYER_SERVICE::Delete(char *layer_name)
{
    ...
    ToClean = true;
    if (lss.Scan_layer_code == index)
        lss.Scan_layer_code = INVALID_LAYER_CODE;
    UNGUARD
}
...
{
    if (index == INVALID_LAYER_CODE || index > lss.Layer_max_index)
        return null;
    return Layer_Table[index];
}
...
{
    if (Layer_Table[index] == 0)
        SE_THROW_MSG(invalid layer index);
    if (!Layer_Table[index]->Add(eid, priority))
        SE_THROW;
    return true;
}
