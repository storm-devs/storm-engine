    ...
    for (dword k = 0; k < aForts.Size(); k++)
    {
        ...
        for (dword i = 0; i < iMax; i++)
        {
            ...
            if (!pC->isFired() && pC->isReady2Fire() && bFiredTimer)
            {
                ...
                for (dword j = 0; j < AIShip::AIShips.Size(); j++)
...
bool AIFort::AddFort(ATTRIBUTES *pIslandAP, ATTRIBUTES *pFortLabelAP, ATTRIBUTES *pFortCharacter, ENTITY_ID eidModel,
                     ENTITY_ID eidBlot)
{
    Assert(pFortLabelAP);
...
void AIFort::AddFortHit(long iCharacterIndex, CVECTOR &vHitPos)
{
    ...
    for (i = 0; i < aForts.Size(); i++)
    {
        ...
        for (j = 0; j < iMax; j++)
        {
            ...
            // VDATA * pVData = api->Event(FORT_CANNON_DAMAGE, "llallfffff", iCharacterIndex,
...
AIFort::AI_FORT *AIFort::FindFort(ENTITY_ID eidModel)
{
    ...
    {
        if (aForts[i]->GetModelEID().pointer == eidModel.pointer)
            return aForts[i];
    }
    return null;
}
