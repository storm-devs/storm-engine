...
void AIShip::Execute(float fDeltaTime)
{
    ...
    SHIP_BASE *pShip = (SHIP_BASE *)GetShipPointer();
    Assert(pShip);
...
void AIShip::CheckSituation()
{
    ...
    for (dword i = 0; i < AIShips.Size(); i++)
        if (AIShips[i] != this)
        {
...
void AIShip::CreateShip(ENTITY_ID _eidShip, ATTRIBUTES *_pACharacter, ATTRIBUTES *_pAShipBase, CVECTOR *vInitPos)
{
    ...
    eidShip = _eidShip;
    Assert(api->ValidateEntity(&eidShip));
...
AIShip *AIShip::FindShip(ATTRIBUTES *pACharacter)
{
    for (dword i = 0; i < AIShip::AIShips.Size(); i++)
        if (*AIShip::AIShips[i] == pACharacter)
            return AIShip::AIShips[i];
    return null;
}
...
void AIShip::ShipSetRunAway(dword dwPriority, ATTRIBUTES *pACharacter1)
{
    AIShip *pShip = FindShip(pACharacter1);
    if (pShip)
        pShip->GetTaskController()->SetNewTask(dwPriority, AITASK_RUNAWAY, null);
}
...
void AIShip::ShipSetDrift(dword dwPriority, ATTRIBUTES *pACharacter1)
{
    AIShip *pShip = FindShip(pACharacter1);
    if (pShip)
        pShip->GetTaskController()->SetNewTask(dwPriority, AITASK_DRIFT, null);
}
