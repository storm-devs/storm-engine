...
float AITemplate::GetPercent(info_t *pI)
{
    ...
    if (Info.iOurNation < 0 || Info.iOurNation == pI->iOurNation)
        fPercent++;
    if (Info.iEnemyNation < 0 || Info.iEnemyNation == pI->iEnemyNation)
        fPercent++;

    // trade ships
    if (Info.iOurTradeNum < 0 || Info.iOurTradeNum == pI->iOurTradeNum)
        fPercent++;
    else
    {
    ...
    if (Info.iEnemyTradeNum < 0 || Info.iEnemyTradeNum == pI->iEnemyTradeNum)
        fPercent++;
    else
    {
    ...
    // war ships
    if (Info.iOurWarNum < 0 || Info.iOurWarNum == pI->iOurWarNum)
        fPercent++;
    else
    {
    ...
    if (Info.iEnemyWarNum < 0 || Info.iEnemyWarNum == pI->iEnemyWarNum)
        fPercent++;
    else
    {
