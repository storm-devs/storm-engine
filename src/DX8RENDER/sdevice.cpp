...
DWORD __cdecl DX8SetTexturePath(VS_STACK *pS)
{
    ...
    if (!DX8RENDER::pRS)
    {
        api->CreateService("dx8render");
        Assert(DX8RENDER::pRS);
    }
...
DWORD __cdecl RPrint(VS_STACK *pS)
{
    ...
    if (pString->GetString())
        DX8RENDER::pRS->Print(x, y, pString->GetString());
    VDATA *pVR = (VDATA *)pS->Push();
    pVR->Set(long(0));
    return IFUNCRESULT_OK;
}
...
#define CalcKColor(ind)                                                                                                \
    {                                                                                                                  \
        kColor = light | !CVECTOR(DX8sphereVertex[t * 3 + ind].v.x, DX8sphereVertex[t * 3 + ind].v.y,                  \
                                  DX8sphereVertex[t * 3 + ind].v.z);                                                   \
        if (kColor < 0.0f)                                                                                             \
            kColor = 0.0f;                                                                                             \
    }
...
D3DERRORS D3D_ERRORS[] = {
    ...
    {D3DERR_CONFLICTINGTEXTUREPALETTE,
     "The current textures cannot be used simultaneously This generally occurs when a multitexture device requires "
     "that all palletized textures simultaneously enabled also share the same palette"},
...
__forceinline bool DX8RENDER::ErrorHandler(char *message, HRESULT hres)
{
    ...
    for (long e = 0;; e++)
    {
        if (D3D_ERRORS[e].descrition == NULL)
            break;
    ...
    api->Trace(message);
    api->Trace("Undefined error %d", (int)hres);
    return true;
}

const dword DX8RENDER::rectsVBuffer_SizeInRects = 512;
// const dword DX8RENDER::rectsVBuffer_SizeInRects = 2048;

//################################################################################
DX8RENDER::DX8RENDER() : stRenderTarget(_FL_), aCaptureBuffers(_FL_)
{
    rectsVBuffer = null;
...
bool DX8RENDER::Init()
{
    ...
    if (ini)
    {
        ...
        if (stricmp(str, "D3DFMT_A8R8G8B8") == 0)
        {
            screen_bpp = D3DFMT_A8R8G8B8;
            stencil_format = D3DFMT_D24S8;
        }
        if (stricmp(str, "D3DFMT_X8R8G8B8") == 0)
        {
            screen_bpp = D3DFMT_X8R8G8B8;
            stencil_format = D3DFMT_D24S8;
        }
        if (stricmp(str, "D3DFMT_R5G6B5") == 0)
        {
            screen_bpp = D3DFMT_R5G6B5;
            stencil_format = D3DFMT_D16;
        }
    ...
    d3d9->CreateVertexBuffer(rectsVBuffer_SizeInRects * 6 * sizeof(RECT_VERTEX), D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC,
                             RS_RECT_VERTEX_FORMAT, D3DPOOL_DEFAULT, &rectsVBuffer, NULL); // D3DPOOL_MANAGED
...
DX8RENDER::~DX8RENDER()
{
    ...
    progressImage = null;
    if (progressBackImage)
        delete progressBackImage;
    progressBackImage = null;
    if (progressTipsImage)
...
bool DX8RENDER::InitDevice(bool windowed, HWND _hwnd, long width, long height)
{
        ...
        D3D_OK)
    {
        if (ErrorHandler("InitDevice::CreateDevice",
                         d3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, hwnd,
                                           D3DCREATE_SOFTWARE_VERTEXPROCESSING, &d3dpp, &d3d9)) == true)
            return false;
    }
    ...
    d3d9->CreateTexture(width, height, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &pPostProcessTexture,
                        NULL);
    d3d9->CreateTexture((int)fSmallWidth, (int)fSmallHeight, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT,
                        &pSmallPostProcessTexture, NULL);
    d3d9->CreateTexture((int)(fSmallWidth * 2.0f), (int)(fSmallHeight * 2.0f), 1, D3DUSAGE_RENDERTARGET,
                        D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &pSmallPostProcessTexture2, NULL);
...
bool DX8RENDER::ReleaseDevice()
{
    ...
    for (long t = 0; t < MAX_STEXTURES; t++)
    {
        ...
        {
            if (ErrorHandler("Release texture", Textures[t].d3dtex->Release()) == false)
                res = false;
            Textures[t].ref = NULL;
            if (Textures[t].name != NULL)
    ...
    //	if (d3d9!=NULL && ErrorHandler("Release d3d9", d3d9->Release()) == false)	res = false;
    //	d3d9 = NULL;
...
{
    if (ErrorHandler("Clear", d3d9->Clear(0L, NULL, type, dwBackColor, 1.0f, 0L)) == true)
        return false;
    return true;
}
...
bool DX8RENDER::DX8BeginScene()
{
    ...
    dwNumLV = 0;
    dwNumLI = 0;
    if (ErrorHandler("BeginScene", d3d9->BeginScene()) == true)
        return false;
    return true;
}

void DX8RENDER::CreateRenderQuad(float fWidth, float fHeight, float fSrcWidth, float fSrcHeight, float fMulU,
                                 float fMulV)
{
    float StartX = -0.5f;
    float StartY = -0.5f;
...
void DX8RENDER::BlurGlowTexture()
{
    ...
    for (int i = 0; i < iBlurPasses; i++)
    {
        ...
        d3d9->SetRenderTarget(0, pSmallPostProcessSurface);
...
void DX8RENDER::CopyPostProcessToScreen()
{
    ...
    if (bSeaEffect)
    {
        d3d9->SetVertexShader(NULL);
        d3d9->SetFVF(POST_PROCESS_FVF);
...
{
    HRESULT hr = d3d9->SetRenderTarget(0, pSurf);
    hr = d3d9->BeginScene();
...
{
    HRESULT hr = d3d9->SetRenderTarget(0, pOriginalScreenSurface);
    hr = d3d9->SetDepthStencilSurface(pOriginalDepthSurface);
    SetViewport(&OriginalViewPort);
}

void DX8RENDER::SetPostProcessTextureAsRenderTarget()
{
    HRESULT hr = d3d9->SetRenderTarget(0, pPostProcessSurface);
    hr = d3d9->SetDepthStencilSurface(pOriginalDepthSurface);
    SetViewport(&OriginalViewPort);
}
...
void DX8RENDER::MakePostProcess()
{
    ...
    IDirect3DStateBlock9 *sbToken = NULL;
    d3d9->CreateStateBlock(D3DSBT_ALL, &sbToken);
    sbToken->Capture();
...
bool DX8RENDER::DX8EndScene()
{
    ...
    if (bShowFps)
    {
        ...
        for (t = 0; t < MAX_BUFFERS; t++)
        {
            ...
            {
                dwTotalIBSize += IndexBuffers[t].ntrgs;
                dwTotalIB++;
            }
        }
    }
    ...
    if (bDropVideoConveyor && pDropConveyorVBuffer)
    {
        ...
        d3d9->SetVertexShader(NULL);
        d3d9->SetFVF(D3DFVF_XYZ);
        DrawPrimitive(D3DPT_LINELIST, 0, 1);
    }

    if (ErrorHandler("EndScene", d3d9->EndScene()))
        return false;
...
void DX8RENDER::RecoverDevice()
{
    HRESULT hr = NULL;

    hr = d3d9->TestCooperativeLevel();
...
long DX8RENDER::RestoreRender()
{
    d3d9->GetRenderTarget(0, &pOriginalScreenSurface);
    d3d9->GetDepthStencilSurface(&pOriginalDepthSurface);

    d3d9->CreateVertexBuffer(rectsVBuffer_SizeInRects * 6 * sizeof(RECT_VERTEX), D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC,
                             RS_RECT_VERTEX_FORMAT, D3DPOOL_DEFAULT, &rectsVBuffer, NULL);
    ...
    OriginalViewPort.X = 0;
    OriginalViewPort.Y = 0;
    OriginalViewPort.Width = screen_size.x;
    OriginalViewPort.Height = screen_size.y;
    OriginalViewPort.MinZ = 0.0f;
    OriginalViewPort.MaxZ = 1.0f;
    ...
    */
    ...
    for (long b = 0; b < MAX_BUFFERS; b++)
    {
        if (VertexBuffers[b].size > 0)
        {
            if (ErrorHandler("CreateVertexBuffer::CreateVertexBuffer",
                             d3d9->CreateVertexBuffer(VertexBuffers[b].size, VertexBuffers[b].usage,
                                                      VertexBuffers[b].type, D3DPOOL_DEFAULT, &VertexBuffers[b].buff,
                                                      NULL)) == true)
            {
                return -1;
            }
        }
        if (IndexBuffers[b].ntrgs > 0)
        {
            if (ErrorHandler("CreateIndexBuffer::CreateIndexBuffer",
                             d3d9->CreateIndexBuffer(IndexBuffers[b].ntrgs, IndexBuffers[b].usage, D3DFMT_INDEX16,
                                                     D3DPOOL_DEFAULT, &IndexBuffers[b].buff, NULL)) == true)
            {
                return -1;
            }
        }
    ...
    if (api->GetEntity(&eid))
        do
        {
            ((ENTITY *)eid.pointer)->RestoreRender();
        } while (api->GetEntityNext(&eid));
...
long DX8RENDER::TextureCreate(const char *fname)
{
    ...
    if (iSetupPath)
    {
        ...
        if (fname && fname[0])
            strcpy(TexPaths[dwSetupNumber].str, fname);
        iSetupPath = 0;
        return -1;
    }
...
bool DX8RENDER::TextureLoad(long t)
{
    ...
    if (!(head.flags & TX_FLAGS_CUBEMAP))
    {
        ...
        //Позиция в файле
        if (seekposition)
            api->fio->_SetFilePointer(file, seekposition, 0, FILE_CURRENT);
        //Создаём текстуру
        IDirect3DTexture9 *tex = null;
        if (ErrorHandler("CreateTexture", d3d9->CreateTexture(head.width, head.height, head.nmips, 0, d3dFormat,
                                                              D3DPOOL_MANAGED, &tex, NULL)) == true ||
            !tex)
        {
            if (bTrace)
                api->Trace(
    ...
    else
    {
        ...
        if (ErrorHandler("d3d9->CreateCubeTexture", d3d9->CreateCubeTexture(head.width, head.nmips, 0, d3dFormat,
                                                                            D3DPOOL_MANAGED, &tex, null)) == true ||
            !tex)
        {
            if (bTrace)
    ...
    if (texLog)
    {
        char s[256];
        if (totSize == 0)
            api->fio->_DeleteFile("texLoad.txt");
        ...
        sprintf(s, "%.2f, size: %d, %d * %d, %s   %p\n", totSize / 1024.0f / 1024.0f, Textures[t].dwSize, head.width,
                head.height, Textures[t].name, Textures[t].d3dtex);
...
dword DX8RENDER::LoadCubmapSide(HANDLE file, IDirect3DCubeTexture9 *tex, D3DCUBEMAP_FACES face, dword numMips,
                                dword mipSize, dword size, bool isSwizzled)
{
    dword texsize = 0;
    //Заполняем уровни
...
bool DX8RENDER::LoadTextureSurface(HANDLE file, IDirect3DSurface9 *suface, dword mipSize, dword width, dword height,
                                   bool isSwizzled)
{
    //Указатель на поверхность
    D3DLOCKED_RECT lock;
    if (ErrorHandler("suface->LockRect", suface->LockRect(&lock, NULL, 0L)) == true)
        return false;
    //Зачитывание
    DWORD readingBytes = 0;
    if (!api->fio->_ReadFile(file, lock.pBits, mipSize, &readingBytes) || readingBytes != mipSize)
    {
        if (ErrorHandler("suface->UnlockRect", suface->UnlockRect()) == true)
            return false;
        return false;
    }
...
bool DX8RENDER::TextureSet(long stage, long texid)
{
    if (texid == -1)
    {
        if (ErrorHandler("SetTexture", d3d9->SetTexture(stage, NULL)) == true)
            return false;
        return true;
    }

    if (ErrorHandler("SetTexture", d3d9->SetTexture(stage, Textures[texid].d3dtex)) == true)
        return false;
    return true;
}
...
bool DX8RENDER::TextureRelease(long texid)
{
    ...
    if (Textures[texid].name != NULL)
    {
        ...
        {
            HANDLE fh =
                api->fio->_CreateFile("texLoad.txt", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, OPEN_ALWAYS);
    ...
    if (Textures[texid].d3dtex)
        if (ErrorHandler("Release", Textures[texid].d3dtex->Release()) == true)
            return false;
...
bool DX8RENDER::SetPerspective(float perspective, float fAspectRatio)
{
    ...
    if (ErrorHandler("SetPerspective::SetTransform", d3d9->SetTransform(D3DTS_PROJECTION, &mtx)) == true)
        return false;
    Fov = perspective;
    FindPlanes(d3d9);
    return true;
...
{
    if (ErrorHandler("SetMaterial::SetMaterial", d3d9->SetMaterial(&m)))
        return false;
    return true;
}
bool DX8RENDER::SetLight(dword dwIndex, const D3DLIGHT9 *pLight)
{
    ...
    if (ErrorHandler("SetLight::SetLight", d3d9->SetLight(dwIndex, &tmpLight)))
        return false;
    return true;
}
...
{
    if (ErrorHandler("LightEnable::LightEnable", d3d9->LightEnable(dwIndex, bOn)))
        return false;
    return true;
}
...
{
    if (ErrorHandler("LightEnable::LightEnable", d3d9->GetLightEnable(dwIndex, pEnable)))
        return false;
    return true;
}
...
{
    if (ErrorHandler("LightEnable::LightEnable", d3d9->GetLight(dwIndex, pLight)))
        return false;
...
long DX8RENDER::CreateVertexBuffer(long type, long size, dword dwUsage)
{
    ...
    if (ErrorHandler("CreateVertexBuffer::CreateVertexBuffer",
                     d3d9->CreateVertexBuffer(size, dwUsage, type, D3DPOOL_DEFAULT, &VertexBuffers[b].buff, NULL)) ==
        true)
        return -1;

    VertexBuffers[b].type = type;
    VertexBuffers[b].size = size;
    VertexBuffers[b].usage = dwUsage;
    VertexBuffers[b].isManaged = false;

    return b;
}

long DX8RENDER::CreateVertexBufferManaged(long type, long size, dword dwUsage)
{
    long b = 0;
    if (size <= 0)
        return -1; // fix
    ...
    if (ErrorHandler("CreateVertexBuffer::CreateVertexBuffer",
                     d3d9->CreateVertexBuffer(size, dwUsage, type, D3DPOOL_MANAGED, &VertexBuffers[b].buff, NULL)) ==
        true)
        return -1;
    ...
    VertexBuffers[b].size = size;
    VertexBuffers[b].usage = dwUsage;
    VertexBuffers[b].isManaged = true;
...
long DX8RENDER::CreateIndexBuffer(long size, dword dwUsage)
{
    ...
    if (ErrorHandler("CreateIndexBuffer::CreateIndexBuffer",
                     d3d9->CreateIndexBuffer(size, dwUsage, D3DFMT_INDEX16, D3DPOOL_DEFAULT, &IndexBuffers[b].buff,
                                             NULL)) == true)
    {
        api->Trace("CreateIndexBuffer %d %d", size, dwUsage);
        return -1;
    }
    IndexBuffers[b].ntrgs = size;
    IndexBuffers[b].usage = dwUsage;
    IndexBuffers[b].isManaged = false;

    return b;
}

long DX8RENDER::CreateIndexBufferManaged(long size, dword dwUsage)
{
    long b = 0;
    for (b = 0; b < MAX_BUFFERS; b++)
        if (IndexBuffers[b].buff == NULL)
    ...
    if (ErrorHandler("CreateIndexBuffer::CreateIndexBuffer",
                     d3d9->CreateIndexBuffer(size, dwUsage, D3DFMT_INDEX16, D3DPOOL_MANAGED, &IndexBuffers[b].buff,
                                             NULL)) == true)
    {
        api->Trace("CreateIndexBuffer %d %d", size, dwUsage);
        return -1;
    ...
    IndexBuffers[b].ntrgs = size;
    IndexBuffers[b].usage = dwUsage;
    IndexBuffers[b].isManaged = true;

    return b;
}
//################################################################################
void DX8RENDER::DrawBuffer(long vbuff, long stride, long ibuff, long minv, long numv, long startidx, long numtrg,
                           const char *cBlockName, dword dwNumParams, ...)
{
    bool bDraw = true;
    ...
    {
        d3d9->SetVertexShader(NULL);
        if (ErrorHandler("DrawBuffer::SetVertexShader", d3d9->SetFVF(VertexBuffers[vbuff].type)) == true)
            return;
    }

    if (ErrorHandler("DrawBuffer::SetIndices", d3d9->SetIndices(IndexBuffers[ibuff].buff)) == true)
        return;

    if (vbuff >= 0)
        if (ErrorHandler("DrawBuffer::SetStreamSource",
                         d3d9->SetStreamSource(0, VertexBuffers[vbuff].buff, 0, stride)) == true)
            return;
...
void DX8RENDER::DrawIndexedPrimitiveNoVShader(D3DPRIMITIVETYPE dwPrimitiveType, long iVBuff, long iStride, long iIBuff,
                                              long iMinV, long iNumV, long iStartIdx, long iNumTrg,
                                              const char *cBlockName, dword dwNumParams, ...)
{
    bool bDraw = true;

    if (ErrorHandler("DrawIndexedPrimitiveNoVShader::SetIndices", d3d9->SetIndices(IndexBuffers[iIBuff].buff)) == true)
        return;

    if (iVBuff >= 0)
    {
        if (ErrorHandler("DrawIndexedPrimitiveNoVShader::SetStreamSource",
                         d3d9->SetStreamSource(0, VertexBuffers[iVBuff].buff, 0, iStride)) == true)
            return;
    }
    ...
    if (bDraw)
        do
        {
            ...
            ErrorHandler("DrawIndexedPrimitiveNoVShader::DrawIndexedPrimitive",
                         d3d9->DrawIndexedPrimitive(dwPrimitiveType, iMinV, 0, iNumV, iStartIdx, iNumTrg));
        } while (cBlockName && TechniqueExecuteNext());
}

void DX8RENDER::DrawIndexedPrimitiveUP(D3DPRIMITIVETYPE dwPrimitiveType, dword dwMinIndex, dword dwNumVertices,
                                       dword dwPrimitiveCount, const void *pIndexData, D3DFORMAT IndexDataFormat,
                                       const void *pVertexData, dword dwVertexStride, const char *cBlockName,
                                       dword dwNumParams, ...)
{
    bool bDraw = true;
    if (cBlockName)
    ...
    if (bDraw)
        do
        {
            ...
            ErrorHandler("DrawIndexedPrimitiveUP::DrawIndexedPrimitiveUP",
                         d3d9->DrawIndexedPrimitiveUP(dwPrimitiveType, dwMinIndex, dwNumVertices, dwPrimitiveCount,
                                                      pIndexData, IndexDataFormat, pVertexData, dwVertexStride));
        } while (cBlockName && TechniqueExecuteNext());
}

void DX8RENDER::DrawPrimitiveUP(D3DPRIMITIVETYPE dwPrimitiveType, dword dwVertexBufferFormat, dword dwNumPT,
                                void *pVerts, dword dwStride, const char *cBlockName, dword dwNumParams, ...)
{
    bool bDraw = true;

    d3d9->SetVertexShader(NULL);
    if (ErrorHandler("DrawPrimitiveUP::SetVertexShader", d3d9->SetFVF(dwVertexBufferFormat)) == true)
        return;
    ...
    if (bDraw)
        do
        {
            ...
            ErrorHandler("DrawPrimitiveUP::DrawPrimitiveUP",
                         d3d9->DrawPrimitiveUP(dwPrimitiveType, dwNumPT, pVerts, dwStride));
        } while (cBlockName && TechniqueExecuteNext());
}

void DX8RENDER::DrawPrimitive(D3DPRIMITIVETYPE dwPrimitiveType, long iVBuff, long iStride, long iStartV, long iNumPT,
                              const char *cBlockName, dword dwNumParams, ...)
{
    bool bDraw = true;
    ...
    if (ErrorHandler("DrawPrimitive::SetVertexShader", d3d9->SetFVF(VertexBuffers[iVBuff].type)) == true)
        return;
    ...
    if (bDraw)
        ...
        {
            dwNumDrawPrimitive++;
            ErrorHandler("DrawPrimitive::DrawPrimitive", d3d9->DrawPrimitive(dwPrimitiveType, iStartV, iNumPT));
        } while (cBlockName && TechniqueExecuteNext());
}

//################################################################################
void DX8RENDER::RenderAnimation(long ib, void *src, long numVrts, long minv, long numv, long startidx, long numtrg,
                                bool isUpdateVB)
{
    ...
    if (isUpdateVB || numVrts > numAniVerteces || !aniVBuffer)
    {
        ...
        if (numVrts > numAniVerteces)
        {
            ...
            if (ErrorHandler("RenderAnimation::CreateVertexBuffer",
                             d3d9->CreateVertexBuffer(size, D3DUSAGE_WRITEONLY, type, D3DPOOL_MANAGED, &aniVBuffer,
                                                      NULL)) == true)
                return;
        ...
        BYTE *ptr;
        RDTSC_B(_rdtsc);
        if (ErrorHandler("RenderAnimation::LockVertexBuffer", aniVBuffer->Lock(0, size, (void **)&ptr, 0)) == true)
            return;
        dwNumLV++;
        RDTSC_E(_rdtsc);
        memcpy(ptr, src, size);
        ErrorHandler("RenderAnimation::UnLockVertexBuffer", aniVBuffer->Unlock());
    }
    // Render
    if (ErrorHandler("RenderAnimation::SetVertexShader", d3d9->SetVertexShader(NULL)) == true)
        return;

    if (ErrorHandler("RenderAnimation::SetFVF", d3d9->SetFVF(type)) == true)
        return;

    if (ErrorHandler("RenderAnimation::SetIndices", d3d9->SetIndices(IndexBuffers[ib].buff)) == true)
        return;

    if (ErrorHandler("RenderAnimation::SetStreamSource", d3d9->SetStreamSource(0, aniVBuffer, 0, sizeof(FVF_VERTEX))) ==
        true)
        return;
...
void *DX8RENDER::LockVertexBuffer(long id, dword dwFlags)
{
    ...
    if (ErrorHandler("DX8RENDER::LockVertexBuffer",
                     VertexBuffers[id].buff->Lock(0, VertexBuffers[id].size, (void **)&ptr, dwFlags)))
        return 0;
    dwNumLV++;
    return ptr;
}
...
{
    VertexBuffers[id].dwNumLocks--;
    ErrorHandler("UnLockVertexBuffer::UnLockVertexBuffer", VertexBuffers[id].buff->Unlock());
}
long DX8RENDER::GetVertexBufferSize(long id)
{
...
void *DX8RENDER::LockIndexBuffer(long id, dword dwFlags)
{
    ...
    if (ErrorHandler("LockIndexBuffer::LockIndexBuffer",
                     IndexBuffers[id].buff->Lock(0, IndexBuffers[id].ntrgs, (void **)&ptr, dwFlags)) == true)
        return 0;
    dwNumLI++;
    return ptr;
}
...
{
    IndexBuffers[id].dwNumLocks--;
    ErrorHandler("UnLockIndexBuffer::UnLockIndexBuffer", IndexBuffers[id].buff->Unlock());
}
//################################################################################
void DX8RENDER::ReleaseVertexBuffer(long id)
{
    if (VertexBuffers[id].buff == NULL)
        return;
    ErrorHandler("ReleaseVertexBuffer::ReleaseVertexBuffer", VertexBuffers[id].buff->Release());
...
{
    if (IndexBuffers[id].buff == NULL)
        return;
    ErrorHandler("ReleaseIndexBuffer::ReleaseIndexBuffer", IndexBuffers[id].buff->Release());
...
bool DX8RENDER::ResetDevice()
{
    ...
    if (api->GetEntity(&eid))
        do
        {
            ((ENTITY *)eid.pointer)->LostRender();
        } while (api->GetEntityNext(&eid));

    for (long b = 0; b < MAX_BUFFERS; b++)
    {
        if (VertexBuffers[b].buff && VertexBuffers[b].isManaged == false)
        {
            VertexBuffers[b].buff->Release();
            VertexBuffers[b].buff = 0;
        }
        ...
        {
            VertexBuffers[b].size = 0;
        }
        if (IndexBuffers[b].buff && IndexBuffers[b].isManaged == false)
        {
            IndexBuffers[b].buff->Release();
            IndexBuffers[b].buff = 0;
        }
    ...
    d3dpp.BackBufferFormat = screen_bpp;
    d3dpp.BackBufferCount = 1;
    d3dpp.hDeviceWindow = api->GetAppHWND();
...
void DX8RENDER::RunStart()
{
    ...
    if (bSeaEffect)
    {
        ...
        for (long y = 0; y < 32; y++)
        {
            for (long x = 0; x < 32; x++)
            {
                ...
                qv[x + y * 32].v0 =
                    (sy * float(y) / 31.0f + sinf(fSin + float(y) / 31.0f * PI * 16.0f) * fDY + fDY) / sy2;
            }
        }
    }
    ...
    if (TechniqueExecuteStart("default"))
        do
        {
    ...
    d3d9->SetRenderState(D3DRS_FILLMODE,
                         (api->Controls->GetDebugAsyncKeyState('F') < 0) ? D3DFILL_WIREFRAME : D3DFILL_SOLID);
...
long _cdecl DX8RENDER::Print(long x, long y, char *format, ...)
{
    ...
    if (idFontCurrent < 0 || idFontCurrent >= nFontQuantity)
        return 0;
    if (FontList[idFontCurrent].font == NULL || FontList[idFontCurrent].ref == 0)
        return 0;
...
long _cdecl DX8RENDER::Print(long nFontNum, DWORD color, long x, long y, char *format, ...)
{
    ...
    if (nFontNum < 0 || nFontNum >= nFontQuantity)
        return 0;
    if (FontList[nFontNum].font == NULL || FontList[nFontNum].ref == 0)
        return 0;
...
long DX8RENDER::StringWidth(char *string, long nFontNum, float fScale, long scrWidth)
{
    ...
    if (FontList[nFontNum].ref == 0 || pFont == NULL)
        return 0;
...
long DX8RENDER::CharHeight(long fontID)
{
    if (fontID < 0 || fontID >= nFontQuantity)
        return 0;
    if (FontList[fontID].ref == 0 || FontList[fontID].font == NULL)
        return 0;

    return FontList[fontID].font->GetHeight();
}

long _cdecl DX8RENDER::ExtPrint(long nFontNum, DWORD foreColor, DWORD backColor, int wAlign, bool bShadow, float fScale,
                                long scrWidth, long scrHeight, long x, long y, char *format, ...)
{
...
{
    if (fontIniFileName != NULL && iniName != NULL && stricmp(fontIniFileName, iniName) == 0)
        return true;
    if (fontIniFileName != NULL)
        delete fontIniFileName;
    if (iniName == NULL)
...
dword DX8RENDER::SetRenderStateFloat(dword State, double Value)
{
    return d3d9->SetRenderState((D3DRENDERSTATETYPE)State, Value);
}
...
dword DX8RENDER::SetSamplerState(dword Stage, dword Type, dword Value)
{
    return d3d9->SetSamplerState(Stage, (D3DSAMPLERSTATETYPE)Type, Value);
}

dword DX8RENDER::GetSamplerState(dword Stage, dword Type, dword *pValue)
{
    return d3d9->GetSamplerState(Stage, (D3DSAMPLERSTATETYPE)Type, pValue);
}
...
void DX8RENDER::MakeScreenShot()
{
    ...
    if (FAILED(d3d9->CreateOffscreenPlainSurface(screen_size.x, screen_size.y, D3DFMT_X8R8G8B8, D3DPOOL_SYSTEMMEM,
                                                 &surface, NULL)))
    {
    ...
    if (FAILED(d3d9->GetRenderTargetData(renderTarget, surface)) || FAILED(surface->LockRect(&lr, &r, 0)))
    {
        surface->Release();
        renderTarget->Release();
...
{
    if (!cBlockName || strlen(cBlockName) == 0)
        return false;
...
void DX8RENDER::DrawRects(RS_RECT *pRSR, dword dwRectsNum, const char *cBlockName, dword dwSubTexturesX,
                          dword dwSubTexturesY, float fScaleX, float fScaleY, dword dwNumParams, ...)
{
    ...
    for (dword cnt = 0; cnt < dwRectsNum;)
    {
        ...
        ErrorHandler("SetCamera::GetTransform -> SetFVF", d3d9->SetFVF(RS_RECT_VERTEX_FORMAT));
        ...
        if (bDraw)
            do
            {
                ErrorHandler("SetCamera::GetTransform -> SetStreamSource",
                             SetStreamSource(0, rectsVBuffer, sizeof(RECT_VERTEX)));
                ErrorHandler("SetCamera::GetTransform -> DrawPrimitive",
                             DrawPrimitive(D3DPT_TRIANGLELIST, 0, drawCount * 2));
            } while (cBlockName && TechniqueExecuteNext());
    }
...
#define RS_SPRITE_VERTEX_FORMAT (D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_TEX1)
    ...
    for (i = 0; i < dwSpritesNum; i++)
    {
        ...
        pIndices[i * 6 + 1] = WORD(i * 4 + 3);
        pIndices[i * 6 + 2] = WORD(i * 4 + 2);
        pIndices[i * 6 + 3] = WORD(i * 4 + 0);
        pIndices[i * 6 + 4] = WORD(i * 4 + 2);
        pIndices[i * 6 + 5] = WORD(i * 4 + 1);
    }

    d3d9->SetVertexShader(NULL);
    d3d9->SetFVF(RS_SPRITE_VERTEX_FORMAT);
...
//-----------------------
HRESULT DX8RENDER::CreateVertexBuffer(UINT Length, DWORD Usage, DWORD FVF, D3DPOOL Pool,
...
HRESULT DX8RENDER::CreateRenderTarget(UINT Width, UINT Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample,
                                      DWORD MultisampleQuality, BOOL Lockable, IDirect3DSurface9 **ppSurface)
{
    return d3d9->CreateRenderTarget(Width, Height, Format, MultiSample, MultisampleQuality, Lockable, ppSurface, NULL);
}

HRESULT DX8RENDER::StretchRect(IDirect3DSurface9 *pSourceSurface, const RECT *pSourceRect,
...
HRESULT DX8RENDER::GetCubeMapSurface(IDirect3DCubeTexture9 *ppCubeTexture, D3DCUBEMAP_FACES FaceType, UINT Level,
...
{
    HRESULT hr = d3d9->SetRenderTarget(0, pRenderTarget);
    hr = d3d9->SetDepthStencilSurface(pNewZStencil);
    return hr;
}
...
HRESULT DX8RENDER::CreateTexture(UINT Width, UINT Height, UINT Levels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool,
                                 IDirect3DTexture9 **ppTexture)
{
    return d3d9->CreateTexture(Width, Height, Levels, Usage, Format, Pool, ppTexture, null);
}

HRESULT DX8RENDER::CreateCubeTexture(UINT EdgeLength, UINT Levels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool,
...
HRESULT DX8RENDER::CreateDepthStencilSurface(UINT Width, UINT Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample,
                                             IDirect3DSurface9 **ppSurface)
{
    return d3d9->CreateDepthStencilSurface(Width, Height, Format, MultiSample, 0, 0, ppSurface, NULL);
}

HRESULT DX8RENDER::CreateVertexDeclaration(const D3DVERTEXELEMENT9 *pVertexElements,
...
HRESULT DX8RENDER::DeleteVertexShader(IDirect3DVertexShader9 *pShader)
{
    if (pShader)
...
HRESULT DX8RENDER::DeletePixelShader(IDirect3DPixelShader9 *pShader)
{
    if (pShader)
        return pShader->Release();
    return D3D_OK;
...
HRESULT DX8RENDER::LockRect(IDirect3DCubeTexture9 *ppCubeTexture, D3DCUBEMAP_FACES FaceType, UINT Level,
...
HRESULT DX8RENDER::LockRect(IDirect3DTexture9 *ppTexture, UINT Level, D3DLOCKED_RECT *pLockedRect, CONST RECT *pRect,
                            DWORD Flags)
{
...
HRESULT DX8RENDER::CopyRects(IDirect3DSurface9 *pSourceSurface, CONST RECT *pSourceRectsArray, UINT cRects,
                             IDirect3DSurface9 *pDestinationSurface, CONST RECT *pDestRectsArray)
{
    return d3d9->StretchRect(pSourceSurface, pSourceRectsArray, pDestinationSurface, pDestRectsArray, D3DTEXF_NONE);
}

HRESULT DX8RENDER::GetRenderTargetData(IDirect3DSurface9 *pSourceSurface, IDirect3DSurface9 *pDestinationSurface)
{
    return d3d9->GetRenderTargetData(pSourceSurface, pDestinationSurface);
}

HRESULT DX8RENDER::UpdateSurface(IDirect3DSurface9 *pSourceSurface, CONST RECT *pSourceRectsArray,
                                 IDirect3DSurface9 *pDestinationSurface, CONST POINT *pDestPoint)
{
    return d3d9->UpdateSurface(pSourceSurface, pSourceRectsArray, pDestinationSurface, pDestPoint);
}

HRESULT DX8RENDER::SetVertexShaderConstant(DWORD Register, CONST void *pConstantData, DWORD ConstantCount)
{
    return d3d9->SetVertexShaderConstantF(Register, (float *)pConstantData, ConstantCount);
}

HRESULT DX8RENDER::SetPixelShaderConstant(DWORD Register, CONST void *pConstantData, DWORD ConstantCount)
{
    return d3d9->SetPixelShaderConstantF(Register, (float *)pConstantData, ConstantCount);
}
...
HRESULT DX8RENDER::ImageBlt(long TextureID, RECT *pDstRect, RECT *pSrcRect)
{
    ...
    if (bDraw)
        ...
        {
            d3d9->SetVertexShader(NULL);
            d3d9->SetFVF(D3DFVF_XYZRHW | D3DFVF_TEX1 | D3DFVF_TEXTUREFORMAT2);
...
void DX8RENDER::ProgressView()
{
    ...
    if (abs((long)(progressUpdateTime - time)) < 50)
        return;
...
void DX8RENDER::MakeDrawVector(RS_LINE *pLines, dword dwNumSubLines, const CMatrix &mMatrix, CVECTOR vUp, CVECTOR v1,
                               CVECTOR v2, float fScale, dword dwColor)
{
    dword i;
    dword k;
...
void DX8RENDER::DrawVector(const CVECTOR &v1, const CVECTOR &v2, dword dwColor, const char *pTechniqueName,
                           dword dwNumParams, ...)
{
    RS_LINE lines[51 * 2];
    CMatrix mView;
...
void DX8RENDER::DrawSphere(const CVECTOR &vPos, float fRadius, dword dwColor)
{
    ...
    DrawPrimitiveUP(D3DPT_TRIANGLELIST, D3DFVF_XYZ | D3DFVF_DIFFUSE, DX8sphereNumTrgs, DX8sphereVertex,
                    sizeof(DX8SphVertex), "DX8Sphere");
}
...
IDirect3DBaseTexture9 *DX8RENDER::CreateTextureFromFileInMemory(const char *pFile, dword dwSize)
{
    ...
    D3DXCreateTextureFromFileInMemoryEx((LPDIRECT3DDEVICE9)GetD3DDevice(), pFile, dwSize, D3DX_DEFAULT, D3DX_DEFAULT, 1,
                                        0, d3dFormat, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, null, null,
                                        &pTexture);

    return pTexture;
}

IDirect3DVolumeTexture9 *DX8RENDER::CreateVolumeTexture(dword Width, dword Height, dword Depth, dword Levels,
...
bool DX8RENDER::PushRenderTarget()
{
    ...
    d3d9->GetRenderTarget(0, &stRenderTarget.Top().pRenderTarget);
...
bool DX8RENDER::PopRenderTarget()
{
    ...
    d3d9->SetRenderTarget(0, stRenderTarget.Top().pRenderTarget);
    d3d9->SetDepthStencilSurface(stRenderTarget.Top().pDepthSurface);
...
bool DX8RENDER::SetRenderTarget(IDirect3DCubeTexture9 *pRenderTarget, dword FaceType, dword dwLevel,
...
void DX8RENDER::SetView(const CMatrix &mView)
{
    SetTransform(D3DTS_VIEW, mView);
}
void DX8RENDER::SetWorld(const CMatrix &mWorld)
{
    SetTransform(D3DTS_WORLD, mWorld);
}
void DX8RENDER::SetProjection(const CMatrix &mProjection)
{
