...
void NetServer::AddPacketAllExcept(word wDestination, NMSend &nmSend, bool bGarantedDelivery)
{
    ...
    for (long i = 0; i < aClients; i++)
        if (aClients[i]->GetClientID() != wDestination)
...
void NetServer::Realize(float fDeltaTime)
{
    ...
    pRS->ExtPrint(FONT_DEFAULT, 0xFFFFFFFF, 0x00000000, ALIGN_LEFT, 0, 0.85f, 0, 0, long(0), long(0),
    (char*)str.GetBuffer());*/
}
...
void NetServer::ReceiveMessage(SOCKET s, dword lParam)
{
    ...
    if (iLen > 0)
    {
        ...
        if (wNewCRC16 == wCRC16)
        {
            ...
            api->Event("NetServer_OnNetMessage", "llll", long(dwIP), long(wPort), long(&nmRecv),
