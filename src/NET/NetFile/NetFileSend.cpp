...
void NetFileSend::SendData(long iIndex)
{
    ...
    api->Event("NetFileSend_SendData", "llllll", iClientID, (long)this, (long)pRecvHandle, iIndex,
               (long)&pBuffer[iIndex * 256], dwSendSize);
}
...
void NetFileSend::Execute(float fDeltaTime)
{
    if (!bLoaded)
    {
        fLoadedCheckTime -= fDeltaTime;
        if (fLoadedCheckTime > 0.0f)
            return;
        fLoadedCheckTime = 2.0f;
        if (fio->LoadFile(sFileName, &pBuffer, &dwFileSize))
        {
            api->Event("NetFileSend_TransferReady", "lllll", iClientID, (long)this, (long)pRecvHandle,
                       CRC32::GetCRC32(pBuffer, dwFileSize), dwFileSize);
            bLoaded = true;
            iCurrentIndex = 0;
        }
        else
