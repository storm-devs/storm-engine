...
word NetClient::GetCurrentPacketIndex()
{
    ...
    if (wCurrentDeliveryIndex >= 32767 || wCurrentDeliveryIndex == 0)
        wCurrentDeliveryIndex = 1;
...
void NetClient::Send(packet *pPacket)
{
    ...
    int iValue =
        sendto(sock, crc16.GetDataBuffer(), crc16.GetDataSize(), 0, (struct sockaddr *)&srv_sin, sizeof(srv_sin));
...
void NetClient::Realize(float fDeltaTime)
{
    ...
    pRS->ExtPrint(FONT_DEFAULT, 0xFFFFFFFF, 0x00000000, ALIGN_LEFT, 0, 0.85f, 0, 0, long(0), long(20),
    (char*)str.GetBuffer());
    ...
    pRS->ExtPrint(FONT_DEFAULT, 0xFFFFFFFF, 0x00000000, ALIGN_LEFT, 0, 0.85f, 0, 0, long(0), long(40),
    (char*)str.GetBuffer());
    */
}
...
void NetClient::ReceiveMessage(SOCKET s, dword lParam)
{
    ...
    if (iLen > 0)
    {
        ...
        word wNewCRC16 =
            crc16.GetCRC16(nmRecv.GetDataBuffer() + IS_NM_DEBUG(3, 2), nmRecv.GetDataSize() - IS_NM_DEBUG(3, 2));
        ...
        if (wNewCRC16 == wCRC16)
        {
            ...
            api->Event("NetClient_OnNetMessage", "llll", long(dwIP), long(wPort), long(&nmRecv),
                       (bGarantedDelivery) ? long(wGarantedDeliveryIndex) : long(0));
        }
        else
        {
...
void NetClient::SendMasterServerMessage(const char *pAddress, word wPort, NMSend &nmSend)
{
    ...
    hostent *he = gethostbyname(pAddress);
    if (!he)
        return;
...
dword NetClient::AttributeChanged(ATTRIBUTES *pA)
{
    ...
    if (*pA == "ServerAddr")
    {
        sServerAddr = pA->GetThisAttr();
        UpdateServerAddress();
        return 0;
    }
    if (*pA == "ServerPort")
    {
        wServerPort = (word)pA->GetAttributeAsDword();
        UpdateServerAddress();
        return 0;
    }
