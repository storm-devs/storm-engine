...
void NetBalls::Execute(dword Delta_Time)
{
    ...
    if (IsServer())
    {
        if (!pSea && NetFindClass(true, &EID, "NetSea"))
            pSea = (CANNON_TRACE_BASE *)api->GetEntityPointer(&EID);
        if (!pSail && NetFindClass(true, &EID, "NetSail"))
            pSail = (CANNON_TRACE_BASE *)api->GetEntityPointer(&EID);
        if (!pIsland && NetFindClass(true, &EID, "NetIsland"))
            pIsland = (CANNON_TRACE_BASE *)api->GetEntityPointer(&EID);
        if (!pNetFort && NetFindClass(true, &EID, "NetFort"))
            pNetFort = (CANNON_TRACE_BASE *)api->GetEntityPointer(&EID);
    ...
    for (i = 0; i < aBallTypes.Size(); i++)
    {
        ...
        for (j = 0; j < pBallsType->Balls.Size(); j++)
        {
            ...
            if (IsServer())
                AttributesPointer->SetAttributeUseFloat("CurrentBallDistance",
                                                        sqrtf(~(pBall->vPos - pBall->vFirstPos)));
...
void NetBalls::AddBall(ATTRIBUTES *pABall)
{
    ...
    dword i = 0;
    for (i = 0; i < aBallTypes.Size(); i++)
        if (stricmp(aBallTypes[i].sName, pBallName) == 0)
            break;
...
#define GetAFloat(x) pABall->GetAttributeAsFloat(x)
    ...
    if (IsClient() && aBallTypes[i].sParticleName.Len())
    {
        ...
        if (NetFindClass(false, &eidParticle, "particles"))
        {
            pBall->pParticle = (VPARTICLE_SYSTEM *)api->Send_Message(
                eidParticle, "lsffffffl", PS_CREATE_RIC, (char *)aBallTypes[i].sParticleName.GetBuffer(), pBall->vPos.x,
    ...
    if (IsClient())
    {
        ...
        if (pShipsLights)
            pShipsLights->AddDynamicLights(null, pBall->vPos);
    }
}
