...
void NetSky::CreateFogSphere()
{
    ...
    for (y = 0; y < iNumLevels; y++)
    {
        ...
        for (x = 0; x < iNumAngles; x++)
        {
            ...
            pVerts[idx].pos = vPos;
            VDATA *pvData = api->Event("NetClient_WhrOnCalcFogColor", "fff", vPos.x, vPos.y, vPos.z);
            Assert(pvData);
...
{
    char str[256],
        *names[SKY_NUM_TEXTURES] = {"sky_back.tga", "sky_left.tga", "sky_front.tga", "sky_right.tga", "sky_up.tga"};
...
dword NetSky::AttributeChanged(ATTRIBUTES *pAttribute)
{
    if (*pAttribute == "dir")
    {
        sSkyDir = pAttribute->GetThisAttr();
        return 0;
    }
    if (*pAttribute == "color")
    {
