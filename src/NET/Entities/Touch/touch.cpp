...
void NetTouch::Execute(dword dwCoreDeltaTime)
{
    ...
    // while (iTempDeltaTime>0)
    {
        ...
        for (i = 0; i < aShips.Len(); i++)
            Touch(i, INVALID_SHIP_IDX, 0, 0, 0.0f, 0.0f);
    }
...
BOOL NetTouch::IsIntersectShipsRects(long idx1, long idx2)
{
    ...
    float dist = SQR(pS1->State.vPos.x - pS2->State.vPos.x) + SQR(pS1->State.vPos.z - pS2->State.vPos.z);
...
CVECTOR NetTouch::GetPoint(float x, float y, float xx, float yy, float xscale, float yscale, float fCos, float fSin,
                           POINT ss)
{
    CVECTOR vPos;
    vPos.x = xscale * xx;
    ...
    for (i = 0; i < aShips.Len(); i++)
        if (aShips[i])
        {
            ...
            for (j = 0; j < aShips[i]->iNumVContour; j++)
            {
                ...
                p2 = GetPoint(x, y, aShips[i]->vContour[nextj].x, aShips[i]->vContour[nextj].z, xscale, yscale, fCos,
                              fSin, ss);
                ...
                // dwColor = 0xFFFFFF;
                if (pIslandBase &&
                    pIslandBase->GetDepth(aShips[i]->vContourTemp[0][j].x, aShips[i]->vContourTemp[0][j].z, &fRes))
                {
                    if (fRes == 0.0f)
                        dwColor = 0xFFFF00;
                }
...
BOOL NetTouch::IsIntersectShipsReal(long idx, long cidx, CVECTOR *vPos, CVECTOR *vAng, CVECTOR *vRecoil, float *fPower,
                                    float *fSlide)
{
    long i, j;
    ...
    for (i = 0; i < pS1->iNumVContour; i++)
        if (IsPointInContour(&pS1->vContourTemp[1][i], &pS2->vContourTemp[1][0],
                             pS2->iNumVContour)) // FIX ME and pS2->vContourTemp[1][0]!!!
        {
...
float NetTouch::Touch(long idx, long skip_idx, CVECTOR *vPos, CVECTOR *vAng, float fPower, float fSlide)
{
    ...
    if (vPos && vAng && fPower != 0.0f)
    {
        ...
        api->Event("NetServer_Ship2ShipCollision", "aafflfff", pOur->GetACharacter(), pEnemy->GetACharacter(),
                   fPowerApplied, fSlide, GetTouchPoint(idx, vPosTemp), vPos->x, vPos->y, vPos->z);
    ...
    for (i = ISLAND_CODE; i < aShips.Len(); i++)
        if (i != idx && i != skip_idx && IsIntersectShipsRects(idx, i))
        {
            ...
            if (IsIntersectShipsReal(idx, i, &vPos1, &vAng1, &vRecoil, &fPower1, &fSlide1))
            {
                ...
                // script event initiate
                if (i == ISLAND_CODE)
                    api->Event("NetServer_Ship2IslandCollision", "afflfff", pOur->GetACharacter(), fPower1, fSlide1,
                               GetTouchPoint(idx, vPosTemp), vPos1.x, vPos1.y, vPos1.z);
                else
                    api->Event("NetServer_Ship2ShipCollision", "aafflfff", pOur->GetACharacter(),
                               aShips[i]->pShip->GetACharacter(), fPower1, fSlide1, GetTouchPoint(idx, vPosTemp),
...
BOOL NetTouch::FakeTouch()
{
    ...
    for (i = 0; i < aShips.Len(); i++)
    {
        if (IsSinked(i))
            continue;
        for (j = ISLAND_CODE; j < aShips.Len(); j++)
