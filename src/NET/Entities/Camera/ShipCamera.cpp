...
void NetShipCamera::Move(float fDeltaTime)
{
    ...
    // Current distance
    CVECTOR boxSize =
        pShip->GetBoxSize() * CVECTOR(SCMR_BOXSCALE_X * 0.5f, SCMR_BOXSCALE_Y * 0.5f, SCMR_BOXSCALE_Z * 0.5f);
    boxSize.x += boxSize.y;
    boxSize.z += boxSize.y;
...
void NetShipCamera::ShipsCollision(CVECTOR &pos)
{
    ...
    for (; res; res = NetFindClassNext(IsServer(), &id))
    {
        ...
        CVECTOR s =
            ship->GetBoxSize() * CVECTOR(SCMR_BOXSCALE_X * 0.5f, SCMR_BOXSCALE_Y * 0.5f, SCMR_BOXSCALE_Z * 0.5f);
...
bool NetShipCamera::IslandCollision(CVECTOR &pos)
{
    ...
    {
        ENTITY_ID island_id;
        if (NetFindClass(IsServer(), &island_id, "NetIsland"))
