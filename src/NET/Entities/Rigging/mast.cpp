...
void _cdecl NetMast::Mount(ENTITY_ID modelEI, ENTITY_ID shipEI, NODE *mastNodePointer)
{
    ...
    if (mastNodePointer != null)
    {
        ...
        if (NetFindClass(IsServer(), &tmpEI, "NetShip"))
            do
            {
                if (tmpEI == ship_id)
                    continue;
    ...
    if (mdl != 0)
    {
        ...
        else // мачта не тонет... пока падает
        {
            ...
            while (bNextClass)
            {
                ...
                // коллизим с кораблем
                if (NetFindClass(IsServer(), &findEI, "NetShip"))
                    do
                    {
...
int NetMast::GetSlide(ENTITY_ID &mod, CVECTOR &pbeg, CVECTOR &pend, CVECTOR &dp, CVECTOR &lrey, CVECTOR &rrey,
                      float &angl)
{
    int retVal = 0;
