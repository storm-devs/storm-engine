    ...
    if (nVert && nIndx)
    {
        if (ropeQuantity > wRopeLast)
        {
            ...
            if (vBuf >= 0 && iBuf >= 0)
            {
                ...
                api->Trace("Can`t create index or vertex buffer (index = %d, vertex = %d)", nIndx, nVert);
        }
...
void NetRope::SetAdd(int firstNum)
{
    ...
    for (int rn = firstNum; rn < ropeQuantity; rn++)
    {
        ...
        {
            api->Trace("Bad rope data for rope: (rope num = %d) (begin group=%d, end group=%d)", rlist[rn]->ropeNum,
                       rlist[rn]->bgnum, rlist[rn]->egnum);
