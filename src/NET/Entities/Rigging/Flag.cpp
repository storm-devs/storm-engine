    ...
    if (bUse)
    {
        ...
        if (nVert != 0 && nIndx != 0)
        {
            ...
            for (long fn = 0; fn < flagQuantity; fn++)
            {
                ...
                // RenderService->DrawBuffer(vBuf, sizeof(FLAGLXVERTEX), iBuf, flist[fn]->sv, flist[fn]->nv,
                // flist[fn]->st, flist[fn]->nt, "ShipFlag");
                RenderService->DrawBuffer(vBuf, sizeof(FLAGLXVERTEX), iBuf, 0, nVert, flist[fn]->st, flist[fn]->nt,
                                          "ShipFlag");
            }
        }
...
dword _cdecl NetFlag::ProcessMessage(MESSAGE &message)
{
    ...
    case MSG_FLAG_INIT: {
        ...
        gdata[groupQuantity - 1].char_attributes =
            ((VAI_OBJBASE *)gdata[groupQuantity - 1].ship_id.pointer)->GetACharacter();
...
void NetFlag::DoMove(FLAGDATA *pr, float delta_time)
{
    ...
    if ((pr->Alfa += (ALFA_DEPEND + ALFA_RAND * (float)rand() / (float)RAND_MAX) * delta_time) > PIm2)
        pr->Alfa = 0.f;
    if ((pr->Beta += (BETA_DEPEND + BETA_RAND * (float)rand() / (float)RAND_MAX) * delta_time) > PIm2)
        pr->Beta = 0.f;
    float Alfa = -pr->Alfa;
    float Beta = -pr->Beta;
...
void NetFlag::AddLabel(GEOS::LABEL &gl, NODE *nod, bool isSpecialFlag)
{
    ...
    for (fn = 0; fn < flagQuantity; fn++)
        if (flist[fn] != NULL && flist[fn]->HostGroup == groupQuantity - 1 && flist[fn]->grNum == grNum &&
            flist[fn]->nod == nod)
        {
            fd = flist[fn];
            break;
...
void NetFlag::SetAll()
{
    ...
    for (int fn = 0; fn < flagQuantity; fn++)
    {
        ...
        else
        {
            ...
            flist[fn]->texNum = 0;
            VDATA *pvdat = api->Event("NetClient_GetRiggingData", "sa", "GetFlagTexPointer",
                                      gdata[flist[fn]->HostGroup].char_attributes);
            if (pvdat)
            {
                flist[fn]->pTexture = (IDirect3DBaseTexture9 *)pvdat->GetLong();
                if (flist[fn]->pTexture)
                    flist[fn]->pTexture->AddRef();
            }
            else
                flist[fn]->pTexture = 0;
...
void NetFlag::MoveOtherHost(ENTITY_ID newm_id, long flagNum, ENTITY_ID oldm_id)
{
    ...
    int fn = 0;
    for (fn = 0; fn < flagQuantity; fn++)
        if (flist[fn] != NULL && flist[fn]->grNum == flagNum && flist[fn]->HostGroup == oldgn)
            break;
