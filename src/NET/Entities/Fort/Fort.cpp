...
bool NetFort::AddFort(ATTRIBUTES *pIslandAP, ATTRIBUTES *pFortLabelAP, ENTITY_ID eidModel, ENTITY_ID eidBlot)
{
    ...
    if (IsServer())
    {
        ATTRIBUTES *pACannonsType1 = GetACharacter()->FindAClass(GetACharacter(), "Fort.Cannons.Type.Cannon");
        Assert(pACannonsType1);
        ATTRIBUTES *pACannonsType2 = GetACharacter()->FindAClass(GetACharacter(), "Fort.Cannons.Type.Culverine");
        Assert(pACannonsType2);
        ATTRIBUTES *pACannonsType3 = GetACharacter()->FindAClass(GetACharacter(), "Fort.Cannons.Type.Mortar");
...
void NetFort::Execute(dword dwDeltaTime)
{
    ...
    {
        if (!pVWShips)
            pVWShips = (VIDWALKER *)api->LayerGetWalker("ship_cannon_trace");
    }
    ...
    for (long i = 0; i < aCannons; i++)
    {
        ...
        if (!bInitCannons)
            aCannons[i].QuickRecharge(true);
        aCannons[i].Execute(fDeltaTime);
        if (bTryFire)
            TryFire(&aCannons[i], fSpeedV0);
    }
    bInitCannons = true;
    ...
    for (long i = 0; i < aCulverins; i++)
    {
        ...
        if (!bInitCulverins)
            aCulverins[i].QuickRecharge(true);
        aCulverins[i].Execute(fDeltaTime);
        if (bTryFire)
            TryFire(&aCulverins[i], fSpeedV0);
    }
    bInitCulverins = true;
    ...
    for (long i = 0; i < aMortars; i++)
    {
        ...
        if (!bInitMortars)
            aMortars[i].QuickRecharge(true);
        aMortars[i].Execute(fDeltaTime);
        if (bTryFire)
            TryFire(&aMortars[i], fSpeedV0);
    }
    bInitMortars = true;
}
...
{
    VDATA *pV = (VDATA *)api->Event("NetServer_FortSelectCannonType", "l", dwCType);
    Assert(pV);
...
void NetFort::TryFire(NetCannon *pC, float fSpeedV0)
{
    ...
    if (pC->isDamaged() || pC->isFired() || !pC->isReady2Fire())
        return;
    ...
    ENTITY_ID *pEID;
    if (pVWShips && (pEID = pVWShips->GetID()) != 0)
        do
        {
            ...
            if (pShip && pShip->IsServer()) // FIX-ME: add enemy test
            {
                ...
                if (pShip->isDead() || dwTeam == dwFriendlyTeam)
                    continue;
    ...
    if (pFireShip)
        pC->Fire(fSpeedV0, pFireShip->GetPos());
}
...
void NetFort::CheckCannonForBoom(NetCannon *pC, long iBallOwner, dword dwCType, const CVECTOR &vPnt)
{
    ...
    VDATA *pV = api->Event("NetServer_OnFortCannonDamage", "llllfffff", long(dwCType), iTotalCannons, iDamagedCannons,
                           iBallOwner, pC->GetDamage(), fDistanceRatio, vCPos.x, vCPos.y, vCPos.z);
    Assert(pV);
...
float NetFort::Cannon_Trace(long iBallOwner, const CVECTOR &vSrc, const CVECTOR &vDst)
{
    ...
    if (iBallOwner == GetNetID() || !GetModel())
        return 2.0f;

    float fRes = GetModel()->Trace(vSrc, vDst);
    if (fRes > 1.0f)
        return 2.0f;
    ...
    for (long i = 0; i < aCannons; i++)
        CheckCannonForBoom(&aCannons[i], iBallOwner, dwCannonType, vPnt);
    for (long i = 0; i < aCulverins; i++)
        CheckCannonForBoom(&aCulverins[i], iBallOwner, dwCulverinType, vPnt);
    for (long i = 0; i < aMortars; i++)
        CheckCannonForBoom(&aMortars[i], iBallOwner, dwMortarType, vPnt);
