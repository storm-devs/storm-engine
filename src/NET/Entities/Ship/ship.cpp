...
void NetShip::LoadServices()
{
    ...
    if (IsServer())
    {
        if (NetFindClass(IsServer(), &touch_id, "NetTouch"))
            pNetTouch = (NetTouch *)api->GetEntityPointer(&touch_id);
        if (pNetTouch)
            pNetTouch->AddShip(this);
    }

    if (NetFindClass(IsServer(), &ent, "NetIsland"))
        pIsland = (ISLAND_BASE *)api->GetEntityPointer(&ent);
...
CVECTOR NetShip::ShipRocking(float fDeltaTime)
{
    ...
    float fNewPos =
        fFullY /
        36.0f; //(ShipPoints[2][2].fY + ShipPoints[3][2].fY + ShipPoints[2][3].fY + ShipPoints[3][3].fY) / 4.0f;
    State.vPos.y = State.vPos.y + Min(fDelta * fRockingY, 1.0f) *
                                      (fNewPos - State.vPos.y); //(ShipPoints[2][2].fY + ShipPoints[3][2].fY +
                                                                //ShipPoints[2][3].fY + ShipPoints[3][3].fY) / 4.0f;
...
bool NetShip::ApplyStrength(float dtime, bool bCollision)
{
    ...
    CVECTOR new_speed = CVECTOR(0.0f, 0.0f, 0.0f), new_rotate = CVECTOR(0.0f, 0.0f, 0.0f);
...
void NetShip::SetDead()
{
    if (!isDead())
    {
        ...
        ATTRIBUTES *pASinkSpeed = AP()->FindAClass(AP(), "ship.sink.speed");
        Assert(pASinkSpeed);
...
void NetShip::Execute(DWORD __DeltaTime)
{
    ...
    if (!pASpeed)
    {
        pASpeed = AP()->CreateSubAClass(AP(), "ship.speed");
        Assert(pASpeed);
    }
    ...
    {
        VDATA *pVDeltaTime = (VDATA *)api->GetScriptVariable("iClientDeltaTime");
        Assert(pVDeltaTime);
    ...
    if (IsServer())
    {
        ATTRIBUTES *pAS = AP()->FindAClass(AP(), "Ship");
        Assert(pAS);
    ...
    if (isDead())
    {
        ...
        if (isVisible() && vPos.y < -50.0f)
        {
            ...
            // stop all fireplaces
            for (dword i = 0; i < aFirePlaces.Size(); i++)
                aFirePlaces[i].Stop();
    ...
    if (!pAPos)
    {
        pAPos = AP()->CreateSubAClass(AP(), "ship.pos");
        Assert(pAPos);
    }
    if (!pAAng)
    {
        pAAng = AP()->CreateSubAClass(AP(), "ship.ang");
        Assert(pAAng);
    }
    if (!pABox)
    {
        pABox = AP()->CreateSubAClass(AP(), "ship.boxsize");
        Assert(pABox);
    }
            ...
            for (long i=0; i<iNumMasts; i++) if (!pMasts[i].bBroken)
            {
                ...
                if (pVWShip)	// from ship damage
                {
                    ...
                    if (fShipRes <= 1.0f)
                    {
                        ...
                        pV = api->Event(SHIP_MAST_DAMAGE, "llffffaa", SHIP_MAST_TOUCH_SHIP, pM->iMastNum, v1.x, v1.y,
    v1.z, pM->fDamage, GetACharacter(), pACollideCharacter); pM->fDamage = Clamp(pV->GetFloat());
                    }
                }
                if (pVWIsland)	// from island damage
                {
                    ...
                    if (fIslRes <= 1.0f)
                    {
                        pV = api->Event(SHIP_MAST_DAMAGE, "llffffa", SHIP_MAST_TOUCH_ISLAND, pM->iMastNum, v1.x, v1.y,
    v1.z, pM->fDamage, GetACharacter()); pM->fDamage = Clamp(pV->GetFloat());
                    }
                }
    ...
    if (pCannonController)
        pCannonController->Execute(fDeltaTime);
    // execute cannons controller
    if (pCannonController)
        pCannonController->Realize(fDeltaTime);
...
void NetShip::UpdateCurves(long iMsgServerTime, const CVECTOR &vPos, const CVECTOR &vSpeed)
{
    ...
    curvePosition.vTime[4] = vPos;
    curveSpeed.vTime[4] = vSpeed;
    curvePosition.iTime = iTime - 4;
    curveSpeed.iTime = iTime - 4;

    for (long i = 0; i < 5; i++)
    {
        curvePosition.curve.SetLine(curvePosition.vTime[1], curvePosition.vTime[1], curvePosition.vTime[2],
                                    curvePosition.vTime[3], curvePosition.vTime[3]);
        curveSpeed.curve.SetLine(curveSpeed.vTime[0], curveSpeed.vTime[1], curveSpeed.vTime[2], curveSpeed.vTime[3],
                                 curveSpeed.vTime[4]);
    }
...
void NetShip::MastFall(mast_t *pM)
{
    ...
    {
        ENTITY_ID ent;
        api->CreateEntity(&ent, "NetMast");
        ENTITY *pE = (ENTITY *)ent.pointer;
        if (pE)
            pE->SetServer(IsServer());
        api->Send_Message(ent, "lpii", MSG_MAST_SETGEOMETRY, pM->pNode, GetID(), GetModelEID());
        api->LayerAdd("net_execute", ent, 100);
        if (IsClient())
            api->LayerAdd("net_realize", ent, 100);
...
void NetShip::Realize(DWORD dtime)
{
    ...
    /*if (dtime)
    {
        ...
        if (IsClient() && GetNetID() == pVClientID->GetLong())
        {
            ...
            Render().ExtPrint(FONT_DEFAULT, 0xFFFFFFFF, 0x00000000, ALIGN_LEFT, 0, 0.85f, 0, 0, long(0), long(100), "HP:
    %.1f, SP: %.1f, Crew: %.1f", fHP, fSP, fCrew); Render().ExtPrint(FONT_DEFAULT, 0xFFFFFFFF, 0x00000000, ALIGN_LEFT,
    0, 0.85f, 0, 0, long(0), long(120), "sZ: %.2f, rY: %.2f", State.vSpeed.z, State.vRotate.y);
            ...
            Render().ExtPrint(FONT_DEFAULT, 0xFFFFFFFF, 0x00000000, ALIGN_LEFT, 0, 0.85f, 0, 0, long(0), long(60),
    "STime: %d, CTime: %d", pVSTime->GetLong(), pVCTime->GetLong());
        }
    }*/
...
bool NetShip::IsCanPlace(CVECTOR &vNewPos)
{
    ...
    for (dword i = 0; i < aShips.Size(); i++)
        if (aShips[i] && this != aShips[i])
        {
            float fMinDist = (vBoxSize.z + aShips[i]->GetBoxSize().z) / 2.0f;
            CVECTOR vOtherPos = aShips[i]->GetPos();
            if ((vOtherPos - vNewPos).GetLength2D() < fMinDist)
                return false;
        }

    if (pIsland && pIsland->Check2DBoxDepth(vNewPos, vBoxSize, 0.0f, -14.0f))
        return false;
...
void NetShip::LoadPositionFromAttributes()
{
    ...
    if (IsServer())
    {
        ENTITY_ID eid;
        aShips.DelAll();
        if (NetFindClass(true, &eid, "NetShip"))
            do
            {
    ...
    for (long i = 0; i < 5; i++)
    {
        curvePosition.vTime[i] = State.vPos;
    }
...
float NetShip::Cannon_Trace(long iBallOwner, const CVECTOR &vSrc, const CVECTOR &vDst)
{
    ...
    for (long i = 0; i < iNumMasts; i++)
        if (!pMasts[i].bBroken)
        {
            ...
            if (fRes <= 1.0f)
            {
                ...
                VDATA *pV = api->Event("NetServer_OnShipMastDamage", "llffffll", NETSHIP_MAST_TOUCH_BALL, pM->iMastNum,
                                       v1.x, v1.y, v1.z, pM->fDamage, long(GetNetID()), iBallOwner);
    ...
    /*
        for (long i=0; i<iNumHulls; i++) if (!pHulls[i].bBroken)
        {
            ...
            {
                CVECTOR v1 = vSrc + fRes * (vDst - vSrc);
                VDATA * pV = api->Event(SHIP_HULL_DAMAGE, "llffffaas", SHIP_HULL_TOUCH_BALL, pM->iHullNum, v1.x, v1.y,
       v1.z, pM->fDamage, GetACharacter(), iBallOwner, pM->pNode->GetName()); pM->fDamage = Clamp(pV->GetFloat());
                HullFall(pM);
            }
        }
            // ugeen 2017
    */
    ...
    if (fRes <= 1.0f)
    {
        ...
        api->Event("NetServer_OnShipHullHit", "illffflf", GetID(), iBallOwner, long(GetNetID()), vTemp.x, vTemp.y,
                   vTemp.z, iBestIndex, fMinDistance);
...
dword NetShip::AttributeChanged(ATTRIBUTES *pAttribute)
{
        ...
        if (bCanFire)
        {
            ...
            if (NetFindClass(true, &eid, "NetShip"))
                do
                {
                    ...
                    if (!pShip || pShip->GetNetID() == GetNetID())
                        continue;
                    CVECTOR vFakeShipPos =
                        pShip->GetPos() + (!(pShip->GetPos() - vOurPos)) * pShip->GetBoxSize().z * 0.8f;
            ...
            // check fort here
            NetFort *pFort = null;
            if (NetFindClass(true, &eid, "NetFort"))
                pFort = (NetFort *)api->GetEntityPointer(&eid);
            if (pFort)
                for (dword i = 0; i < pFort->GetNumCannons(); i++)
                {
    ...
    if (*pAttribute == "DoCannonFireCameraOutside" && pCannonController)
    {
        ...
        if (NetFindClass(true, &shipEID, "NetShip"))
            do
            {
                ...
                if (dwFriendlyTeam != -1 && dwFriendlyTeam == dwTeam)
                    continue;
        ...
        NetFort *pFort = null;
        if (NetFindClass(true, &fortEID, "NetFort"))
            pFort = (NetFort *)api->GetEntityPointer(&fortEID);
        if (pFort && pFort->GetACharacter()->GetAttributeAsDword("FriendlyTeam") != dwOurTeam)
            for (dword i = 0; i < pFort->GetNumCannons(); i++)
            {
                NetCannon *pC = pFort->GetCannon(i);
                if (pC->isDamaged())
        ...
        if (aShipFire.Size())
        {
            ...
            for (dword i = 0; i < aShipFire.Size(); i++)
            {
                ...
                if (pFire->pShip)
                    pCannonController->Fire(pFire->pShip->GetPos());
                if (pFire->pFortCannon)
                    pCannonController->Fire(pFire->pFortCannon->GetPos());
            }
        }
    }
    ...
    {
        if (pCannonController)
            pCannonController->Reload();
    }

    if (*pAttribute == "Dead")
    {
        if (pAttribute->GetAttributeAsDword())
            SetDead();
    }
...
CVECTOR NetShip::GetBoxSize()
{
    return State.vBoxSize;
};
ENTITY_ID NetShip::GetModelEID()
{
    return model_id;
}
MODEL *NetShip::GetModel()
{
    Assert(api->ValidateEntity(&GetModelEID()));
    return (MODEL *)api->GetEntityPointer(&GetModelEID());
}
CMatrix *NetShip::GetMatrix()
{
    return &GetModel()->mtx;
}
void NetShip::SetMatrix(CMatrix &mtx)
{
    GetModel()->mtx = mtx;
}
CVECTOR NetShip::GetAng()
{
    return State.vAng;
}
CVECTOR NetShip::GetPos()
{
    return State.vPos;
}
void NetShip::SetPos(CVECTOR &vNewPos)
{
    State.vPos = vNewPos;
}

void NetShip::SetSpeed(float fSpeed)
{
    fSailState = fSpeed;
}
float NetShip::GetSpeed()
{
    return fSailState;
}

void NetShip::SetRotate(float fRotSpd)
{
    Strength[0].vRotate.y = fRotSpd;
}
float NetShip::GetRotate()
{
    return Strength[0].vRotate.y;
}

float NetShip::GetCurrentSpeed()
{
