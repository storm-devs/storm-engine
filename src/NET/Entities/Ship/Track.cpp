...
VDX8RENDER *NetShipTracks::ShipTrack::pRS = null;
...
void NetShipTracks::ShipTrack::Realize(float fDeltaTime)
{
    ...
    if (aTrack1.Len() > 1)
    {
        ...
        // pShip->Render().DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, aTrack1.Len() * 20, 0, 19 * (aTrack1.Len() - 1) *
        // 2, "SeaTrack");
    ...
    if (aTrack2.Len() > 1)
    {
        ...
        // pShip->Render().DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, aTrack2.Len() * 4, 0, 3 * (aTrack2.Len() - 1) *
        // 2, "SeaTrack");
