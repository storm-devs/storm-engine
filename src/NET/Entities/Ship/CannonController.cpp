...
bool NetShipCannonController::Fire2Position(dword dwBort, CVECTOR &vFirePos, float fFireHeight)
{
    ...
    /*VDATA * pVData = api->Event(SHIP_GET_BORT_FIRE_DELTA, "afff", GetShip()->GetACharacter(), vTempFirePos.x,
    vTempFirePos.y, vTempFirePos.z); float fRadius = pVData->GetFloat(); float fAng = FRAND(PIm2); vTempFirePos +=
    CVECTOR(fRadius * sinf(fAng), 0.0f, fRadius * cosf(fAng));*/
...
bool NetShipCannonController::Fire(CVECTOR &vFireCamPos, CVECTOR &vFireDir)
{
    ...
    if (dwBort != INVALID_BORT_INDEX)
        ...
        {
            float fMaxDist =
                NetCannon::CalcMaxFireDistance(aShipBorts[dwBort].fOurBortFireHeight + vOurPos.y, fSpeedV0, fAngle);
...
/*bool NetShipCannonController::Fire(AIShip * pEnemy)
{
    ...
    VDATA * pVData = api->Event(CANNON_GET_FIRE_HEIGHT, "aa", GetShip()->GetACharacter(), pEnemy->GetACharacter());
Assert(pVData); fFireHeight = pVData->GetFloat();
...
void NetShipCannonController::Execute(float fDeltaTime)
{
    ...
    for (i = 0; i < aShipBorts.Size(); i++)
        if (aShipBorts[i].aCannons.Size())
        {
            ...
            ATTRIBUTES *pACurBort = pABorts->FindAClass(pABorts, (char *)pBort->sName.GetBuffer());
            Assert(pACurBort);
            ...
            pACurBort->SetAttributeUseFloat("DamageRatio",
                                            1.0f - (float)GetBortIntactCannonsNum(i) / (float)pBort->aCannons.Size());
        }
        else
        {
            AISHIP_BORT *pBort = &aShipBorts[i];
...
float NetShipCannonController::GetFireDistance(bool bMaxFireDistance)
{
    ...
    for (dword i = 0; i < aShipBorts.Size(); i++)
        ...
        {
            AISHIP_BORT *pBort = &aShipBorts[i];
            float fMaxFireDistance = NetCannon::CalcMaxFireDistance(pBort->fOurBortFireHeight + vOurPos.y, fSpeedV0,
...
bool NetShipCannonController::UpdateParameters()
{
    ...
    for (dword i = 0; i < aShipBorts.Size(); i++)
        if (aShipBorts[i].aCannons.Size())
        {
            ...
            pBort->fMaxFireDistance = NetCannon::CalcMaxFireDistance(pBort->fOurBortFireHeight + vOurPos.y, fSpeedV0,
                                                                     GetBortHeightAngle(i) + pBort->fFireAngMax);
...
bool NetShipCannonController::isCanFire(CVECTOR &vCamDir)
{
    ...
    for (dword i = 0; i < aShipBorts.Size(); i++)
        if (aShipBorts[i].aCannons.Size())
        {
            ...
            if (fY0 < pBort->fCosFireZone && fabsf(fY0 - pBort->fCosFireZone) > 1e-20f)
                continue;
...
dword NetShipCannonController::GetNextFireBort(dword dwPrevBort, CVECTOR &vFirePos, float *pfZapasDistance)
{
    ...
    for (dword i = dwPrevBort + 1; i < aShipBorts.Size(); i++)
        if (aShipBorts[i].aCannons.Size() && isCanFireBort(i, vFirePos, pfZapasDistance) &&
            !aShipBorts[i].isBortDamaged())
            return i;
...
void NetShipCannonController::Realize(float fDeltaTime)
{
    ...
    /*for (dword i=0; i<aShipBorts.Size(); i++)
    {
        ...
        Render().Print(0, 140 + i * 20, "%s, %d, %.2f, ", (char*)pB->sName.GetBuffer(), pB->aCannons.Size(),
    pB->fChargePercent);
    }*/
}
...
bool NetShipCannonController::Init(ATTRIBUTES *_pABaseShip)
{
    ...
    ATTRIBUTES *pAP = pABaseShip->FindAClass(pABaseShip, "Cannons.Borts");
    if (!pAP)
        return false;
    dword dwIdx = 0;
    while (true)
...
dword NetShipCannonController::GetBortIntactCannonsNum(dword dwBortIdx)
{
    ...
    dword dwCannons = 0;
    for (dword j = 0; j < aShipBorts[dwBortIdx].aCannons.Size(); j++)
...
bool NetShipCannonController::isHaveEnoughtBallsForBort(dword dwBortIdx)
{
    ...
    if (dwIntactCannonsNum == 0)
        return true;

    ATTRIBUTES *pACharge = GetShip()->GetACharacter()->FindAClass(GetShip()->GetACharacter(), "Ship.Cannons.Charge");
    Assert(pACharge);
    ATTRIBUTES *pAGoods = GetShip()->GetACharacter()->FindAClass(GetShip()->GetACharacter(), "Ship.Goods");
    Assert(pAGoods);
    ATTRIBUTES *pAGood = pAGoods->GetAttributeClass(pACharge->GetAttributeAsDword("Type"));
    Assert(pAGood);
...
/*void NetShipCannonController::CheckCannonsBoom(float fTmpCannonDamage, const CVECTOR & vPnt)
{
    ...
    for (i=0; i<aShipBorts.Size(); i++)
    {
        ...
        for (j=0; j<pBort->aCannons.Size(); j++)
        {
            ...
            VDATA * pVData = api->Event(CANNON_DAMAGE, "affffff", GetShip()->GetACharacter(), fTmpCannonDamage,
pC->GetDamage(), fDistance, vPnt.x, vPnt.y, vPnt.z);
            ...
            if (pC->isDamaged())
            {
                ...
                pACurBort->SetAttributeUseFloat("DamageRatio", 1.0f - (float)GetBortIntactCannonsNum(i) /
(float)pBort->aCannons.Size());
            }
        }
    }
}*/
