...
void NetLightPillar::Realize(dword DeltaTime)
{
    ...
    Render().DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, D3DFVF_XYZ | D3DFVF_DIFFUSE, (NUM_SEGMENTS - 1) * 2, &Vertex[0],
                             sizeof(Vertex[0]), "LightPillar");
}
...
dword NetLightPillar::AttributeChanged(ATTRIBUTES *pAttribute)
{
    ...
    if (*pAttribute == "Radius")
        fRadius = pAttribute->GetAttributeAsFloat();
    if (*pAttribute == "Height")
        fHeight = pAttribute->GetAttributeAsFloat();
    if (*pAttribute == "Color1")
        dwColor1 = pAttribute->GetAttributeAsDword();
    if (*pAttribute == "Color2")
        dwColor2 = pAttribute->GetAttributeAsDword();

    if (*pParent == "Pos")
    {
        if (*pAttribute == "x")
        {
            vPos.x = pAttribute->GetAttributeAsFloat();
            return 0;
        }
        if (*pAttribute == "z")
        {
            vPos.z = pAttribute->GetAttributeAsFloat();
            return 0;
        }
    }
