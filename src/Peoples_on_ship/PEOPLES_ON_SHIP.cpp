...
PEOPLE_ON_SHIP::PEOPLE_ON_SHIP()
    : shipsCount(0), manSpeed(0.0f), cameraOnDeck(false),
      hidePeopleOnDeck(true){GUARD(PEOPLE_ON_SHIP::PEOPLE_ON_SHIP) UNGUARD}
...
void PEOPLE_ON_SHIP::Realize(DWORD _dTime)
{
    ...
    for (int shipN = 0; shipN < shipsCount; shipN++)
    {
        ...
        for (int manN = 0; manN < shipWalk[shipN].showCount; manN++)
        {
            ...
            // clip by planes
            m.BuildPosition(shipWalk[shipN].crew[manN].pos.x, shipWalk[shipN].crew[manN].pos.y,
                            shipWalk[shipN].crew[manN].pos.z);
...
void PEOPLE_ON_SHIP::AddShipWalk(ENTITY_ID &_shipID, int vCount, VDATA *vArray, VDATA *gArray, VDATA *tArray)
{
    ...
    for (int i = 0; i < nElements; i++)
    {
        ...
        if ((gCouple > 0) && (v1 < vCount) && (v2 < vCount))
            shipWalk[shipsCount].graph.AddPair(v1, v2, shipWalk[shipsCount].vertTypes[gCouple >> 8],
...
void PEOPLE_ON_SHIP::InitShipMan(int _shipN, int _manN)
{
    ...
    {
    case 0:
        _CORE_API->Send_Message(shipWalk[_shipN].crew[_manN].modelID, "ls", MSG_MODEL_LOAD_GEO,
...
{
    float dNow = (float)SQR(_man->pos.x - _shipWalk->verts[_man->destI].x) +
                 SQR(_man->pos.y - _shipWalk->verts[_man->destI].y) +
                 SQR(_man->pos.z - _shipWalk->verts[_man->destI].z);
    float dirVect = _man->speed * _dTime;
    float dFuture = (float)SQR(_man->pos.x + dirVect * _man->dir.x - _shipWalk->verts[_man->destI].x) +
...
void PEOPLE_ON_SHIP::ChooseNewAction(tShipWalk *_shipWalk, tShipMan *_man)
{
    ...
    else
    {
        ...
        if (_man->state != MAN_RELOAD)
            _man->state = (tManState)getRandomByProbs(_shipWalk->state, _shipWalk->vertTypes[dstI] == LOCATOR_VANT);

        // handle cannon reload
        if ((_man->state == MAN_RELOAD) && (_shipWalk->vertTypes[_man->sourceI] == LOCATOR_CANNON) &&
