...
void BackgroundShipTask::WriteDataToAttribute()
{
    ...
    if (pATmp)
        m_StartDate.WriteDataToAttributeClass(pATmp);
    pATmp = pARoot->CreateSubAClass(pARoot, "enddate");
    if (pATmp)
        m_EndDate.WriteDataToAttributeClass(pATmp);
}
...
void BackgroundShipTask::ReadParametersFromAttributes()
{
    ...
    if (pATmp)
        m_StartDate.ReadDataFromAttributeClass(pATmp);
    pATmp = pARoot->GetAttributeClass("enddate");
    if (pATmp)
        m_EndDate.ReadDataFromAttributeClass(pATmp);
