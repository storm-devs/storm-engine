...
void SEA::EnvMap_GetSideMatrix(D3DCUBEMAP_FACES Face, Matrix &mView)
{
    Vector vLookTo, vUp;
    ...
    mView.BuildView(0.0f, vLookTo, vUp);
}
...
bool SEA::SunRoad_Render2()
{
    ...
    // rs->SetProjection( Matrix().BuildProjection(PI / 2.0f, 256.0f, 256.0f, 1.0f, 4000.0f) );
...
bool SEA::EnvMap_Render2()
{
    ...
    auto _mWorld = Matrix();
    Matrix _mView = rs->GetView();
    Matrix _mProj = rs->GetProjection();
    ...
    Matrix mTex;
    mTex.BuildScale(Vector(0.5f, -0.5f, 0.5f));
    mTex.pos = Vector(0.5f, 0.5f, 0.5f);

    Matrix mProj = rs->GetProjection();
    mProj = mProj * mTex;

    Matrix mCurWorld = rs->GetWorld();
    Matrix mCurView = rs->GetView();
...
{
    Matrix mOldView = rs->GetView();
    Matrix mOldProjection = rs->GetProjection();
