...
void ClearComment(char * pStr)
{
	...
	for (int32_t i=iStart;i<iLen;i++) pStr[i] = 0;
}
...
uint32_t CTechnique::GetSRSIndex(char *pStr)
{
	...
	for (uint32_t i=0;i<dwNumParam;i++) if (storm::iEquals(pStr,RenderStates[i].cName)) return i;
...
uint32_t CTechnique::GetSTSSIndex(char *pStr)
{
	...
	for (uint32_t i=0;i<dwNumParam;i++) if (storm::iEquals(pStr,TexturesStageStates[i].cName)) return i;
...
uint32_t CTechnique::GetSAMPIndex(char *pStr)
{
	...
	for (uint32_t i = 0; i<dwNumParam; i++) if (storm::iEquals(pStr, SampleStates[i].cName)) return i;
...
{
	for (uint32_t i=0;i<dwNumParam;i++) if (storm::iEquals(pStr,pParam[i].cName)) return i;
...
uint32_t CTechnique::AddShader(char *pShaderName)
{
	...
	for (uint32_t i=0;i<dwNumShaders;i++) if (storm::iEquals(pShaders[i].pName,pShaderName)) return i;
...
char *CTechnique::Preprocessor(char *pBuffer, uint32_t & dwSize)
{
	...
	while (nullptr != (pStr = GetString(pBuffer,dwBufSize,pStr)))
	{
		...
		while(nullptr != (pToken = GetToken(pToken,sToken,bToken)))
		{
			...
			{
				// find define
				for (i=0;i<dwNumDefines;i++) if (dwTempLen==pDefines[i].dwNameLen && storm::iEquals(pDefines[i].pName,sToken))
				{
...
#ifdef _WIN32 // FIX_LINUX replace D3DXAssembleShader with D3DCompile or ???
#ifndef _XBOX
...
bool CTechnique::ExecutePassStart()
{
	...
	//for (uint32_t i=0;i<dwNumBlocks;i++) if (pBlocks[i].dwHashBlockName == dwHashCode && (storm::iEquals(pBlocks[i].pBlockName,sCurrentBlockName))) break;
