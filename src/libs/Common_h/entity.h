...
class ENTITY
{
  ...
  public:
    ...
    virtual dword AttributeChanged(ATTRIBUTES *)
    {
        return 0;
    };
    ...
    {
        if (!__pRenderService)
            __pRenderService = (VDX8RENDER *)_VSYSTEM_API->CreateService("dx8render");
        return *__pRenderService;
    };
    virtual VGEOMETRY &Geometry()
    {
        if (!__pGeoService)
            __pGeoService = (VGEOMETRY *)_VSYSTEM_API->CreateService("geometry");
        return *__pGeoService;
    };
    virtual COLLIDE &Collide()
    {
        if (!__pCollideService)
            __pCollideService = (COLLIDE *)_VSYSTEM_API->CreateService("coll");
        return *__pCollideService;
    };

    virtual void SetServer(bool bServer)
    {
