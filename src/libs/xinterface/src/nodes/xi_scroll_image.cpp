        ...
        for (n = 0; n < m_nSlotsQnt; n++)
        {
            ...
            while (pScroll != nullptr)
            {
                ...
                else if (m_Image[pScroll->imageNum].slots[n].img != -1)
                {
                    ...
                    pPictureService->BGFXGetTexturePos(m_Image[pScroll->imageNum].slots[n].img, rectTex);

                    auto texture = m_rs->GetBGFXTextureFromID(m_nGroupTex[m_Image[pScroll->imageNum].slots[n].tex]);
                    m_rs->GetPrimitiveRenderer()->Texture = texture;
                }
                else
                {
                ...
                if (m_Image[pScroll->imageNum].slots[n].useSpecTechnique)
                {
                    ...
                    for (int i = 0; i < 4; i += 4)
                    {
                        ...
                        vertices.push_back(VERTEX_POSITION_TEXTURE_COLOR{pVertices[i + 0].pos.x, pVertices[i + 0].pos.y,
                                                                         pVertices[i + 0].pos.z, pVertices[i + 0].tu,
                                                                         pVertices[i + 0].tv, pVertices[i + 0].color});
                        vertices.push_back(VERTEX_POSITION_TEXTURE_COLOR{pVertices[i + 2].pos.x, pVertices[i + 2].pos.y,
                                                                         pVertices[i + 2].pos.z, pVertices[i + 2].tu,
                                                                         pVertices[i + 2].tv, pVertices[i + 1].color});
                        vertices.push_back(VERTEX_POSITION_TEXTURE_COLOR{pVertices[i + 1].pos.x, pVertices[i + 1].pos.y,
                                                                         pVertices[i + 1].pos.z, pVertices[i + 1].tu,
                                                                         pVertices[i + 1].tv, pVertices[i + 1].color});
                        vertices.push_back(VERTEX_POSITION_TEXTURE_COLOR{pVertices[i + 3].pos.x, pVertices[i + 3].pos.y,
                                                                         pVertices[i + 3].pos.z, pVertices[i + 3].tu,
                                                                         pVertices[i + 3].tv, pVertices[i + 3].color});
                        m_rs->GetPrimitiveRenderer()->PushVertices(vertices);
                    }
                }
                else
...
void CXI_SCROLLIMAGE::ReleaseAll()
{
    ...
    if (m_idBadPic && m_idBadTexture)
        for (i = 0; i < m_nSlotsQnt; i++)
        {
            if (m_idBadPic[i] == -1)
            {
                BGFX_TEXTURE_RELEASE(m_rs, m_idBadTexture[i]);
            }
            else
                m_idBadTexture[i] = -1;
            m_idBadPic[i] = -1;
        ...
        for (i = nScrollItemNum; i < nScrollLastNum; i++)
        {
            ...
            if (pAttribute != nullptr)
            {
                ...
                for (n = 0; n < m_nSlotsQnt; n++)
                {
                    ...
                    if (m_Image[i].slots[n].tex != -1)
                    {
                        m_Image[i].slots[n].img = pPictureService->BGFXGetImageNum(
                            m_sGroupName[m_Image[i].slots[n].tex], pAttribute->GetAttribute(param));
                    }
                    else
                    {
                        m_Image[i].slots[n].img = -1;
        ...
        for (i = 0; i < m_Image.size(); i++)
        {
            ...
            if (pListEntity != nullptr)
            {
                ...
                for (n = 0; n < m_nSlotsQnt; n++)
                {
                    ...
                    {
                        m_Image[i].slots[n].img = pPictureService->BGFXGetImageNum(
                            m_sGroupName[m_Image[i].slots[n].tex], pListEntity->GetAttribute(param));
                    }
                    else
                    {
                        m_Image[i].slots[n].img = -1;
