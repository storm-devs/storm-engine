...
uint32_t _IsKeyPressed(VS_STACK *pS)
{
    ...
    if (strKeyName)
    {
        ...
        {
            bIsPressed = (core.Controls->GetDebugAsyncKeyState(VK_SHIFT) < 0);
        }
        else if (storm::iEquals(strKeyName, "control"))
        {
            bIsPressed = (core.Controls->GetDebugAsyncKeyState(VK_CONTROL) < 0);
        }
        else if (storm::iEquals(strKeyName, "alt"))
        {
            bIsPressed = (core.Controls->GetDebugAsyncKeyState(VK_MENU) < 0);
        }
    }
    // set return data
    pDat = (VDATA *)pS->Push();
