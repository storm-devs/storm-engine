...
WdmEventWindow::WdmEventWindow()
{
    ...
    {
        ATTRIBUTES *at =
            wdmObjects->wm->AttributesPointer->FindAClass(wdmObjects->wm->AttributesPointer, "eventWindow");
        if (at)
            at = at->FindAClass(at, "font");
        if (at)
        {
            char *s = at->GetAttribute("header");
            if (s)
                headerFont = wdmObjects->rs->LoadFont(s);
            if (headerFont < 0)
                headerFont = FONT_DEFAULT;
            s = at->GetAttribute("text");
            if (s)
                textFont = wdmObjects->rs->LoadFont(s);
            if (textFont < 0)
                textFont = FONT_DEFAULT;
            s = at->GetAttribute("button");
            if (s)
                buttonFont = wdmObjects->rs->LoadFont(s);
            if (buttonFont < 0)
                buttonFont = FONT_DEFAULT;
        }
    }
...
WdmEventWindow::~WdmEventWindow()
{
    ...
    if (token)
        delete token;
    if (textureBkg >= 0)
        wdmObjects->rs->TextureRelease(textureBkg);
    if (textureButton >= 0)
        wdmObjects->rs->TextureRelease(textureButton);
    if (headerFont != FONT_DEFAULT)
        wdmObjects->rs->UnloadFont(headerFont);
    if (textFont != FONT_DEFAULT)
        wdmObjects->rs->UnloadFont(headerFont);
    if (buttonFont != FONT_DEFAULT)
        wdmObjects->rs->UnloadFont(headerFont);
}
...
void WdmEventWindow::SplitText()
{
        ...
        else
        {
            ...
            if (tk)
            {
                *t = 0;
                ...
                if (numTokens > 0)
                {
                    if (maxsw < token[numTokens].w + token[numTokens - 1].w)
                        maxsw = token[numTokens].w + token[numTokens - 1].w;
                }
                else
                {
                    if (maxsw < token[numTokens].w)
                        maxsw = token[numTokens].w;
                }
                tk = null;
                numTokens++;
            }
    ...
    if (yesWidth && noWidth)
        butW *= 2;
    if (w < butW + 5 + 2 * WDM_EW_WSPACE)
        w = butW + 5 + 2 * WDM_EW_WSPACE;
    float scrw, scrh;
...
void WdmEventWindow::LRender(VDX8RENDER *rs)
{
    ...
    //Текст
    if (headerWidth)
        rs->Print(headerFont, 0xffffffff, long(winX + (winW - headerWidth) * 0.5f), long(winY + WDM_EW_UPSPACE), text);
    for (long i = 0; i < numTokens; i++)
