...
WdmDateLabel::~WdmDateLabel()
{
    dateFont = -1;
    if (txTextBar >= 0)
        wdmObjects->rs->TextureRelease(txTextBar);
    if (txSkyClock >= 0)
        wdmObjects->rs->TextureRelease(txSkyClock);
    if (txSkyClockFrame >= 0)
        wdmObjects->rs->TextureRelease(txSkyClockFrame);
}
...
void WdmDateLabel::LRender(VDX8RENDER *rs)
{
    ...
    if (fy >= 0.0f)
    {
        ...
        fy *= 0.5f;
    rs->Print(font, 0xffffffff, long(cx + 8), long(cy + fy), "%i %s", wdmObjects->wm->day,
              month[wdmObjects->wm->mon - 1]);
