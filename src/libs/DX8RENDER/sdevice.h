...
#define RS_RECT_VERTEX_FORMAT (D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_TEX1)
  ...
  public:
    ...
    virtual void _cdecl DrawBuffer(long vbuff, long stride, long ibuff, long minv, long numv, long startidx,
                                   long numtrg, const char *cBlockName = 0, dword dwNumParams = 0, ...);
    ...
    virtual float GetHeightDeformator()
    {
    ...
    virtual HRESULT CreateDepthStencilSurface(UINT Width, UINT Height, D3DFORMAT Format,
                                              D3DMULTISAMPLE_TYPE MultiSample, IDirect3DSurface9 **ppSurface);
    ...
    void MakeScreenShot();
    bool LoadTextureSurface(HANDLE file, IDirect3DSurface9 *suface, dword mipSize, dword width, dword height,
                            bool isSwizzled);
    dword LoadCubmapSide(HANDLE file, IDirect3DCubeTexture9 *tex, D3DCUBEMAP_FACES face, dword numMips, dword mipSize,
                         dword size, bool isSwizzled);
    ...
    void MakeDrawVector(RS_LINE *pLines, dword dwNumSubLines, const CMatrix &mMatrix, CVECTOR vUp, CVECTOR v1,
                        CVECTOR v2, float fScale, dword dwColor);
    void DrawVector(const CVECTOR &v1, const CVECTOR &v2, dword dwColor, const char *pTechniqueName,
                    dword dwNumParams = 0, ...);
