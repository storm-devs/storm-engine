...
float TSinkSplash::HeightF(uint32_t _time, float _r, float _k)
{
    ...
    const auto secondPart =
        ((GRID_LENGTH - _r) / GRID_LENGTH) *
        (1.0f + fabs(sin((-PI * static_cast<float>(_time) / SPLASH_FADE_TIME) + PIm2 * _r / GRID_LENGTH)));
...
void TSinkSplash::Process(uint32_t _dTime, uint16_t *_indexes, SINK_VERTEX *_vertexes)
{
    ...
    for (z = 0; z < GRID_STEPS; ++z)
        for (x = 0; x < GRID_STEPS; ++x)
        {
            ...
            _vertexes->color = ((((3 * SPLASH_FADE_TIME - static_cast<long>(time))) << 8) / (3 * SPLASH_FADE_TIME))
                               << 24;
