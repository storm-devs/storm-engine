...
bool HELPCHOOSER::RunChooser(char *ChooserGroup)
{
    ...
    if (m_nRectQ > 0)
    {
        ...
        if (m_pRectList == null || m_psRectName == null)
        {
    ...
    if (m_idVBuf == -1)
        api->Trace("WARNING! Can`t create vertex buffer for help chooser");
    else
    {
        HCHOOSER_VERTEX *pv = (HCHOOSER_VERTEX *)rs->LockVertexBuffer(m_idVBuf);
        if (pv != null)
...
{
    if (m_nCurRect < 0 || m_nCurRect >= m_nRectQ || m_pRectList == null)
        return 0;
