...
class CText
{
  ...
  public:
    ...
    CText &operator=(CText &text)
    {
        return *this = text.pString;
    }
    ...
    {
        if (pStr && pString)
            return stricmp(pStr, pString) == 0;
        return pStr == pString;
    }
    bool operator==(CText &text)
    {
        return *this == text.pString;
    }
    ...
    CText operator+(char *pStr)
    {
        return CText(pString) += pStr;
    }
    CText operator+(CText &text)
    {
        return CText(pString) += text.pString;
    }
        ...
        for (int n = 0; n < q; n++)
            if (strnicmp(&pString[n], pStr, q2) == 0)
            {
                if (q1 > q2)
                {
                    ...
                    {
                        if (n > 0)
                            memcpy(pNew, pString, n);
                        if (n < q)
                            memcpy(&pNew[n], &pString[n + q2], q - n);
                    }
                    delete pString;
                    pString = pNew;
                }
