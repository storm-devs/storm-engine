...
bool CXI_LINECOLLECTION::Init(INIFILE *ini1, char *name1, INIFILE *ini2, char *name2, VDX8RENDER *rs, XYRECT &hostRect,
                              XYPOINT &ScreenSize)
{
...
void CXI_LINECOLLECTION::LoadIni(INIFILE *ini1, char *name1, INIFILE *ini2, char *name2)
{
    ...
    if (ini1->ReadString(name1, "line", param, sizeof(param) - 1, ""))
        do
        {
            ...
            m_aLines[n].vPos.x = (float)scrRect.left;
            m_aLines[n + 1].vPos.x = (float)scrRect.right;
            m_aLines[n].vPos.y = (float)scrRect.top;
            m_aLines[n + 1].vPos.y = (float)scrRect.bottom;
...
void CXI_LINECOLLECTION::SaveParametersToIni()
{
    ...
    // save position
    //	_snprintf( pcWriteParam, sizeof(pcWriteParam), "%d,%d,%d,%d", m_rect.left, m_rect.top, m_rect.right,
