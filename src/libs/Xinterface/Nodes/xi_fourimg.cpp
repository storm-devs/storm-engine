...
void CXI_FOURIMAGE::Draw(bool bSelected, dword Delta_Time)
{
    ...
    if (m_bUse)
    {
        ...
        if (bUseOneString || bUseTwoString)
        {
            ...
            if (m_xOneOffset > 0)
                nAlignment1 = ALIGN_RIGHT;
            else if (m_xOneOffset < 0)
                nAlignment1 = ALIGN_LEFT;
            if (m_xTwoOffset > 0)
                nAlignment2 = ALIGN_RIGHT;
            else if (m_xTwoOffset < 0)
                nAlignment2 = ALIGN_LEFT;
...
bool CXI_FOURIMAGE::Init(INIFILE *ini1, char *name1, INIFILE *ini2, char *name2, VDX8RENDER *rs, XYRECT &hostRect,
                         XYPOINT &ScreenSize)
{
...
void CXI_FOURIMAGE::SaveParametersToIni()
{
    ...
    for (long n = 0; n < 4; n++)
    {
        _snprintf(pcWriteParam, sizeof(pcWriteParam), "%d,%d,%d,%d", m_imgRect[n].left, m_imgRect[n].top,
