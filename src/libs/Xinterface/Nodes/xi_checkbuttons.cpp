...
#define PicName(bDisable, bSelect)                                                                                     \
    ((bDisable && !m_sDisablePicture.IsEmpty()) ? m_sDisablePicture : ((bSelect) ? m_sSelectPicture : m_sNormalPicture))
#define PicColor(bDisable, bSelect)                                                                                    \
    ((bDisable) ? m_dwDisablePicColor : ((bSelect) ? m_dwSelectPicColor : m_dwNormalPicColor))

CXI_CHECKBUTTONS::CXI_CHECKBUTTONS() : m_aButton(_FL)
{
    m_bClickable = true;
...
bool CXI_CHECKBUTTONS::Init(INIFILE *ini1, char *name1, INIFILE *ini2, char *name2, VDX8RENDER *rs, XYRECT &hostRect,
                            XYPOINT &ScreenSize)
{
...
void CXI_CHECKBUTTONS::LoadIni(INIFILE *ini1, char *name1, INIFILE *ini2, char *name2)
{
    ...
    m_fTextSectionInterval =
        GetIniFloat(ini1, name1, ini2, name2, "sectioninterval", (float)m_rs->CharHeight(m_nFontNum));
    ...
    if (ReadIniString(ini1, name1, ini2, name2, "alignment", param, sizeof(param), ""))
    {
        if (stricmp(param, "center") == 0)
            m_nFontAlignment = ALIGN_CENTER;
        if (stricmp(param, "right") == 0)
            m_nFontAlignment = ALIGN_RIGHT;
    }
...
void CXI_CHECKBUTTONS::SaveParametersToIni()
{
    ...
    if (m_bIndividualPos)
    {
        for (long n = 0; n < m_aButton; n++)
        {
            ...
            {
                _snprintf(pcWriteParam, sizeof(pcWriteParam), "%f,%f", m_aButton[n]->pos.x, m_aButton[n]->pos.y);
...
dword _cdecl CXI_CHECKBUTTONS::MessageProc(long msgcode, MESSAGE &message)
{
    ...
    {
        long nButtonNum = message.Long() - 1;
        if (nButtonNum < 0 || nButtonNum >= m_aButton)
            return 0;
    ...
    case 5: // запретить/разрешить кнопку
    {
        ...
        m_aButton[nButtonNum]->pImg->LoadFromBase(m_sIconGroupName, PicName(bDisable, m_aButton[nButtonNum]->bChoose),
                                                  false);
    }
    break;
    }
...
void CXI_CHECKBUTTONS::UpdateTextInfo(long nButtonNum)
{
    ...
    for (long n = 0; n < asOutStr; n++)
    {
        ...
        case ALIGN_LEFT:
            ...
            break;
        case ALIGN_CENTER:
            m_aButton[nButtonNum]->aStr[n].fX = (float)((nWidth - nOffset) / 2);
            break;
        case ALIGN_RIGHT:
            m_aButton[nButtonNum]->aStr[n].fX = (float)(nWidth - nOffset);
            break;
        }
    }
}
...
{
    if (nButtonIndex < 0 || nButtonIndex >= m_aButton)
        return;
    char atrName[128];
