...
CXI_WINDOW::CXI_WINDOW() : m_aNodeNameList(_FL)
{
...
bool CXI_WINDOW::Init(INIFILE *ini1, char *name1, INIFILE *ini2, char *name2, VDX8RENDER *rs, XYRECT &hostRect,
                      XYPOINT &ScreenSize)
{
...
void CXI_WINDOW::SetActive(bool bActive)
{
    ...
    for (long n = 0; n < m_aNodeNameList; n++)
    {
        ...
        if (pNod)
        {
            ...
            if (pNod->m_nNodeType == NODETYPE_WINDOW)
                ((CXI_WINDOW *)pNod)->SetActive(bActive);
        }
    }
}
