...
CXI_FORMATEDTEXT::STRING_DESCRIBER::STRING_DESCRIBER(char *ls) : m_tags(_FL)
{
...
CXI_FORMATEDTEXT::CXI_FORMATEDTEXT() : m_asSyncNodes(_FL)
{
        ...
        if (sd->lineStr != NULL && sd->lineStr[0] != 0)
        {
            ...
            else
            {
                ...
                if (m_nAlignment == ALIGN_CENTER)
                    nX -= nAllWidth / 2;
                if (m_nAlignment == ALIGN_RIGHT)
                    nX -= nAllWidth;
                for (long n = 0; n < sd->m_tags; n++)
                {
                    m_rs->ExtPrint(m_idFont, sd->m_tags[n].dwColor, 0, ALIGN_LEFT, true, m_fFontScale, m_screenSize.x,
                                   m_screenSize.y, nX, curY, "%s", sd->m_tags[n].str.GetBuffer());
...
bool CXI_FORMATEDTEXT::Init(INIFILE *ini1, char *name1, INIFILE *ini2, char *name2, VDX8RENDER *rs, XYRECT &hostRect,
                            XYPOINT &ScreenSize)
{
...
bool CXI_FORMATEDTEXT::IsClick(int buttonID, long xPos, long yPos)
{
    ...
    for (int i = 0; i < nNum && pdescr != null; i++)
        pdescr = pdescr->next;
    if (pdescr != null && m_nCurGroupNum != pdescr->strGroup)
...
void CXI_FORMATEDTEXT::ChangePosition(XYRECT &rNewPos)
{
    ...
    if (m_nAlignment == ALIGN_CENTER)
        m_nPrintLeftOffset = m_leftOffset + long((m_rect.left + m_rect.right - m_leftOffset) * .5f);
    ...
    if (m_idVBuf != -1)
        pv = (XI_ONLYONETEX_VERTEX *)m_rs->LockVertexBuffer(m_idVBuf);
    if (pv != null)
    {
        pv[0].tu = pv[1].tu = 0.f;
        pv[2].tu = pv[3].tu = 1.f;
...
void CXI_FORMATEDTEXT::LoadIni(INIFILE *ini1, char *name1, INIFILE *ini2, char *name2)
{
    ...
    if (m_bSelectableCursor)
        m_bSelected = true;
    m_idVBuf =
        m_rs->CreateVertexBufferManaged(XI_ONLYONETEX_FVF, 4 * 4 * sizeof(XI_ONLYONETEX_VERTEX), D3DUSAGE_WRITEONLY);
...
void CXI_FORMATEDTEXT::SetPointer(float fPos)
{
    ...
    else
    {
        ...
        for (int nq = (int)(m_nAllTextStrings * fPos); nq > 0; nq--)
            if (m_listCur != null && m_listCur->next != null)
...
void CXI_FORMATEDTEXT::MakeTagChecking(bool &tagState, dword &tagColor, dword normColor, STRING_DESCRIBER *pStrDescr)
{
    ...
    while (str && str[0])
    {
        ...
        if (q > 0)
        {
            ...
            if (tagState)
                pStrDescr->m_tags[n].dwColor = normColor;
            else
                pStrDescr->m_tags[n].dwColor = tagColor;
...
dword _cdecl CXI_FORMATEDTEXT::MessageProc(long msgcode, MESSAGE &message)
{
    ...
    case 3: // заполнить атрибуты размерами текстов
    {
        ...
        int oldgroup = -1;
        int idx = 0;
        if (m_listCur != null && m_listCur->prev != null)
...
long CXI_FORMATEDTEXT::FindUpGroup(long grNum)
{
    ...
    {
        for (; dscrTmp != null && dscrTmp->strGroup >= grNum; dscrTmp = dscrTmp->prev)
            upPos--;
        upPos++;
        return upPos;
    }

    for (; dscrTmp != null && dscrTmp->strGroup < grNum; dscrTmp = dscrTmp->next)
        upPos++;
    if (dscrTmp == null)
        upPos--;
    return upPos;
    ...
    if (pv != null)
    {
        ...
        if (m_bBackRectangle)
        {
            pv[12].pos.y = pv[14].pos.y =
                (float)(m_rect.top - m_nUpRectOffset - m_rBorderOffset.top + m_vertOffset * upIdx);
            pv[13].pos.y = pv[15].pos.y =
                (float)(m_rect.top - m_nUpRectOffset + m_rBorderOffset.bottom + m_vertOffset * (downIdx + 1));
        }
        m_rs->UnLockVertexBuffer(m_idVBuf);
    }
