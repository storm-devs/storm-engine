...
bool CAviPlayer::PlayMedia(char *fileName)
{
    ...
    hr = rs->CreateTexture(AVI_GetTextureSize(srcWidth), AVI_GetTextureSize(srcHeight), 1, 0, d3d8surf_desc.Format,
...
bool CAviPlayer::GetInterfaces()
{
    ...
    hr = CoCreateInstance(CLSID_AMMultiMediaStream, NULL, CLSCTX_INPROC_SERVER, IID_IAMMultiMediaStream,
                          (void **)&pAMStream);
    if (FAILED(hr))
    {
