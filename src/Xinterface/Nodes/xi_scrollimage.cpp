                        ...
                        if (m_bUseOneString)
                        {
                            if (m_Image[pScroll->imageNum].str1 != -1L)
                                ptrOwner->PrintIntoWindow(
                                    ...
                                    (int)(m_ImageSize.x * pScroll->fCurScale), (int)(-24 * m_nOneStrScale));
                            else if (m_Image[pScroll->imageNum].string1 != NULL)
                                ptrOwner->PrintIntoWindow(
                                    m_rect.left, m_rect.right, m_nOneStrFont, m_dwOneStrForeColor, m_dwOneStrBackColor,
                                    m_nOneStrAlign, true, m_nOneStrScale, m_screenSize.x, m_screenSize.y,
                                    long(pScroll->pCenter.x + m_lOneStrX), m_rect.top + m_lOneStrOffset,
                                    m_Image[pScroll->imageNum].string1, (int)(m_ImageSize.x * pScroll->fCurScale),
                        ...
                        if (m_bUseTwoString)
                        {
                            if (m_Image[pScroll->imageNum].str2 != -1L)
                                ptrOwner->PrintIntoWindow(
                                    ...
                                    (int)(m_ImageSize.x * pScroll->fCurScale), (int)(-24 * m_nTwoStrScale));
                            if (m_Image[pScroll->imageNum].string2 != NULL)
                                ptrOwner->PrintIntoWindow(
                                    m_rect.left, m_rect.right, m_nTwoStrFont, m_dwTwoStrForeColor, m_dwTwoStrBackColor,
                                    m_nTwoStrAlign, true, m_nTwoStrScale, m_screenSize.x, m_screenSize.y,
                                    long(pScroll->pCenter.x + m_lTwoStrX), m_rect.top + m_lTwoStrOffset,
                                    m_Image[pScroll->imageNum].string2, (int)(m_ImageSize.x * pScroll->fCurScale),
...
bool CXI_SCROLLIMAGE::Init(INIFILE *ini1, char *name1, INIFILE *ini2, char *name2, VDX8RENDER *rs, XYRECT &hostRect,
                           XYPOINT &ScreenSize)
{
...
void CXI_SCROLLIMAGE::LoadIni(INIFILE *ini1, char *name1, INIFILE *ini2, char *name2)
{
    ...
    if (m_bUseOneString)
    {
        ...
        if (m_lOneStrX > 0)
            m_nOneStrAlign = ALIGN_RIGHT;
        else if (m_lOneStrX < 0)
            m_nOneStrAlign = ALIGN_LEFT;
        else
            m_nOneStrAlign = ALIGN_CENTER;
    ...
    if (m_bUseTwoString)
    {
        ...
        if (m_lTwoStrX > 0)
            m_nTwoStrAlign = ALIGN_RIGHT;
        else if (m_lTwoStrX < 0)
            m_nTwoStrAlign = ALIGN_LEFT;
        else
            m_nTwoStrAlign = ALIGN_CENTER;
