...
#define ITEXTURE_RELEASE(rs, id)                                                                                       \
    if (rs != NULL && id != -1)                                                                                        \
    {                                                                                                                  \
        rs->TextureRelease(id);                                                                                        \
        id = -1;                                                                                                       \
    }
#define IFONT_RELEASE(rs, id)                                                                                          \
    if (rs != NULL && id != -1)                                                                                        \
    {                                                                                                                  \
        rs->UnloadFont(id);                                                                                            \
        id = -1;                                                                                                       \
    }
#define IPTR_DELETE(ptr)                                                                                               \
    if (ptr != NULL)                                                                                                   \
    {                                                                                                                  \
        delete ptr;                                                                                                    \
        ptr = NULL;                                                                                                    \
    }
#define IVERTEXBUFFER_RELEASE(rs, id)                                                                                  \
    if (rs != NULL && id != -1)                                                                                        \
    {                                                                                                                  \
        rs->ReleaseVertexBuffer(id);                                                                                   \
        id = -1;                                                                                                       \
    }
#define IINDEXBUFFER_RELEASE(rs, id)                                                                                   \
    if (rs != NULL && id != -1)                                                                                        \
    {                                                                                                                  \
...
bool SetVertexToBuffer(ISG_VERTEX *pv, FRECT &frPos, FRECT &frTex)
{
    if (pv == NULL)
        return false;
    pv[0].pos.x = frPos.left;
    pv[0].pos.y = frPos.top;
    pv[0].pos.z = 1.f;
    pv[0].tu = frTex.left;
    pv[0].tv = frTex.top;
    pv[0].w = .5f;
    pv[1].pos.x = frPos.left;
    pv[1].pos.y = frPos.bottom;
    pv[1].pos.z = 1.f;
    pv[1].tu = frTex.left;
    pv[1].tv = frTex.bottom;
    pv[1].w = .5f;
    pv[2].pos.x = frPos.right;
    pv[2].pos.y = frPos.top;
    pv[2].pos.z = 1.f;
    pv[2].tu = frTex.right;
    pv[2].tv = frTex.top;
    pv[2].w = .5f;
    pv[3].pos.x = frPos.right;
    pv[3].pos.y = frPos.bottom;
    pv[3].pos.z = 1.f;
    pv[3].tu = frTex.right;
    pv[3].tv = frTex.bottom;
    pv[3].w = .5f;
    return true;
}
...
bool ISPYGLASS::SetParameters()
{
    ...
    if (m_fFadeSpeed > 0.001f)
        m_fFadeSpeed = 1.f / m_fFadeSpeed;
    else
        m_fFadeSpeed = -1.f;
    m_fCurAlpha = 255.f;
    ...
    if (texName != NULL)
        m_idBackTex = rs->TextureCreate(texName);
    else
        m_idBackTex = -1;
    texName = pAttr->GetAttribute("IconsTextureName");
    if (texName != NULL)
        m_idIconTex = rs->TextureCreate(texName);
    else
        m_idIconTex = -1;
    texName = pAttr->GetAttribute("NationsTextureName");
    if (texName != NULL)
        m_idNationsTex = rs->TextureCreate(texName);
    else
        m_idNationsTex = -1;

    // Создадим буферы
    m_idVBuf = rs->CreateVertexBufferManaged(ISG_VERTEX_FORMAT, 4 * (1 + 1 + 6) * sizeof(ISG_VERTEX),
                                             D3DUSAGE_WRITEONLY); // подложка,
    ...
    if (pA != NULL)
    {
        ...
        if (GetFRectFromAttribute(pA->GetAttributeClass("hull"), fr))
            SetVertexToBuffer(&pv[8], pos, fr);

        pos.right = (pos.left = (float)nIconPos[2]) + fpIconSize.x;
        if (GetFRectFromAttribute(pA->GetAttributeClass("sail"), fr))
            SetVertexToBuffer(&pv[12], pos, fr);

        pos.right = (pos.left = (float)nIconPos[3]) + fpIconSize.x;
        if (GetFRectFromAttribute(pA->GetAttributeClass("crew"), fr))
            SetVertexToBuffer(&pv[16], pos, fr);

        pos.right = (pos.left = (float)nIconPos[4]) + fpIconSize.x;
        if (GetFRectFromAttribute(pA->GetAttributeClass("speed"), fr))
            SetVertexToBuffer(&pv[20], pos, fr);

        pos.right = (pos.left = (float)nIconPos[5]) + fpIconSize.x;
        if (GetFRectFromAttribute(pA->GetAttributeClass("cannons"), fr))
            SetVertexToBuffer(&pv[24], pos, fr);

        pos.right = (pos.left = (float)nIconPos[6]) + fpIconSize.x;
        if (GetFRectFromAttribute(pA->GetAttributeClass("charge0"), m_frChargeUV[0]))
            SetVertexToBuffer(&pv[28], pos, m_frChargeUV[0]);
...
void ISPYGLASS::Execute(dword delta_time)
{
    ...
    {
        if (m_fFadeSpeed < 0)
            m_fCurAlpha = m_bFadeOut ? 256.f : 0.f;
        else if (m_bFadeOut)
        {
            if (m_fCurAlpha < 255.9f)
                m_fCurAlpha += delta_time * .256f * m_fFadeSpeed;
        }
        else
        {
            if (m_fCurAlpha > 0)
...
void ISPYGLASS::Realize(dword delta_time)
{
    if (m_bVisible)
    {
        ...
        if (m_fCurAlpha < 256.9f)
            dwcolor = (((DWORD)m_fCurAlpha) << 24) | 0x00FFFFFF;
        ...
        // тексты
        if (m_sShipName != NULL)
            rs->ExtPrint(m_idFontOne, dwcolor, 0, ALIGN_LEFT, true, m_fShipNameScale, 0, 0, m_nLeftTextShipData,
                         m_nTopTextShipName, "%s", m_sShipName);
        if (m_sShipType != NULL)
            rs->ExtPrint(m_idFontOne, dwcolor, 0, ALIGN_LEFT, true, m_fShipTypeScale, 0, 0, m_nLeftTextShipData,
                         m_nTopTextShipType, "%s", m_sShipType);
        if (m_nShipHull != -1)
            rs->ExtPrint(m_idFontTwo, dwcolor, 0, ALIGN_LEFT, true, m_fFontTwoScale, 0, 0, m_nLeftTextPos[0],
                         m_nTopDigitData, "%d%%", m_nShipHull);
        if (m_nShipSail != -1)
            rs->ExtPrint(m_idFontTwo, dwcolor, 0, ALIGN_LEFT, true, m_fFontTwoScale, 0, 0, m_nLeftTextPos[1],
                         m_nTopDigitData, "%d%%", m_nShipSail);
        if (m_nShipCrew != -1)
            rs->ExtPrint(m_idFontTwo, dwcolor, 0, ALIGN_LEFT, true, m_fFontTwoScale, 0, 0, m_nLeftTextPos[2],
                         m_nTopDigitData, "%d", m_nShipCrew);
        if (m_fShipSpeed != -1)
            rs->ExtPrint(m_idFontTwo, dwcolor, 0, ALIGN_LEFT, true, m_fFontTwoScale, 0, 0, m_nLeftTextPos[3],
                         m_nTopDigitData, "%.1f", m_fShipSpeed);
        if (m_nShipCannons != -1)
            rs->ExtPrint(m_idFontTwo, dwcolor, 0, ALIGN_LEFT, true, m_fFontTwoScale, 0, 0, m_nLeftTextPos[4],
                         m_nTopDigitData, "%d", m_nShipCannons);
        if (m_fSailTo != -1)
            rs->ExtPrint(m_idFontTwo, dwcolor, 0, ALIGN_LEFT, true, m_fFontTwoScale, 0, 0, m_nLeftTextPos[5],
                         m_nTopDigitData, "%.1f", m_fSailTo);
    }
}
...
dword _cdecl ISPYGLASS::ProcessMessage(MESSAGE &message)
{
    ...
    case MSG_ISG_UPDATE: {
        ...
        if ((ftmp = (float)rs->StringWidth(m_sShipType, m_idFontOne, m_fFontOneScale)) > m_fStringMaxWidth)
            ...
            m_fShipTypeScale = m_fFontOneScale;
        // Get next ship data
...
void ISPYGLASS::Update(long chargeType)
{
    ...
    {
    case 0:
        SetVertexUV(&pv[28], m_frChargeUV[0]);
        break;
    case 1:
        SetVertexUV(&pv[28], m_frChargeUV[1]);
        break;
    case 2:
        SetVertexUV(&pv[28], m_frChargeUV[2]);
        break;
    case 3:
        SetVertexUV(&pv[28], m_frChargeUV[3]);
        break;
    }
