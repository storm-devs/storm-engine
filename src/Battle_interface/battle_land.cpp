...
{
    if (pA != NULL)
        return pA->GetAttributeAsDword(sAttrName, defValue);
    else
        return defValue;
}
...
{
    if (pA != NULL)
        return pA->GetAttributeAsFloat(sAttrName, defValue);
    else
        return defValue;
}
...
void BATTLE_LAND_INTERFACE::Execute(dword delta_time)
{
    if (m_nCommandos > 0)
    {
        ...
        if (m_bAlarmUp)
            m_fCurAlarmBlind += m_fAlarmBlindUpSpeed * delta_time;
        else
            m_fCurAlarmBlind -= m_fAlarmBlindDownSpeed * delta_time;
        if (m_fCurAlarmBlind < 0.f)
        {
            m_fCurAlarmBlind = 0.f;
            m_bAlarmUp = true;
        }
        if (m_fCurAlarmBlind > 1.f)
        {
...
void BATTLE_LAND_INTERFACE::Realize(dword delta_time)
{
    ...
    if (m_nCommandos > 0)
    {
        ...
        if (m_bShowCommandos && m_nShowCommand == 0)
        {
            ...
            rs->DrawBuffer(m_idVBufCommandos, sizeof(BI_COLOR_VERTEX), m_idIBufCommandos, m_nMaxCommandos * 4,
                           m_nCommandos * 4, 0, m_nCommandos * 2, "battle_shadow_Rectangle");
            int iconNum = 0;
            for (int i = 0; i < 4; i++)
            {
                if (iconNum >= m_nCommandos)
                    break;
                if (m_idIconTexture[i] < 0)
                    continue;
                rs->TextureSet(0, m_idIconTexture[i]);
                rs->DrawBuffer(m_idVBufCommandos, sizeof(BI_COLOR_VERTEX), m_idIBufCommandos, iconNum * 4, 4, 0, 2,
                               "battle_tex_col_Rectangle");
                iconNum++;
            }
            rs->TextureSet(0, m_idStateTexture);
            rs->DrawBuffer(m_idVBufHealth, sizeof(BI_COLOR_VERTEX), m_idIBufCommandos, 0, m_nCommandos * 4, 0,
                           m_nCommandos * 2, "battle_colorRectangle");
            rs->DrawBuffer(m_idVBufHealth, sizeof(BI_COLOR_VERTEX), m_idIBufHealth, m_nMaxCommandos * 4,
                           m_nCommandos * 7, 0, m_nCommandos * 4, "battle_colorRectangle");
            if (m_nGunCharge > 0)
            {
                rs->TextureSet(0, m_idGunChargeTexture);
                rs->DrawBuffer(m_idVBufHealth, sizeof(BI_COLOR_VERTEX), m_idIBufGunShoot, m_nMaxCommandos * (4 + 7),
                               m_nGunCharge * 4, 0, m_nGunCharge * 2, "battle_colorRectangle");
            }
        }
        if (m_bShowCommandos)
        {
            if (m_pCommandMenu != NULL && m_nShowCommand > 0)
                m_pCommandMenu->Draw();
            else if (m_pMessageIcons != null)
                m_pMessageIcons->Draw();
        }
    }
}
...
dword _cdecl BATTLE_LAND_INTERFACE::ProcessMessage(MESSAGE &message)
{
    ...
    case BI_MSG_ADD_NEWTEXTURE: {
        ...
        int vQ = message.Long();
        if (m_pCommandMenu)
            return m_pCommandMenu->AddTexture(param, hQ, vQ);
        return -1;
    }
    break;
    ...
    case MSG_BATTLE_LAND_SET_ICONTEX: {
        ...
        if (texIdx >= 0 && texIdx < 4)
        {
            int newTexID = -1;
            if (param[0])
                newTexID = rs->TextureCreate(param);
...
void BATTLE_LAND_INTERFACE::SetShowParameters()
{
    ...
    if (m_fAlarmBlindUpSpeed <= 0.f)
        m_fAlarmBlindUpSpeed = 1.f;
    else
        m_fAlarmBlindUpSpeed = .001f / m_fAlarmBlindUpSpeed;
    if (m_fAlarmBlindDownSpeed <= 0.f)
        m_fAlarmBlindDownSpeed = 1.f;
    else
        m_fAlarmBlindDownSpeed = .001f / m_fAlarmBlindDownSpeed;
    m_fCurAlarmBlind = 1.f;

    // Создадим буфера
    m_idVBufCommandos = rs->CreateVertexBufferMananged(
        BI_COLOR_VERTEX_FORMAT, m_nMaxCommandos * 4 * 2 * sizeof(BI_COLOR_VERTEX), D3DUSAGE_WRITEONLY);
    m_idIBufCommandos = rs->CreateIndexBufferManaged(m_nMaxCommandos * 6 * 2);
    m_idVBufHealth = rs->CreateVertexBufferMananged(
        BI_COLOR_VERTEX_FORMAT, m_nMaxCommandos * (4 + 7 + 4 * 4) * sizeof(BI_COLOR_VERTEX), D3DUSAGE_WRITEONLY);
    ...
    if (m_idVBufCommandos == -1 || m_idIBufCommandos == -1 || m_idVBufHealth == -1 || m_idIBufHealth == -1 ||
        m_idIBufGunShoot == -1)
    {
    ...
    for (i = 0; i < m_nMaxCommandos; i++)
    {
        ...
        SetRectanglePos(&pVCBuf[idx], float(lc + i * dc - shadowW), (float)(th - shadowTOP),
                        float(lc + i * dc + wc + shadowW), float(tc + hc + shadowH));
