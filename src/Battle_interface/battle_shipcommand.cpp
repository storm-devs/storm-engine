...
BIShipCommandList::BIShipCommandList(ENTITY_ID &eid, ATTRIBUTES *pA, VDX8RENDER *rs)
    : m_aTexture(_FL), m_aCommand(_FL), m_aUsedCommand(_FL), m_aChargeQuantity(_FL)
{
    m_idHostObj = eid;
    m_pARoot = pA;
    m_pRS = rs;
...
void BIShipCommandList::Draw()
{
    ...
    if (!m_NoteText.IsEmpty())
        m_pRS->ExtPrint(m_NoteFontID, m_NoteFontColor, 0, ALIGN_CENTER, true, m_NoteFontScale, 0, 0, m_NotePos.x,
                        m_NotePos.y, "%s", m_NoteText.GetBuffer());
}
...
{
    if (m_nSelectedCommandIndex >= m_aUsedCommand)
        return 0; // error!
    ...
    {
    case 0:
        api->Event("BI_LaunchCommand", "lsls", m_nCurrentCommandCharacterIndex, m_sCurrentCommandName.GetBuffer(),
                   nTargIndex, sLocName.GetBuffer());
        m_sCurrentCommandName = "";
        break;
    default:
...
void BIShipCommandList::Init()
{
    ...
    pAList = 0;
    if (m_pARoot)
        pAList = m_pARoot->GetAttributeClass("ShipIcon");
    if (pAList)
    {
        // get icon parameters
    ...
    if (pATextures)
    {
        ...
        for (n = 0; n < q; n++)
        {
            ...
            if (pA)
            {
                ...
                if (m_aTexture[i].nCols < 1)
                    m_aTexture[i].nCols = 1;
                if (m_aTexture[i].nRows < 1)
                    m_aTexture[i].nRows = 1;
    ...
    pACommands = 0;
    if (m_pARoot)
        pACommands = m_pARoot->GetAttributeClass("commands");
    if (pACommands)
    {
...
long BIShipCommandList::ShipAdding(bool allLabel, bool bMyShip, bool bEnemy, bool bNeutral, bool bFriend)
{
    ...
    if (m_pARoot && !m_sCurrentCommandName.IsEmpty())
    {
        ...
        if (pA)
            pA = pA->GetAttributeClass(m_sCurrentCommandName);
        if (pA)
            sqrRadius = pA->GetAttributeAsFloat("EffectRadius", -1.f);
    }
    ...
    for (; sd != null; sd = sd->next)
    {
        if (sd != selShip && sd != main_sd)
        {
                ...
                !bMyShip && bNeutral && !sd->isMyShip && sd->relation == BI_RELATION_NEUTRAL)
            {
                ...
                {
                    VDATA *pvdat =
                        api->Event("evntCheckEnableShip", "sl", m_sCurrentCommandName.GetBuffer(), sd->characterIndex);
                    if (pvdat != null && pvdat->GetLong() == 0)
                        continue;
                }
                retVal += AddToIconList(sd->textureNum, sd->pictureNum, sd->selectPictureNum, sd->characterIndex, null,
                                        -1, null, null);
            }
        }
    }
...
long BIShipCommandList::FortAdding(bool allLabel, bool bFriend, bool bNeutral, bool bEnemy)
{
    ...
    if (pA && !m_sCurrentCommandName.IsEmpty())
        pA = pA->GetAttributeClass(m_sCurrentCommandName);
    if (pA)
    ...
    do
    {
            ...
            bEnemy && pL->relation == BI_RELATION_ENEMY)
        {
            ...
            retVal += AddToIconList(pL->texIdx, pL->picIdx, pL->selPicIdx, pL->characterIndex, null, -1,
...
long BIShipCommandList::LandAdding(bool allLabel)
{
    ...
    do
    {
        ...
        if (pL->pA != null)
            pLocName = pL->pA->GetAttribute("name");
        retVal +=
            AddToIconList(pL->texIdx, pL->picIdx, pL->selPicIdx, pL->characterIndex, null, -1, pLocName, pL->pchr_note);
    } while ((pL = g_IslandDescr.GetNext()) != null);
    return retVal;
}
...
long BIShipCommandList::CommandAdding()
{
    ...
    for (long i = 0; i < attrQuant; i++)
    {
        ...
        retVal += AddToIconList(texNum, pictureNum, selPictureNum, -1, eventName, -1, null, pA->GetAttribute("note"));
    }
...
long BIShipCommandList::UserIconsAdding()
{
    ...
    for (long i = 0; i < attrQuant; i++)
    {
        ...
        retVal += AddToIconList(textureNum, pictureNum, selPictureNum, -1, null, i + 1, pA->GetAttribute("name"),
...
long BIShipCommandList::AbilityAdding()
{
    ...
    for (long i = 0; i < attrQuant; i++)
    {
        ...
        char *eventName = pA->GetAttribute("event");
        retVal +=
            AddToIconList(textureNum, pictureNum, selPictureNum, -1, eventName, -1, null, pA->GetAttribute("note"));
    }
...
long BIShipCommandList::AddCancelIcon()
{
    ...
    long selPictureNum = pA->GetAttributeAsDword("selPicNum", 0);
    long textureNum = pA->GetAttributeAsDword("texNum", -1);
    return AddToIconList(textureNum, pictureNum, selPictureNum, -1, pA->GetAttribute("event"), -1, null,
                         pA->GetAttribute("note"));
}

long BIShipCommandList::AddToIconList(long nTextureNum, long nNormPictureNum, long nSelPictureNum, long nCharacterIndex,
                                      const char *pcCommandName, long nTargetIndex, const char *pcLocName,
...
{
    if (nTextureNum < 0 || nTextureNum >= m_aTexture || nPictureNum < 0 ||
