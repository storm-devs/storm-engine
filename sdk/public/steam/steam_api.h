...
//
//	The following functions are part of abstracting API access to the steamclient.dll, but should only be used in very
//specific cases
//----------------------------------------------------------------------------------------------------------------------------------------------------------//
...
// returns the filename path of the current running Steam process, used if you need to load an explicit steam dll by
// name. DEPRECATED - implementation is Windows only, and the path returned is a UTF-8 string which must be converted to
// UTF-16 for use with Win32 APIs
...
// PFNPreMinidumpCallback m_pfnPreMinidumpCallback   -- optional callback which occurs just before a .dmp file is
// written during a crash.  Applications can hook this to allow adding additional information into the .dmp comment
// stream.
...
/*
    ...
    while ( SteamAPI_ManualDispatch_GetNextCallback( hSteamPipe, &callback ) )
    {
        ...
        if ( callback.m_iCallback == SteamAPICallCompleted_t::k_iCallback )
        {
            ...
            bool bFailed;
            if ( SteamAPI_ManualDispatch_GetAPICallResult( hSteamPipe, pCallCompleted->m_hAsyncCall, pTmpCallResult,
