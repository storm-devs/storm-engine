...
enum ELobbyType
{
    ...
    k_ELobbyTypePrivateUnique = 4, // private, unique and does not delete when empty - only one of these may exist per
...
enum ELobbyDistanceFilter
{
    ...
    k_ELobbyDistanceFilterFar,       // for games that don't have many latency requirements, will return lobbies about
                                     // half-way around the globe
    k_ELobbyDistanceFilterWorldwide, // no filtering, will match lobbies as far as India to NY (not recommended, expect
                                     // multiple seconds of latency between the clients)
};
...
class ISteamMatchmaking
{
  public:
    ...
    // use the CCallResult<> object in steam_api.h to match the SteamAPICall_t call result to a function in an object,
    // e.g.
    /*
    ...
    */
    ...
    // sets how many results to return, the lower the count the faster it is to download the lobby results & details to
    // the client
    ...
    // this is an asynchronous request
    // results will be returned by LobbyEnter_t callback & call result, check m_EChatRoomEnterResponse to see if was
    // successful lobby metadata is available to use immediately on this call completing
    ...
    // either the IP/Port or the steamID of the game server has to be valid, depending on how you want the clients to be
    // able to connect
    ...
    // only lobbies that are k_ELobbyTypePublic or k_ELobbyTypeInvisible, and are set to joinable, will be returned by
    // RequestLobbyList() calls
    ...
    // it is possible (bur rare) to join a lobby just as the owner is leaving, thus entering a lobby with self as the
    // owner
...
{
  public:
    // Request a new list of servers of a particular type.  These calls each correspond to one of the EMatchMakingType
    // values. Each call allocates a new asynchronous request object. Request object must be released by calling
    ...
    // progress. RefreshComplete callback is not posted when request is released.
    ...
    /* the filter operation codes that go in the key part of MatchMakingKeyValuePair_t should be one of these:
       ...
       of the specified strings.  The value field is a comma-delimited list of strings to match. "gamedatanor"
        ...
        "nor" !(x1 || x2 || ... || xn)
            ...
            "length" of its operands, not the number of immediate children.
                ...
                ( server.map == "cp_dustbowl" || ( server.gametags.contains("payload") &&
       !server.gametags.contains("payloadrace") ) ) "or", "4" "map", "cp_dustbowl" "and", "2" "gametagsand", "payload"
    ...
    */
    ...
    // Request the list of rules that the server is running (See ISteamGameServer::SetKeyValue() to set the rules server
    // side)
...
class ISteamGameSearch
{
  public:
    ...
    // all players in lobby enter the queue and await a SearchForGameNotificationCallback_t callback. fails if another
    // search is currently in progress if not the owner of the lobby or search already in progress this call fails
    ...
    // currently in progress periodic callbacks will be sent as queue time estimates change
    ...
    // multiple SearchForGameResultCallback_t will follow as players accept game until the host starts or cancels the
    // game
    ...
    // ( allows host to accept after all players confirm, some confirm, or none confirm. decision is entirely up to the
    // host )
...
class ISteamParties
{
  public:
    ...
    // Note: The user may already be in-flight to your game, so it's possible they will still connect and try to join
    // your party.
    ...
    // Utils
    virtual bool GetBeaconLocationData(SteamPartyBeaconLocation_t BeaconLocation, ESteamPartyBeaconLocationData eData,
...
//
//			if the user outside a game chooses to join, your game will be launched with the parameter "+connect_lobby
//<64-bit lobby id>", 			or with the callback GameLobbyJoinRequested_t if they're already in-game
//-----------------------------------------------------------------------------
...
struct LobbyChatUpdate_t
{
    ...
    uint64 m_ulSteamIDMakingChange; // Chat member who made the change (different from SteamIDUserChange if kicking,
                                    // muting, etc.) for example, if one user kicks another from the lobby, this will be
                                    // set to the id of the user who initiated the kick
...
struct LobbyKicked_t
{
    ...
    uint8 m_bKickedDueToDisconnect; // true if you were kicked from the lobby due to the user losing connection to Steam
...
struct LobbyCreated_t
{
    ...
    EResult m_eResult; // k_EResultOK - the lobby was successfully created
                       ...
                       // k_EResultAccessDenied - your game isn't set to allow lobbies, or your client does haven't
                       // rights to play the game k_EResultLimitExceeded - your game client has created too many lobbies
...
struct SearchForGameProgressCallback_t
{
    ...
    EResult m_eResult;  // if search has started this result will be k_EResultOK, any other value indicates search has
