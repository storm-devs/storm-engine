...
class ISteamUserStats
{
  public:
    ...
    // UserAchievementIconFetched_t callback which will notify you when the bits are ready. If the callback still
    // returns zero, then there is no image set for the specified achievement.
    ...
    // LeaderboardScoresDownloaded_t will contain a handle to pull the results from GetDownloadedLeaderboardEntries()
    // (below) You can ask for more entries than exist, and it will return as many as do exist.
    // k_ELeaderboardDataRequestGlobal requests rows in the leaderboard from the full table, with nRangeStart &
    // nRangeEnd in the range [1, TotalEntries] k_ELeaderboardDataRequestGlobalAroundUser requests rows around the
    // current user, nRangeStart being negate
    //   e.g. DownloadLeaderboardEntries( hLeaderboard, k_ELeaderboardDataRequestGlobalAroundUser, -3, 3 ) will return 7
    //   rows, 3 before the user, 3 after
    ...
    // k_ELeaderboardDataRequestUsers if a user doesn't have a leaderboard entry, they won't be included in the result
    ...
    // ISteamUserRemoteStorage::FileShare(). This call is asynchronous, with the result returned in LeaderboardUGCSet_t.
    ...
    // or another GetNextMostAchievedAchievementInfo call passing the iterator from the previous call. Returns -1 after
    // the last achievement has been iterated.
...
#ifdef _PS3
    // Call to kick off installation of the PS3 trophies. This call is asynchronous, and the results will be returned in
    // a PS3TrophiesInstalled_t callback.
    virtual bool InstallPS3Trophies() = 0;

    // Returns the amount of space required at boot to install trophies. This value can be used when comparing the
    // amount of space needed by the game to the available space value passed to the game at boot. The value is set
    // during InstallPS3Trophies().
    ...
    // Call to get the user's current stats data. You should retrieve this data after receiving successful
    // UserStatsReceived_t & UserStatsStored_t callbacks, and store the data with the user's save game data. You can
    // call this method with pvData = NULL and cubData = 0 to get the required buffer size.
...
//-----------------------------------------------------------------------------
// Purpose: call result for finding a leaderboard, returned as a result of FindOrCreateLeaderboard() or
// FindLeaderboard()
...
//-----------------------------------------------------------------------------
// Purpose: call result indicating scores for a leaderboard have been downloaded and are ready to be retrieved, returned
// as a result of DownloadLeaderboardEntries()
//			use CCallResult<> to map this async result to a member function
//-----------------------------------------------------------------------------
...
struct LeaderboardScoreUploaded_t
{
    ...
    int m_nGlobalRankPrevious; // the previous global rank of the user in this leaderboard; 0 if the user had no
...
struct UserAchievementIconFetched_t
{
    ...
    int m_nIconHandle; // Handle to the image, which can be used in SteamUtils()->GetImageRGBA(), 0 means no image is
...
struct PS3TrophiesInstalled_t
{
    ...
    uint64 m_nGameID;             // Game these stats are for
    EResult m_eResult;            // The result of the operation
    uint64 m_ulRequiredDiskSpace; // If m_eResult is k_EResultDiskFull, will contain the amount of space needed to
                                  // install trophies
};
