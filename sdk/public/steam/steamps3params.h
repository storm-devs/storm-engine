...
struct SteamPS3Params_t
{
    ...
    char m_rgchInstallationPath[STEAM_PS3_PATH_MAX]; // directory containing latest steam prx's and sdata. Can be read
                                                     // only (BDVD)
...
{
    bool m_bSingleAllocation; // If true, Steam will request one 6MB allocation and use the returned memory for all
                              // future allocations If false, Steam will make call malloc for each allocation
