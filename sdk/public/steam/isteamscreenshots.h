...
class ISteamScreenshots
{
  public:
    ...
    // wide and the same aspect ratio as the screenshot, otherwise a thumbnail will be generated if the user uploads the
    // screenshot.  The screenshots must be in either JPEG or TGA format. The return value is a handle that is valid for
    // the duration of the game process and can be used to apply tags. JPEG, TGA, and PNG formats are supported.
    ...
    // Toggles whether the overlay handles screenshots when the user presses the screenshot hotkey, or the game handles
    // them.  If the game is hooking screenshots, then the ScreenshotRequested_t callback will be sent if the user
    // presses the hotkey, and the game is expected to call WriteScreenshot or AddScreenshotToLibrary in response.
