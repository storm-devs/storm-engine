...
class ISteamFriends
{
  public:
    ...
    // this will only be known by the local user if steamIDFriend is in their friends list; on the same game server; in
    // a chat room or lobby; or in a small group with the local user
    virtual EPersonaState GetFriendPersonaState(CSteamID steamIDFriend) = 0;

    // returns the name another user - guaranteed to not be NULL.
    // same rules as GetFriendPersonaState() apply as to whether or not the user knowns the name of the other user
    // note that on first joining a lobby, chat room or game server the local user will not known the name of the other
    // users automatically; that information will arrive asyncronously
    //
    ...
    // Returns nickname the current user has set for the specified player. Returns NULL if the no nickname has been set
    // for that player. DEPRECATED: GetPersonaName follows the Steam nickname preferences, so apps shouldn't need to
    // care about nicknames explicitly.
    ...
    // "Achievements", "chatroomgroup/nnnn"
    ...
    //		"jointrade" - opens a window to a Steam Trading session that was started with the ISteamEconomy/StartTrade Web
    //API 		"stats" - opens the overlay web browser to the specified user's stats 		"achievements" - opens the overlay web
    //browser to the specified user's achievements 		"friendadd" - opens the overlay in minimal mode prompting the user to
    //add the target user as a friend 		"friendremove" - opens the overlay in minimal mode prompting the user to remove
    //the target friend 		"friendrequestaccept" - opens the overlay in minimal mode prompting the user to accept an
    //incoming friend invite 		"friendrequestignore" - opens the overlay in minimal mode prompting the user to ignore an
    ...
    virtual void ActivateGameOverlayToWebPage(
        const char *pchURL, EActivateGameOverlayToWebPageMode eMode = k_EActivateGameOverlayToWebPageMode_Default) = 0;
    ...
    // gets the small (32x32) avatar of the current user, which is a handle to be used in IClientUtils::GetImageRGBA(),
    // or 0 if none set
    virtual int GetSmallFriendAvatar(CSteamID steamIDFriend) = 0;

    // gets the medium (64x64) avatar of the current user, which is a handle to be used in IClientUtils::GetImageRGBA(),
    // or 0 if none set
    virtual int GetMediumFriendAvatar(CSteamID steamIDFriend) = 0;

    // gets the large (184x184) avatar of the current user, which is a handle to be used in
    // IClientUtils::GetImageRGBA(), or 0 if none set returns -1 if this image has yet to be loaded, in this case wait
    // for a AvatarImageLoaded_t callback and then call this again
    virtual int GetLargeFriendAvatar(CSteamID steamIDFriend) = 0;

    // requests information about a user - persona name & avatar
    // if bRequireNameOnly is set, then the avatar of a user isn't downloaded
    // - it's a lot slower to download avatars and churns the local cache, so if you don't need avatars, don't request
    // them if returns true, it means that data is being requested, and a PersonaStateChanged_t callback will be posted
    // when it's retrieved if returns false, it means that we already have all the details about that user, and
    ...
    //https://partner.steamgames.com/doc/api/ISteamFriends#richpresencelocalization 		"steam_player_group"		- When
    //set, indicates to the Steam client that the player is a member of a particular group. Players in the same group
    ...
    //Steam client may use this number to 									  display additional information about a group when all of the members are not
    //part of a user's friends list.
    ...
    // Rich invite support.
    // If the target accepts the invite, a GameRichPresenceJoinRequested_t callback is posted containing the connect
    // string. (Or you can configure yout game so that it is passed on the command line instead.  This is a deprecated
    ...
    // the behavior is somewhat sophisticated, because the user may or may not be already in the group chat from outside
    // the game or in the overlay use ActivateGameOverlayToUser( "chat", steamIDClan ) to open the in-game overlay
    // version of the chat
    ...
    virtual int GetClanChatMessage(CSteamID steamIDClanChat, int iMessage, void *prgchText, int cchTextMax,
                                   EChatEntryType *peChatEntryType, STEAM_OUT_STRUCT() CSteamID *psteamidChatter) = 0;
    ...
    // to your specified protocol (scheme) uris and instead dispatch a OverlayBrowserProtocolNavigation_t callback to
    // your game. ActivateGameOverlayToWebPage() must have been called with k_EActivateGameOverlayToWebPageMode_Modal
...
// these flags describe what the client has learned has changed recently, so on startup you'll see a name, avatar &
// relationship change for every friend
enum EPersonaChange
{
...
struct SetPersonaNameResponse_t
{
    ...
    bool m_bLocalSuccess; // true if name change was retained locally.  (We might not have been able to communicate with
                          // Steam)
...
//-----------------------------------------------------------------------------
// Purpose: Dispatched when an overlay browser instance is navigated to a protocol/scheme registered by
