...
class ISteamUtils
{
  public:
    ...
    // IP-to-location database) e.g "US" or "UK".
    ...
    virtual bool GetAPICallResult(SteamAPICall_t hSteamAPICall, void *pCallback, int cubCallback, int iCallbackExpected,
                                  bool *pbFailed) = 0;

    // Deprecated. Applications should use SteamAPI_RunCallbacks() instead. Game servers do not need to call this
    // function.
    ...
    //   k_ECheckFileSignatureNoSignaturesFoundForThisApp - This app has not been configured on the signing tab of the
    //   partner site to enable this function. k_ECheckFileSignatureNoSignaturesFoundForThisFile - This file is not
    //   listed on the signing tab for the partner site. k_ECheckFileSignatureFileNotFound - The file does not exist on
    //   disk. k_ECheckFileSignatureInvalidSignature - The file exists, and the signing tab has been set for this file,
    //   but the file is either not signed or the signature does not match. k_ECheckFileSignatureValidSignature - The
    ...
    virtual bool ShowGamepadTextInput(EGamepadTextInputMode eInputMode, EGamepadTextInputLineMode eLineInputMode,
                                      const char *pchDescription, uint32 unCharMax, const char *pchExistingText) = 0;
    ...
    // Filters the provided input message and places the filtered result into pchOutFilteredText, using legally required
    // filtering and additional filtering based on the context and user settings
    //   eContext is the type of content in the input string
    //   sourceSteamID is the Steam ID that is the source of the input string (e.g. the player with the name, or who
    //   said the chat text) pchInputText is the input string that should be filtered, which can be ASCII or UTF-8
...
struct GamepadTextInputDismissed_t
{
    ...
    bool m_bSubmitted; // true if user entered & accepted text (Call ISteamUtils::GetEnteredGamepadTextInput() for
                       // text), false if canceled input
