...
enum EUGCReadAction
{
    // Keeps the file handle open unless the last byte is read.  You can use this when reading large files (over 100MB)
    // in sequential chunks. If the last byte is read, this will behave the same as k_EUGCRead_Close.  Otherwise, it
    // behaves the same as k_EUGCRead_ContinueReading. This value maintains the same behavior as before the
...
class ISteamRemoteStorage
{
  public:
    ...
    // Gets the amount of data downloaded so far for a piece of content. pnBytesExpected can be 0 if function returns
    // false or if the transfer hasn't started yet, so be careful to check for that before dividing to get a percentage
    ...
    // Small files can be read all at once by calling this function with an offset of 0 and cubDataToRead equal to the
    // size of the file. Larger files can be read in chunks to reduce memory usage (since both sides of the IPC client
    // and the game itself must allocate enough memory for each chunk).  Once the last byte is read, the file is
    // implicitly closed and further calls to UGCRead will fail unless UGCDownload is called again. For especially large
    ...
    // The following functions are only necessary on the Playstation 3. On PC & Mac, the Steam client will handle these
    // operations for you On Playstation 3, the game controls which files are stored in the cloud, via FilePersist,
    // FileFetch, and FileForget.
    ...
    virtual SteamAPICall_t PublishWorkshopFile(const char *pchFile, const char *pchPreviewFile, AppId_t nConsumerAppId,
                                               const char *pchTitle, const char *pchDescription,
                                               ERemoteStoragePublishedFileVisibility eVisibility,
                                               SteamParamStringArray_t *pTags, EWorkshopFileType eWorkshopFileType) = 0;
    ...
    // A value of k_WorkshopForceLoadPublishedFileDetailsFromCache will use cached data if it exists, no matter how old
    // it is.
