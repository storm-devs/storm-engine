...
enum EResult
{
    ...
    k_EResultExternalAccountAlreadyLinked = 59, // External account (PSN, Facebook...) is already linked to some other
    ...
    k_EResultRateLimitExceeded = 84,      // Temporary rate limit exceeded, try again later, different from
...
enum EAuthSessionResponse
{
    ...
    k_EAuthSessionResponseLoggedInElseWhere = 4,  // The user account has logged in elsewhere and the session containing
...
enum EChatEntryType
{
    ...
    // k_EChatEntryTypeLobbyGameStart = 5,	// lobby game is starting (dead - listen for LobbyGameCreated_t callback
    // instead)
...
enum EMarketingMessageFlags
{
    ...
    // aggregate flags
    k_EMarketingMessageFlagsPlatformRestrictions = k_EMarketingMessageFlagsPlatformWindows |
...
{
    return eType == k_ELaunchOptionType_OpenVR || eType == k_ELaunchOptionType_OpenVROverlay ||
           eType == k_ELaunchOptionType_OculusVR || eType == k_ELaunchOptionType_OSVR;
}
...
{
    return eType == k_eEVRHMDType_HTC_Dev || eType == k_eEVRHMDType_HTC_VivePre || eType == k_eEVRHMDType_HTC_Vive ||
           eType == k_eEVRHMDType_HTC_Unknown || eType == k_eEVRHMDType_HTC_VivePro;
}
  ...
  private:
    ...
    union SteamID_t {
        struct SteamIDComponent_t
        {
            ...
            unsigned int
                m_EAccountType : 4; // type of account - can't show as EAccountType, due to signed / unsigned difference
...
//  just before minidump file is captured after a crash has occurred.  (Allows app to append additional comment data to
//  the dump, etc.)
//-----------------------------------------------------------------------------
