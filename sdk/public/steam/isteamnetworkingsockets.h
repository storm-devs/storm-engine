...
class ISteamNetworkingSockets
{
  public:
    ...
    virtual int ReceiveMessagesOnListenSocket(HSteamListenSocket hSocket, SteamNetworkingMessage_t **ppOutMessages,
                                              int nMaxMessages) = 0;
...
/// Certificate provision by the application.  (On Steam, Steam will handle all this automatically)
    ...
    virtual int FindRelayAuthTicketForServer(const SteamNetworkingIdentity &identityGameServer, int nVirtualPort,
                                             SteamDatagramRelayAuthTicket *pOutParsedTicket) = 0;
...
#error "Must define VALVE_CALLBACK_PACK_SMALL or VALVE_CALLBACK_PACK_LARGE"
    ...
    ///   k_ESteamNetworkingConnectionState_ProblemDetectedLocally. m_info.m_eEndReason and m_info.m_szEndDebug will
    ///   have for more details. NOTE: upon receiving this callback, you must still destroy the connection using
    ///   ISteamNetworkingSockets::CloseConnection to free up local resources.  (The details
    ///   passed to the function are not used in this case, since the connection is already closed.)
    ///
