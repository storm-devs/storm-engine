...
class ISteamHTMLSurface
{
  public:
    ...
    // tell the html control if it has key focus currently, controls showing the I-beam cursor in text controls amongst
    // other things
    ...
    virtual void SetCookie(const char *pchHostname, const char *pchKey, const char *pchValue, const char *pchPath = "/",
                           RTime32 nExpires = 0, bool bSecure = false, bool bHTTPOnly = false) = 0;

    // Zoom the current page by flZoom ( from 0.0 to 2.0, so to zoom to 120% use 1.2 ), zooming around point X,Y in the
    // page (use 0,0 if you don't care)
...
//-----------------------------------------------------------------------------
// Purpose: The browser has been requested to close due to user interaction (usually from a javascript window.close()
// call)
//-----------------------------------------------------------------------------
...
STEAM_CALLBACK_MEMBER(1, HHTMLBrowser,
                      unOldBrowserHandle) // the handle for the browser before the restart, if your handle was this then
                                          // switch to using unBrowserHandle for API calls
STEAM_CALLBACK_END(2)
