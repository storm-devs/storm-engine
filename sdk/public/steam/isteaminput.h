...
class ISteamInput
{
  public:
    ...
    // Xbox/Playstation/etc) via the Steam Input settings in the Steamworks site or users can opt-in in their controller
    // settings in Steam. handlesOut should point to a STEAM_INPUT_MAX_COUNT sized array of InputHandle_t handles
    ...
    // Lookup the handle for an Action Set. Best to do this once on startup, and store the handles for all future API
    // calls.
    ...
    // Lookup the handle for a digital action. Best to do this once on startup, and store the handles for all future API
    // calls.
    ...
    // originsOut. Use this to display the appropriate on-screen prompt for the action. originsOut should point to a
    // STEAM_INPUT_MAX_ORIGINS sized array of EInputActionOrigin handles. The EInputActionOrigin enum will get extended
    // as support for new controller controllers gets added to the Steam client and will exceed the values from this
    // header, please check bounds if you are using a look up table.
    virtual int GetDigitalActionOrigins(InputHandle_t inputHandle, InputActionSetHandle_t actionSetHandle,
                                        ...
                                        STEAM_OUT_ARRAY_COUNT(STEAM_INPUT_MAX_ORIGINS, Receives list of action origins)
                                            EInputActionOrigin *originsOut) = 0;

    // Lookup the handle for an analog action. Best to do this once on startup, and store the handles for all future API
    // calls.
    ...
    // Trigger a vibration event on supported controllers - Steam will translate these commands into haptic pulses for
    // Steam Controllers
    virtual void TriggerVibration(InputHandle_t inputHandle, unsigned short usLeftSpeed,
                                  unsigned short usRightSpeed) = 0;

    // Set the controller LED color on supported controllers. nFlags is a bitmask of values from ESteamInputLEDFlag - 0
    // will default to setting a color. Steam will handle the behavior on exit of your program so you don't need to try
    // restore the default as you are shutting down
    ...
    // TriggerVibration instead. Good uses for Haptic pulses include chimes, noises, or directional gameplay feedback
    // (taking damage, footstep locations, etc).
    virtual void TriggerHapticPulse(InputHandle_t inputHandle, ESteamControllerPad eTargetPad,
    ...
    // approximating rumble you may want to use TriggerVibration instead. nFlags is currently unused and reserved for
    // future use.
    virtual void TriggerRepeatedHapticPulse(InputHandle_t inputHandle, ESteamControllerPad eTargetPad,
    ...
    // functions to identify controllers presented to your game over Xinput. Returns 0 if the Xinput index isn't
    // associated with Steam Input
    virtual InputHandle_t GetControllerForGamepadIndex(int nIndex) = 0;

    // Returns the associated gamepad index for the specified controller, if emulating a gamepad or -1 if not associated
    // with an Xinput index
    ...
    // GetGlyphForActionOrigin to provide future proof glyphs for non-Steam Input API action games. Note - this only
    // translates the buttons directly and doesn't take into account any remapping a user has made in their
    // configuration
    ...
    // return k_EInputActionOrigin_None When a new input type is added you will be able to pass in
    // k_ESteamInputType_Unknown and the closest origin that your version of the SDK recognized will be returned ex: if
    // a Playstation 5 controller was released this function would return Playstation 4 origins.
    virtual EInputActionOrigin TranslateActionOrigin(ESteamInputType eDestinationInputType,
                                                     EInputActionOrigin eSourceOrigin) = 0;

    // Get the binding revision for a given device. Returns false if the handle was not valid or if a mapping is not yet
