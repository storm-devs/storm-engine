...
class ISteamHTTP
{
  public:
    ...
    // sending the request.  This is just so the caller can easily keep track of which callbacks go with which request
    // data.
    ...
    // the specified request to the tail of the queue.  Returns false on invalid handle, or if the request is not yet
    // sent.
    virtual bool DeferHTTPRequest(HTTPRequestHandle hRequest) = 0;

    // Prioritizes a request you have sent, the actual HTTP client code may have many requests queued, and this will
    // move the specified request to the head of the queue.  Returns false on invalid handle, or if the request is not
    // yet sent.
    ...
    // Gets the size of the body data from a HTTP response given a handle from HTTPRequestCompleted_t, will return false
    ...
    // BGetHTTPResponseBodySize first to find out the correct buffer size to use.
    ...
    // false if the handle is invalid or is to a non-streaming response (meaning it wasn't sent with
    // SendHTTPRequestAndStreamResponse), or if the buffer size and offset do not match the size and offset sent in
    // HTTPRequestDataReceived_t.
    virtual bool GetHTTPStreamingResponseBodyData(HTTPRequestHandle hRequest, uint32 cOffset, uint8 *pBodyDataBuffer,
                                                  uint32 unBufferSize) = 0;

    // Releases an HTTP response handle, should always be called to free resources after receiving a
    // HTTPRequestCompleted_t callback and finishing using the response.
    virtual bool ReleaseHTTPRequest(HTTPRequestHandle hRequest) = 0;

    // Gets progress on downloading the body for the request.  This will be zero unless a response header has already
    // been received which included a content-length field.  For responses that contain no content-length it will report
    ...
    // params have already been set for the request.  Setting this raw body makes it the only contents for the post, the
    // pchContentType parameter will set the content-type header for the request so the server may know how to interpret
    // the body.
    ...
    // bAllowResponsesToModify=true than any response to your requests using this cookie container may add new cookies
    // which may be transmitted with future requests.  If bAllowResponsesToModify=false than only cookies you explicitly
    // set will be sent.  This API is just for during process lifetime, after steam restarts no cookies are persisted
    // and you have no way to access the cookie container across repeat executions of your process.
    ...
    // Set the extra user agent info for a request, this doesn't clobber the normal user agent, it just adds the extra
    // info on the end
    ...
    // activity timeout which can bump everytime we get more data
