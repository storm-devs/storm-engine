...
bool NewNSGLContext(unsigned long *attribs, PseudoNSGLContextPtr nsglShareCtx, PseudoNSGLContextPtr *nsglCtxOut,
                    CGLContextObj *cglCtxOut);
...
{
    CGLMBuffer *m_buffer; // NULL allowed in which case m_offset is the full 32-bit pointer.. so you can draw from plain
                          // RAM if desired
...
class GLMContext
{
  public:
    ...
    void SetDrawingFBO(
        CGLMFBO *fbo); // as with samplers, the notion of the target FBO is latched til draw time and then checked
    ...
    void SetProgramParametersB(EGLMProgramType type, uint baseSlot, int *slotData,
  ...
  protected:
    ...
    // programs
    // void	BindProgramToCtx( EGLMProgramType type, CGLMProgram *prog );						// will set program mode enable
    // appropriately
    ...
    CGLContextObj m_ctx;
    bool
        m_oneCtxEnable; // true if we use the window's context directly instead of making a second one shared against it
    ...
    GLMVertexAttributeDesc
        m_lastKnownVertexAttribs[kGLMVertexAttributeIndexMax]; // tracked per attrib for dupe-set-absorb
...
class GLMTester
{
  public:
    ...
    void InternalError(int errcode,
                       const char *comment); // if errcode!=0, obey m_params setting for console / debugger response
