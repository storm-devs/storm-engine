...
class CGLMShaderPair // a container for a linked GLSL shader pair, and metadata obtained post-link
{
  ...
  public:
    ...
    bool m_samplersFixed; // set on first draw (can't write the uniforms until the program is in use, and we don't want
                          // to mess with cur program inside cglmprogram)
...
class CGLMShaderPairCache // cache for linked GLSL shader pairs
{
  ...
  protected:
    ...
    void HashRowProbe(CGLMPairCacheEntry *row, CGLMProgram *vp, CGLMProgram *fp, uint extraKeyBits, int *hitwayOut,
                      int *emptywayOut, int *oldestwayOut);
    //===============================
