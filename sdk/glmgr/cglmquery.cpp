...
void CGLMQuery::Start(void) // "start counting"
{
    ...
    case EOcclusion: {
        ...
        else
        {
            ...
            {
                const char *decodedStr = GLMDecode(eGL_ERROR, errorcode);
                printf("\nCGLMQuery::Start(OQ) saw %s error (%d) from glBeginQueryARB (GL_SAMPLES_PASSED_ARB) name=%d",
                       decodedStr, errorcode, m_name);
            }
        }
    }
    break;
    ...
    case EFence: {
        ...
        {
            const char *decodedStr = GLMDecode(eGL_ERROR, errorcode);
            printf("\nCGLMQuery::Start(fence) saw %s error (%d) from glSetFenceAPPLE name=%d", decodedStr, errorcode,
                   m_name);
        }
...
void CGLMQuery::Stop(void) // "stop counting"
{
    ...
    case EOcclusion: {
        ...
        else
        {
            ...
            {
                const char *decodedStr = GLMDecode(eGL_ERROR, errorcode);
                printf("\nCGLMQuery::Stop(OQ) saw %s error (%d) from glEndQueryARB( GL_SAMPLES_PASSED_ARB ) name=%d",
                       decodedStr, errorcode, m_name);
            }
        }
    }
    ...
    if (!m_done) // you can ask more than once, but we only check until it comes back as done.
    {
        ...
        case EOcclusion: // just test the fence that was set after the query begin
        {
            ...
            else
            {
                ...
                {
                    const char *decodedStr = GLMDecode(eGL_ERROR, errorcode);
                    printf("\nCGLMQuery::IsDone saw %s error (%d) from glGetQueryObjectivARB(a2) name=%d", decodedStr,
                           errorcode, m_name);
                }
        ...
        case EFence: {
            ...
            {
                const char *decodedStr = GLMDecode(eGL_ERROR, errorcode);
                printf("\nCGLMQuery::IsDone saw %s error (%d) from glTestFenceAPPLE(b) name=%d", decodedStr, errorcode,
                       m_name);
            }
            ...
            if (m_done)
            {
                ...
                {
                    const char *decodedStr = GLMDecode(eGL_ERROR, errorcode);
                    printf("\nCGLMQuery::IsDone saw %s error (%d) from glFinishFenceAPPLE(b) name=%d", decodedStr,
                           errorcode, m_name);
                }
            }
        }
...
void CGLMQuery::Complete(uint *result)
{
    ...
    case EOcclusion: {
        ...
        else
        {
            ...
            {
                const char *decodedStr = GLMDecode(eGL_ERROR, errorcode);
                printf("\nCGLMQuery::Complete saw %s error (%d) from glGetQueryObjectivARB "
                       "GL_QUERY_RESULT_AVAILABLE_ARB name=%d",
                       decodedStr, errorcode, m_name);
            ...
            else
            {
                if (!available)
                {
                    ...
                    {
                        glFlush(); // ISTR some deadlock cases on pre-SLGU drivers if you didn't do this to kick the
                                   // queue along..
                    }
                }
                ...
                {
                    const char *decodedStr = GLMDecode(eGL_ERROR, errorcode);
                    printf("\nCGLMQuery::Complete saw %s error (%d) from glGetQueryObjectivARB GL_QUERY_RESULT_ARB "
                           "name=%d",
                           decodedStr, errorcode, m_name);
    ...
    case EFence: {
        if (!m_done)
        {
            ...
            {
                const char *decodedStr = GLMDecode(eGL_ERROR, errorcode);
                printf("\nCGLMQuery::Complete saw %s error (%d) from glFinishFenceAPPLE (EFence) name=%d", decodedStr,
                       errorcode, m_name);
            }
