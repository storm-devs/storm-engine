        ...
        if (isNative != 1)
        {
            ...
            for (GLMShaderLimitDesc *desc = g_glmShaderLimitDescs; desc->m_valueEnum != 0; desc++)
            {
                if (desc->m_flags & checkmask)
                {
                    ...
                    if (value > limit)
                    {
                        GLMPRINTF(("-D- Invalid %s program: program has %d %s; limit is %d", targnames[m_type], value,
                                   desc->m_debugName, limit));
                    }
                }
            }
        }
...
bool CGLMShaderPair::SetProgramPair(CGLMProgram *vp, CGLMProgram *fp)
{
    ...
    if (vpgood && fpgood)
    {
        ...
        else
        {
            ...
            GLMPRINTF(("-D- ----- GLSL vertex program selected: %08x (handle %08x)", vp,
                       vp->m_descs[kGLMGLSL].m_object.glsl));
            GLMPRINTTEXT((vtemp + vp->m_descs[kGLMGLSL].m_textOffset, eDebugDump, GLMPRINTTEXT_NUMBEREDLINES));

            GLMPRINTF(("-D- ----- GLSL fragment program selected: %08x (handle %08x)", fp,
                       vp->m_descs[kGLMGLSL].m_object.glsl));
...
CGLMShaderPair *CGLMShaderPairCache::SelectShaderPair(CGLMProgram *vp, CGLMProgram *fp, uint extraKeyBits)
{
    ...
    else
    {
        ...
        else
        {
            ...
            ///////////////////////FIXME may need to do a shoot-down if the pair being evicted is currently active in
            ///the context
...
void CGLMShaderPairCache::HashRowProbe(CGLMPairCacheEntry *row, CGLMProgram *vp, CGLMProgram *fp, uint extraKeyBits,
                                       int *hitwayOut, int *emptywayOut, int *oldestwayOut)
{
