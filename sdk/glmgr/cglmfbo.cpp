...
void CGLMFBO::TexAttach(GLMFBOTexAttachParams *params, EGLMFBOAttachment attachIndex, GLenum fboBindPoint)
{
    ...
    {
    case GL_TEXTURE_2D: {
        // we will attach the underlying RBO on a multisampled tex, iff the tex hasone, **and** we're not being asked to
        // attach it to the read buffer. if we get a req to attach an MSAA tex to the read buffer, chances are it's
        // BlitTex calling, andit has already resolved the tex, so in those cases you really do want to attach the
        // texture and not the RBO to the FBO in question.
        ...
        if (useRBO)
        {
            ...
            if (attachIndexGL == GL_DEPTH_STENCIL_ATTACHMENT_EXT)
            {
                ...
                glFramebufferRenderbufferEXT(fboBindPoint, GL_DEPTH_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT,
                                             tex->m_rboName);
                GLMCheckError();

                glFramebufferRenderbufferEXT(fboBindPoint, GL_STENCIL_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT,
                                             tex->m_rboName);
                GLMCheckError();
        ...
        else
        {
            ...
            if (attachIndexGL == GL_DEPTH_STENCIL_ATTACHMENT_EXT)
            {
                ...
                glFramebufferTexture2DEXT(fboBindPoint, GL_STENCIL_ATTACHMENT_EXT, target, tex->m_texName,
                                          params->m_mip);
                GLMCheckError();
            }
            else
