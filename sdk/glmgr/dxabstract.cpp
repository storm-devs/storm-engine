...
#if DX9MODE
extern CGameEngineGL
    *g_engine; // so dxabstract (which is C++) can call up to the game engine ObjC object and ask for things..
#endif
                                        ...
                                        char *debugLabel)
{
    ...
    if (Usage & D3DUSAGE_RENDERTARGET)
    {
        ...
        key.m_texFlags |=
            kGLMTexSRGB; // this catches callers of CreateTexture who set the "renderable" option - they get an SRGB tex
    ...
    GLMPRINTF(("-A- IDirect3DDevice9::CreateTexture created '%s' @ %08x (GLM %08x) %s", tex->m_layout->m_layoutSummary,
               dxtex, tex, debugLabel ? debugLabel : ""));
...
HRESULT IDirect3DDevice9::CreateVolumeTexture(UINT Width, UINT Height, UINT Depth, UINT Levels, DWORD Usage,
                                              ...
                                              VD3DHANDLE *pSharedHandle, char *debugLabel)
{
...
HRESULT IDirect3D9::GetDeviceCaps(UINT Adapter, D3DDEVTYPE DeviceType, D3DCAPS9 *pCaps)
{
    ...
    pCaps->PrimitiveMiscCaps =
        0; // only the HDR setup looks at this for D3DPMISCCAPS_SEPARATEALPHABLEND.
...
HRESULT IDirect3D9::GetAdapterIdentifier(UINT Adapter, DWORD Flags, D3DADAPTER_IDENTIFIER9 *pIdentifier)
{
    ...
    sprintf(pIdentifier->Driver, "OpenGL %s (%08x)", GLMDecode(eGL_RENDERER, glmRendererInfo.m_rendererID & 0x00FFFF00),
            glmRendererInfo.m_rendererID);

    sprintf(pIdentifier->Description, "%s - %dx%d - %dMB VRAM",
            GLMDecode(eGL_RENDERER, glmRendererInfo.m_rendererID & 0x00FFFF00), glmDisplayInfo.m_displayPixelWidth,
            glmDisplayInfo.m_displayPixelHeight, glmRendererInfo.m_vidMemory >> 20);
    ...
    case D3DFMT_X8R8G8B8:
        ...
        case D3DRTYPE_TEXTURE:
            ...
            case D3DFMT_R5G6B5:
                ...
                // InitializeColorInformation... which is going to try all 8 combinations of (vertex texturable / render
                // targetable / filterable ) on every image format it knows.
            ...
            case D3DFMT_X8R8G8B8:
                ...
                // one and two channel textures... we'll have to fake these as four channel tex if we want to support
                // them
            case D3DFMT_L8:
                legalUsage = D3DUSAGE_DYNAMIC | D3DUSAGE_AUTOGENMIPMAP | D3DUSAGE_QUERY_FILTER;
                break;
            ...
            {
                DWORD unsatBits =
                    Usage & (~legalUsage); // clear the bits of the req that were legal, leaving the illegal ones
        ...
        case D3DRTYPE_SURFACE:
            ...
            case D3DFMT_D24S8:
                result = S_OK;
                break;
                //** IDirect3D9::CheckDeviceFormat  adapter=0, DeviceType=   1:D3DDEVTYPE_HAL, AdapterFormat=
                //5:D3DFMT_X8R8G8B8, RType=   1:D3DRTYPE_SURFACE, CheckFormat=434f5441:UNKNOWN
                //** IDirect3D9::CheckDeviceFormat  adapter=0, DeviceType=   1:D3DDEVTYPE_HAL, AdapterFormat=
                //5:D3DFMT_X8R8G8B8, RType=   1:D3DRTYPE_SURFACE, CheckFormat=41415353:UNKNOWN
                //** IDirect3D9::CheckDeviceFormat  adapter=0, DeviceType=   1:D3DDEVTYPE_HAL, AdapterFormat=
                //5:D3DFMT_X8R8G8B8, RType=   1:D3DRTYPE_SURFACE, CheckFormat=434f5441:UNKNOWN
                //** IDirect3D9::CheckDeviceFormat  adapter=0, DeviceType=   1:D3DDEVTYPE_HAL, AdapterFormat=
                //5:D3DFMT_X8R8G8B8, RType=   1:D3DRTYPE_SURFACE, CheckFormat=41415353:UNKNOWN
...
HRESULT IDirect3DQuery9::GetData(void *pData, DWORD dwSize, DWORD dwGetDataFlags)
{
    ...
    else
    {
        ...
        case D3DQUERYTYPE_EVENT: {
            // expectation - caller already did an issue end (for fence => start) but has not done anything that would
            // call Stop. that's ok because Stop is a no-op for fences.
            if (flush && (!m_ctx->Caps().m_hasPerfPackage1))
            {
                glFlush();
            }
...
HRESULT IDirect3DVertexBuffer9::Lock(UINT OffsetToLock, UINT SizeToLock, void **ppbData, DWORD Flags)
{
    ...
    GLMPRINTF(("-X- IDirect3DDevice9::Lock on D3D buf %p (GL name %d) offset %d, size %d => address %p", this,
               this->m_vtxBuffer->m_name, OffsetToLock, SizeToLock, *ppbData));
    return S_OK;
}
...
HRESULT IDirect3DDevice9::Create(IDirect3DDevice9Params *params)
{
    ...
    GLScissorBox_t defScissorBox = {0, 0, m_params.m_presentationParameters.BackBufferWidth,
                                    m_params.m_presentationParameters.BackBufferHeight};
    GLViewportBox_t defViewportBox = {0, 0, m_params.m_presentationParameters.BackBufferWidth,
                                      m_params.m_presentationParameters.BackBufferHeight};
    ...
    gl.m_stateDirtyMask = (1 << kGLScissorEnable) | (1 << kGLScissorBox) | (1 << kGLViewportBox) |
                          (1 << kGLViewportDepthRange) | (1 << kGLCullFaceEnable) | (1 << kGLCullFrontFace);

    GLMPRINTF(("<-X- IDirect3DDevice9::Create complete"));

    // so GetClientRect can return sane answers
    uint width, height;
    g_engine->RenderedSize(m_params.m_presentationParameters.BackBufferWidth,
                           m_params.m_presentationParameters.BackBufferHeight, true); // true = set
...
HRESULT IDirect3DDevice9::Reset(D3DPRESENT_PARAMETERS *pPresentationParameters)
{
    ...
    {
        ULONG refc =
            m_defaultDepthStencilSurface->Release(0, "IDirect3DDevice9::Reset public release depthstencil surface");
        Assert(!refc);
        m_defaultDepthStencilSurface = NULL;
    }
    ...
    // so GetClientRect can return sane answers
    uint width, height;
    g_engine->RenderedSize(pPresentationParameters->BackBufferWidth, pPresentationParameters->BackBufferHeight,
                           true); // true = set
...
HRESULT IDirect3DDevice9::SetRenderTarget(DWORD RenderTargetIndex, IDirect3DSurface9 *pRenderTarget)
{
    ...
    if (m_rtSurfaces[RenderTargetIndex])
    {
        m_rtSurfaces[RenderTargetIndex]->Release(
            1, "-A  SetRenderTarget private release"); // note this is the private refcount being lowered
    }
...
HRESULT IDirect3DDevice9::CreatePixelShader(CONST DWORD *pFunction, IDirect3DPixelShader9 **ppShader,
                                            const char *pShaderName, char *debugLabel)
{
    ...
    *ppShader = NULL;
    ...
    bool passthrough = (memcmp(pFunction, "//GLSLfp", 8) ==
                        0); // if we were given GLSL text, pass it through instead of treating it as bytecodes..
    ...
    else
    {
        ...
        else
        {
            if (g_useASMTranslations)
            {
                ...
                g_D3DToOpenGLTranslatorASM.TranslateShader((uint32 *)pFunction, &tempbuf, &bVertexShader,
                                                           D3DToGL_OptionUseEnvParams, nShadowDepthSampler, 0,
                                                           debugLabel);
            ...
            if (g_useGLSLTranslations)
            {
                ...
                g_D3DToOpenGLTranslatorGLSL.TranslateShader((uint32 *)pFunction, &tempbuf, &bVertexShader,
                                                            glslPixelShaderOptions, nShadowDepthSampler, nCentroidMask,
                                                            debugLabel);
        ...
        else
        {
            ...
            //------ find the frag program metadata and extract it.. note this takes place even for passthrough shaders,
            //so they need to supply the needed string too
                                             ...
                                             const char *pShaderName, char *debugLabel)
{
    ...
    *ppShader = NULL;
    ...
    bool passthrough = (memcmp(pFunction, "//GLSLvp", 8) ==
                        0); // if we were given GLSL text, pass it through instead of treating it as bytecodes..
    ...
    else
    {
        ...
        else
        {
            if (g_useASMTranslations)
            {
                ...
                g_D3DToOpenGLTranslatorASM.TranslateShader((uint32 *)pFunction, &tempbuf, &bVertexShader,
                                                           asmTransOptions, -1, 0, debugLabel);
            ...
            if (g_useGLSLTranslations)
            {
                ...
                uint glslVertexShaderOptions =
                    D3DToGL_OptionGLSL | D3DToGL_OptionUseEnvParams | D3DToGL_OptionDoFixupZ | D3DToGL_OptionDoFixupY;
                ...
                g_D3DToOpenGLTranslatorGLSL.TranslateShader((uint32 *)pFunction, &tempbuf, &bVertexShader,
                                                            glslVertexShaderOptions, -1, nCentroidMask, debugLabel);
        ...
        else
        {
            ...
            // find the highwater mark.. note this takes place even for passthrough shaders, so they need to supply the
            // needed string too
...
HRESULT IDirect3DDevice9::SetVertexShaderConstantF(UINT StartRegister, CONST float *pConstantData,
...
HRESULT IDirect3DDevice9::SetVertexShaderConstantB(UINT StartRegister, CONST BOOL *pConstantData,
...
HRESULT IDirect3DDevice9::SetVertexShaderConstantI(UINT StartRegister, CONST int *pConstantData,
    ...
    for (const D3DVERTEXELEMENT9 *src = pVertexElements; (src->Stream != 0xFF); src++)
    {
        ...
        default:
            ...
            /*
               ...
               (v[0]/32767.0,v[1]/32767.0,v[2]/32767.0,v[3]/32767.0) D3DDECLTYPE_USHORT2N  = 11,  // 2D unsigned short
               normalized (v[0]/65535.0,v[1]/65535.0,0,1) D3DDECLTYPE_USHORT4N  = 12,  // 4D unsigned short normalized
               (v[0]/65535.0,v[1]/65535.0,v[2]/65535.0,v[3]/65535.0) D3DDECLTYPE_UDEC3     = 13,  // 3D unsigned 10 10
               10 format expanded to (value, value, value, 1) D3DDECLTYPE_DEC3N     = 14,  // 3D signed 10 10 10 format
               normalized and expanded to (v[0]/511.0, v[1]/511.0, v[2]/511.0, 1) D3DDECLTYPE_FLOAT16_2 = 15,  // Two
               16-bit floating point values, expanded to (value, value, 0, 1) D3DDECLTYPE_FLOAT16_4 = 16,  // Four
               16-bit floating point values D3DDECLTYPE_UNUSED    = 17,  // When the type field in a decl is unused. }
               D3DDECLTYPE;
            */
        }
...
{
    // we just latch it.  At draw time we combine the current vertex decl with the current stream set and generate a
    // vertex setup for GLM. GLM can see what the differences are and act accordingly to adjust vert attrib bindings.
...
D3D_RSINFO g_D3DRS_INFO_packed[] = {
    ...
    D3D_RSI(3, D3DRS_COLORWRITEENABLE,
            D3DCOLORWRITEENABLE_RED | D3DCOLORWRITEENABLE_GREEN | D3DCOLORWRITEENABLE_BLUE |
                D3DCOLORWRITEENABLE_ALPHA), // see transitiontable.cpp "APPLY_RENDER_STATE_FUNC( D3DRS_COLORWRITEENABLE,
                                            // ColorWriteEnable )"
...
static GLenum D3DBlendFactorToGL(DWORD equation)
{
    ...
    /*
        ...
        case	D3DBLEND_BOTHSRCALPHA	: Assert(0); return GL_ZERO;		// Obsolete. Starting with DirectX 6, you
    can achieve the same effect by setting the source and destination blend factors to D3DBLEND_SRCALPHA and
    ...
    destination blend selection is overridden. This blend mode is supported only for the D3DRS_SRCBLEND render state.
        case	D3DBLEND_BLENDFACTOR	: Assert(0); return GL_ZERO;		// Constant color blending factor used by
    the frame-buffer blender. This blend mode is supported only if D3DPBLENDCAPS_BLENDFACTOR is set in the SrcBlendCaps
    or DestBlendCaps members of D3DCAPS9.

    dxabstract.h has not heard of these, so let them hit the debugger if they come through
        case	D3DBLEND_INVBLENDFACTOR:	//Inverted constant color-blending factor used by the frame-buffer blender.
    This blend mode is supported only if the D3DPBLENDCAPS_BLENDFACTOR bit is set in the SrcBlendCaps or DestBlendCaps
    members of D3DCAPS9. case	D3DBLEND_SRCCOLOR2:		// Blend factor is (PSOutColor[1]r, PSOutColor[1]g,
    PSOutColor[1]b, not used).	This flag is available in Direct3D 9Ex only. case	D3DBLEND_INVSRCCOLOR2:	// Blend
    factor is (1 - PSOutColor[1]r, 1 - PSOutColor[1]g, 1 - PSOutColor[1]b, not used)). This flag is available in
    Direct3D 9Ex only.
    */
    default:
        Debugger();
...
static GLenum D3DBlendOperationToGL(DWORD operation)
{
    ...
    case D3DBLENDOP_ADD:
        ...
        /* not covered by dxabstract.h..
            case	D3DBLENDOP_SUBTRACT		: return GL_FUNC_SUBTRACT;			// The result is the destination
           subtracted from to the source. Result = Source - Destination case	D3DBLENDOP_REVSUBTRACT	: return
           GL_FUNC_REVERSE_SUBTRACT;	// The result is the source subtracted from the destination. Result = Destination -
           Source
            case	D3DBLENDOP_MIN			: return GL_MIN;					// The result is the minimum of the
           source and destination. Result = MIN(Source, Destination) case	D3DBLENDOP_MAX			: return GL_MAX;
...
HRESULT IDirect3DDevice9::SetRenderState(D3DRENDERSTATETYPE State, DWORD Value)
{
    ...
    else
    {
        ...
        {
            GLMPRINTF(
                ("-X- IDirect3DDevice9::SetRenderState: set %s(%d) to %d(0x%08x) ( class %d, defval is %d(0x%08x) )",
        ...
        case 3:
            ...
            case D3DRS_STENCILZFAIL: // GLStencilOp_t		"what do you do if stencil test passes *but* depth test
                                     // fails, if depth test happened"
            {
                GLenum stencilop = D3DStencilOpToGL(Value);
...
HRESULT IDirect3DDevice9::SetSamplerState(DWORD Sampler, D3DSAMPLERSTATETYPE Type, DWORD Value)
{
    ...
    switch (Type)
    {
        ...
        // D3DTADDRESS_MIRROR	Similar to D3DTADDRESS_WRAP, except that the texture is flipped at every integer
        // junction. D3DTADDRESS_CLAMP	Texture coordinates outside the range [0.0, 1.0] are set to the texture color at
        // 0.0 or 1.0, respectively. D3DTADDRESS_BORDER	Texture coordinates outside the range [0.0, 1.0] are set to the
        // border color. D3DTADDRESS_MIRRORONCE Similar to D3DTADDRESS_MIRROR and D3DTADDRESS_CLAMP.
...
// 3 D3DTADDRESS_CLAMP	Texture coordinates outside the range [0.0, 1.0] are set to the texture color at 0.0 or 1.0,
// respectively. 4 D3DTADDRESS_BORDER	Texture coordinates outside the range [0.0, 1.0] are set to the border color.
...
static GLenum dxtogl_magFilter[4] = // indexed by _D3DTEXTUREFILTERTYPE
    {
        GL_NEAREST, // D3DTEXF_NONE not applicable to mag filter but we handle it like POINT (mat_showmiplevels hits
                    // this)
...
static GLenum
    dxtogl_minFilter[4][4] = // indexed by _D3DTEXTUREFILTERTYPE on both axes: [row is min filter][col is mip filter].
    {
        //  mip filter ---------------> D3DTEXF_NONE	D3DTEXF_POINT				D3DTEXF_LINEAR
        //  (D3DTEXF_ANISOTROPIC not applicable to mip filter)
...
HRESULT IDirect3DDevice9::FlushSamplers(uint mask)
{
    ...
    uint activeSamplerMask = m_pixelShader ? m_pixelShader->m_pixSamplerMask
                                           : 0; // if no pixel shader bound at time of draw, act like it references no
                                                // samplers (and avoid an access violation while yer at it)
    ...
    for (int index = 0; (index < 16) && (samplerHitMask != 0); index++)
    {
        ...
        if ((samplerHitMask & mask) && (m_textures[index] != NULL))
        {
            ...
            */
            ...
            glsamp->m_minMipLevel = dxsamp->m_maxMipLevel; // it says gl_minMipLevel because we're setting GL's
                                                           // "GL_TEXTURE_MIN_LOD" aka d3d's "maximum mip size index".
            ...
            glsamp->m_maxMipLevel =
                texMipCount -
                1; // d3d has no value for constraining how small we can sample.
            ...
            Assert(((dxsamp->m_minFilter == D3DTEXF_ANISOTROPIC) && (dxsamp->m_maxAniso >= 1)) ||
                   ((dxsamp->m_minFilter < D3DTEXF_ANISOTROPIC) && (dxsamp->m_maxAniso >= 1)));
                ...
                if (mismatch)
                {
                    ...
                    if (srgbCapableTex && !renderableTex)
                    {
                        ...
                        if (print_it)
                        {
                            ...
                            printf("\n");
                            printf(formatStr, index, texname, m_textures[index],
                                   m_textures[index]->m_tex->m_layout->m_layoutSummary, (int)glsamp->m_srgb,
                                   m_textures[index]->m_srgbFlipCount);
                    ...
                    {
                        // GLMPRINTF(("-Z- srgb sampling conflict: NOT fixing tex %08x [%s] (srgb req: %d) because
                        // (tex-srgb-capable=%d  tex-renderable=%d)", m_textures[index],
                        // m_textures[index]->m_tex->m_layout->m_layoutSummary, (int)glsamp->m_srgb, (int)srgbCapableTex,
                        // (int)renderableTex ));
...
//	StartIndex
//	[in] Index of the first index to use when accesssing the vertex buffer. Beginning at StartIndex to index vertices
                                               ...
                                               UINT NumVertices, UINT startIndex, UINT primCount)
{
    ...
    case D3DPT_LINELIST:
        ...
        m_ctx->DrawRangeElements((GLenum)GL_LINES, (GLuint)MinVertexIndex, (GLuint)(MinVertexIndex + NumVertices),
                                 ...
                                 (const GLvoid *)(startIndex * sizeof(short)));
        break;
...
HRESULT IDirect3DDevice9::SetScissorRect(CONST RECT *pRect)
{
    ...
    GLScissorBox_t newScissorBox = {(GLint)pRect->left, (GLint)pRect->top, (GLint)(pRect->right - pRect->left),
                                    (GLint)(pRect->bottom - pRect->top)};
