...
extern CGameEngineGL
    *g_engine; // so glmgr (which is C++) can call up to the game engine ObjC object and ask for things..
...
void DefaultSamplingParams(GLMTexSamplingParams *samp, GLMTexLayoutKey *key)
{
    ...
    // ** none of these really matter all that much because the first time we go to render, the d3d sampler state will
    // be consulted and applied directly to the tex object without regard to any previous values..
    ...
    samp->m_minMipLevel = 0;  // this drives GL_TEXTURE_MIN_LOD - i.e. lowest MIP selection index clamp (largest size),
                              // not "slice defined" boundary
    samp->m_maxMipLevel = 16; // this drives GL_TEXTURE_MAX_LOD - i.e. highest MIP selection clamp (smallest size), not
                              // "slice defined" boundary
...
void GLMContext::DelTex(CGLMTex *tex)
{
    ...
    if (tex->m_rtAttachCount != 0)
    {
        ...
        GLMPRINTF(("-D- ################## Leaking tex %08x [ %s ] - was attached for drawing at time of delete", tex,
...
// ConVar	gl_minify_resolve_mode("gl_minify_resolve_mode", "1" );		// if scaled resolve available, for downscaled
// resolve blits only (i.e. internal blits)
int gl_minify_resolve_mode = 1;

// ConVar	gl_magnify_resolve_mode("gl_magnify_resolve_mode", "2" );	// if scaled resolve available, for upscaled resolve
// blits only
                       ...
                       int dstFace, int dstMip, uint filter)
{
        ...
        if (blitResolves && blitScales)
        {
            ...
            else
            {
                bool blitScalesDown = ((srcRect->xmax - srcRect->xmin) > (dstRect->xmax - dstRect->xmin)) ||
                                      ((srcRect->ymax - srcRect->ymin) > (dstRect->ymax - dstRect->ymin));
                int mode =
                    (blitScalesDown) ? gl_minify_resolve_mode /*.GetInt()*/ : gl_magnify_resolve_mode /*.GetInt()*/;
    ...
    if (blitTwoStep)
    {
        ...
        glBlitFramebufferEXT(0, 0, srcTex->m_layout->m_key.m_xSize, srcTex->m_layout->m_key.m_ySize, 0, 0,
    ...
    if (yflip)
    {
        glBlitFramebufferEXT(srcRect->xmin, srcRect->ymin, srcRect->xmax, srcRect->ymax, dstRect->xmin, dstRect->ymax,
                             dstRect->xmax, dstRect->ymin, // note dest Y's are flipped
                             blitMask, filter);
    }
    else
    {
        glBlitFramebufferEXT(srcRect->xmin, srcRect->ymin, srcRect->xmax, srcRect->ymax, dstRect->xmin, dstRect->ymin,
                             dstRect->xmax, dstRect->ymax, blitMask, filter);
    }
    GLMCheckError();
...
void GLMContext::ResolveTex(CGLMTex *tex, bool forceDirty)
{
    ...
    if ((tex->m_rboName) && ((tex->m_rboDirty) || forceDirty))
    {
        ...
        if (attachIndexGL == GL_DEPTH_STENCIL_ATTACHMENT_EXT)
        {
            ...
            glFramebufferRenderbufferEXT(GL_READ_FRAMEBUFFER_EXT, GL_STENCIL_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT,
                                         tex->m_rboName);
            GLMCheckError();
        ...
        if (attachIndexGL == GL_DEPTH_STENCIL_ATTACHMENT_EXT)
        {
            glFramebufferTexture2DEXT(GL_DRAW_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT, GL_TEXTURE_2D, tex->m_texName,
                                      0);
            GLMCheckError();

            glFramebufferTexture2DEXT(GL_DRAW_FRAMEBUFFER_EXT, GL_STENCIL_ATTACHMENT_EXT, GL_TEXTURE_2D, tex->m_texName,
                                      0);
            GLMCheckError();
        }
        else
        ...
        // blit
        glBlitFramebufferEXT(0, 0, tex->m_layout->m_key.m_xSize, tex->m_layout->m_key.m_ySize, 0, 0,
...
{
    if ((m_pairCache) && (m_drawingLang == kGLMGLSL) && (vp && vp->m_descs[kGLMGLSL].m_valid) &&
        (fp && fp->m_descs[kGLMGLSL].m_valid))
    {
...
void GLMContext::SetProgramParametersI(EGLMProgramType type, uint baseSlot, int *slotData,
    ...
    do
    {
        ...
        if (subrect)
        {
            ...
            if (0)
            {
                ...
                // min of the maxes
                scissorBoxNew.width =
                    (std::min(scissorBoxSave.x + scissorBoxSave.width, box->x + box->width)) - scissorBoxNew.x;

                // height is just min of the max y's, minus the new base Y
                scissorBoxNew.height =
                    (std::min(scissorBoxSave.y + scissorBoxSave.height, box->y + box->height)) - scissorBoxNew.y;
            }
            else
            {
...
void GLMContext::Present(CGLMTex *tex)
{
    ...
    //	g_engine->ShowPixels(&showparams);	// doesn't actually show anything, just syncs window/fs state (would make a
    //useful separate call) 	showparams.m_onlySyncView = false;
    ...
    // NULL means targeting GL_BACK.  Blit2 will break it down into two steps if needed, and will handle resolve, scale,
    // flip.
    bool blitScales = (showparams.m_width != dstWidth) || (showparams.m_height != dstHeight);
    this->Blit2(tex, &srcRect, 0, 0, NULL, &dstRect, 0, 0, blitScales ? GL_LINEAR : GL_NEAREST);

    if (m_oneCtxEnable) // if using single context, we need to blast some state so GLM will recover after the FBO
                        // fiddlin'
    {
        BindFBOToCtx(NULL, GL_READ_FRAMEBUFFER_EXT);
        GLMCheckError();
...
GLMContext::GLMContext(GLMDisplayParams *params)
{
    ...
    m_oneCtxEnable =
        true; // 10.6 only...	//(m_caps.m_osComboVersion >= 0x000A0603) && (gl_singlecontext/*.GetInt()*/ );
    ...
    /*
        ...
        if (CommandLine()->FindParm("-glmwriteshaderslotsoptional"))	m_paramWriteMode =
       eParamWriteShaderSlotsOptional;
...
int GLMContext::BindTexToTMU(CGLMTex *tex, int tmu, bool noCheck)
{
    ...
    if (m_samplers[tmu].m_boundTex)
    {
        ...
        //{
        //	glBindTexture( m_samplers[tmu].m_boundTex->m_layout->m_key.m_texGLTarget,
        //m_samplers[tmu].m_boundTex->m_texName );
        //}
        ...
        if ((tex != m_samplers[tmu].m_boundTex) || noCheck)
        {
            ...
            if (!noCheck)
            {
                if ((m_samplers[tmu].m_boundTex))
                {
                    ...
                    {
                        // no, different target.  inbound tex will be set below.  Here, just clear the different target
                        // of the outbound tex.
...
void GLMContext::FlushDrawStates(bool shadersOn) // shadersOn = true for draw calls, false for clear calls
{
    ...
    {
        // GLMPRINTF(("\nGLMContext::FlushDrawStates, setting FBO to %8x(gl %d), was %8x(gl %d)", m_drawingFBO,
        // (m_drawingFBO? m_drawingFBO->m_name: -1),m_boundFBO, (m_boundFBO ? m_boundFBO->m_name : -1) ));
    ...
    // note we generate a mask of which samplers are running "decode sRGB" mode, to help out the shader pair cache
    // mechanism below.
    uint srgbMask = 0;
    for (int i = 0; i < GLM_SAMPLER_COUNT; i++)
    {
    ...
    if (shadersOn)
    {
        ...
        case kGLMARB: {
            ...
            if (fpgood & vpgood)
            {
                ...
                glProgramEnvParameters4fvEXT(GL_VERTEX_PROGRAM_ARB, 0, 256,
                                             (const GLfloat *)&m_programParamsF[kGLMVertexProgram].m_values[0][0]);
                GLMCheckError();

                glProgramEnvParameters4fvEXT(GL_FRAGMENT_PROGRAM_ARB, 0, 32,
                                             (const GLfloat *)&m_programParamsF[kGLMFragmentProgram].m_values[0][0]);
                GLMCheckError();
            }
            else
            {
            ...
            // ARB vert setup. maybe generalize this to handle both ARB and GLSL after we see what GLSL attrib setup
            // looks like.
            ...
            for (int index = 0; index < kGLMVertexAttributeIndexMax; index++)
            {
                ...
                if (relevantMask & mask)
                {
                    ...
                    glVertexAttribPointer(index, setdesc->m_datasize, setdesc->m_datatype, setdesc->m_normalized,
                                          setdesc->m_stride, (const GLvoid *)(uintptr_t)setdesc->m_offset);
                    GLMCheckError();
                    // GLMPRINTF(("--- GLMContext::SetVertexAttributes attr %d set to offset/stride %d/%d in buffer %d
                    // (normalized=%s)", index, setdesc->m_offset, setdesc->m_stride, setdesc->m_buffer->m_name,
                    // setdesc->m_normalized?"true":"false" ));
                }
                else
                {
                    // disable attribute
        ...
        case kGLMGLSL: {
            ...
            CGLMShaderPair *newPair = m_pairCache->SelectShaderPair(
                m_drawingProgram[kGLMVertexProgram], m_drawingProgram[kGLMFragmentProgram], extraKeyBits);
            ...
            if ((newPair != m_boundPair) || (newPairProgram != m_boundPairProgram) ||
                (newPairRevision != m_boundPairRevision))
            {
                m_boundPair = newPair;
                ...
                // values.
                m_programParamsF[kGLMVertexProgram].m_dirtySlotCount =
                    m_drawingProgram[kGLMVertexProgram]->m_descs[kGLMGLSL].m_highWater + 1;
                m_programParamsF[kGLMFragmentProgram].m_dirtySlotCount =
                    m_drawingProgram[kGLMFragmentProgram]->m_descs[kGLMGLSL].m_highWater + 1;
            ...
            {
#if GLMDEBUG
                static uint paramsPushed = 0, paramsSkipped = 0,
                            callsPushed = 0;                               // things that happened on pushed param trips
                ...
                {
                    // spew
                    GLMPRINTF(("VP callsPushed=%d  ( paramsPushed=%d   paramsSkipped=%d )      callsSkipped=%d "
                               "(paramsSkippedByCallSkip=%d)",
            ...
            // ------- bools ---------- //
            if (1 /*m_programParamsB[kGLMVertexProgram].m_dirtySlotCount*/) // optimize this later after the float param
                                                                            // pushes are proven out
            {
            ...
            // ------- int ---------- //
            if (1 /*m_programParamsI[kGLMVertexProgram].m_dirtySlotCount*/) // optimize this later after the float param
                                                                            // pushes are proven out
            {
                GLint vconstInt0Loc = m_boundPair->m_locVertexInteger0; // glGetUniformLocationARB( prog, "i0");
                if (vconstInt0Loc >= 0)
                {
                    glUniform1i(vconstInt0Loc, m_programParamsI[kGLMVertexProgram].m_values[0][0]); // FIXME magic
                                                                                                    // number
                    GLMCheckError();
                }
            ...
            // static char *attribnames[] = { "v0", "v1", "v2", "v3", "v4", "v5", "v6", "v7", "v8", "v9", "v10", "v11",
            // "v12", "v13", "v14", "v15" };
            ...
            for (int index = 0; index < kGLMVertexAttributeIndexMax; index++)
            {
                ...
                if (relevantMask & mask)
                {
                    ...
                    case eAttribWriteDirty:
                        ...
                        // we check buffer revision separately, submitter of vertex setup is not expected to provide it
                        // (zero is preferred). consult the actual buffer directly.
                        ...
                        if ((!(*newDesc == *lastDesc)) || (newDesc->m_buffer->m_revision != lastDesc->m_bufferRevision))
                        {
                            *lastDesc = *newDesc; // latch new setup
                            lastDesc->m_bufferRevision =
                                newDesc->m_buffer->m_revision; // including proper revision of the sourcing buffer
                    ...
                    if (writeAttrib)
                    {
                        ...
                        glVertexAttribPointer(index, newDesc->m_datasize, newDesc->m_datatype, newDesc->m_normalized,
                                              newDesc->m_stride, (const GLvoid *)(uintptr_t)newDesc->m_offset);
                        GLMCheckError();
                    }
                    ...
                    if (!(m_lastKnownVertexAttribMask & (1 << index)))
                    {
                        glEnableVertexAttribArray(
                            index); // enable GLSL attribute- this is just client state - will be turned back off
                        GLMCheckError();
            ...
            if (fconstLoc >= 0)
            {
                ...
                {
                    // spew
                    GLMPRINTF(("FP callsPushed=%d  ( paramsPushed=%d   paramsSkipped=%d )      callsSkipped=%d "
                               "(paramsSkippedByCallSkip=%d)",
                               callsPushed, paramsPushed, paramsSkipped, callsSkipped, paramsSkippedByCallSkip));
                }
#endif
            ...
            if (!m_caps.m_hasGammaWrites) // do we need to think about fake SRGB?
            {
                if (m_boundPair->m_locFragmentFakeSRGBEnable >= 0) // does the shader have that uniform handy?
                {
                    ...
                    if (desiredValue !=
                        m_boundPair
                            ->m_fakeSRGBEnableValue) // and is that different from what it is known to be set to ?
                    {
...
const char *g_vertDumpModeNames[] = {"noTransformDump", "transformedByViewProj", "transformedByModelViewProj",
...
void GLMContext::DebugDump(GLMDebugHookInfo *info, uint options, uint vertDumpMode)
{
    ...
    if (options & (1 << eDumpBatchInfo))
    {
        GLMPRINTF(("-D- %s === %s %d ======================================================== %s %d  frame %d",
                   sg_pPIXName, batchtype, m_debugBatchIndex, batchtype, m_debugBatchIndex, m_debugFrameIndex));
    }
    ...
    if ((options & (1 << eDumpShaderParameters)) && is_draw)
    {
        ...
        {
            usesSkinning |=
                (setup->m_attrMask & (1 << index)) && ((setup->m_vtxAttribMap[index] >> 4) == D3DDECLUSAGE_BLENDWEIGHT);
        }
        if (usesSkinning)
        {
            upperSlotLimit = 256;
    ...
    if ((options & (1 << eDumpTextureSetup)) && is_draw)
    {
        ...
        for (int i = 0; i < GLM_SAMPLER_COUNT; i++)
        {
            if (m_samplers[i].m_boundTex)
            {
                ...
                GLMPRINTF(("-D-"));
                GLMPRINTF(("-D- Sampler %-2d tex %08x  layout %s", i, m_samplers[i].m_boundTex,
                           m_samplers[i].m_boundTex->m_layout->m_layoutSummary));

                GLMPRINTF(("-D-           addressMode[ %s %s %s ]", GLMDecode(eGL_ENUM, samp->m_addressModes[0]),
                           GLMDecode(eGL_ENUM, samp->m_addressModes[1]), GLMDecode(eGL_ENUM, samp->m_addressModes[2])));
    ...
    if ((options & (1 << eDumpVertexAttribSetup)) && is_draw)
    {
        ...
        for (int index = 0; index < kGLMVertexAttributeIndexMax; index++)
        {
            ...
            if (relevantMask & mask)
            {
                ...
                {
                    GLMPRINTF(("-D-   attr=%-2d  decl=$%s%1d  stride=%-2d  offset=%-3d  buf=%08x  bufbase=%08x  "
                               "size=%s  type=%s  normalized=%s  ",
                               index, GLMDecode(eD3D_VTXDECLUSAGE, setup->m_vtxAttribMap[index] >> 4),
                               setup->m_vtxAttribMap[index] & 0x0F, setdesc->m_stride, setdesc->m_offset,
                               setdesc->m_buffer, setdesc->m_buffer->m_lastMappedAddress, sizestr,
                               GLMDecode(eGL_ENUM, setdesc->m_datatype), setdesc->m_normalized ? "Y" : "N"));
                }
                else
                {
                                        ...
                                        if (usage == D3DDECLUSAGE_BLENDINDICES)
                                        {
                                            if (which < 4)
                                            {
                                                ...
                                                vtxBoneIndices[which] =
                                                    unchbase[which]; // ignoring the component reverse which BGRA would
                                                                     // inflict, but we also ignore it below so it
                                                                     // matches up.
                                            }
                                        }
                                    }
    ...
    */
    ...
    GLMPRINTF((
        "-D- ===================================================================================== end %s %d  frame %d",
        batchtype, m_debugBatchIndex, m_debugFrameIndex));
...
void GLMContext::WriteBlendEnableSRGB(GLBlendEnableSRGB_t *src)
{
    ...
    // note however that we're still tracking what this mode should be, so FlushDrawStates can look at it and adjust the
    // pixel shader if fake SRGB mode is in place (m_caps.m_hasGammaWrites is false)
}
...
void GLContextSet(GLBlendEnableSRGB_t *src)
{
    ...
    // GL_FRAMEBUFFER_ATTACHMENT_COLOR_ENCODING, &encoding ); GLMCheckError();
...
void GLMTester::CheckGLError(const char *comment)
{
    ...
    if (errorcode != GL_NO_ERROR)
    {
        ...
        if (errorcode == GL_INVALID_FRAMEBUFFER_OPERATION_EXT)
        {
            ...
            sprintf(errbuf, "\n%s - GL Error %08x/%08x = '%s / %s'", comment, errorcode, errorcode2, decodedStr,
                    decodedStr2);
        }
        else
        {
                        ...
                        case 1: {
                            ...
                            for (int face = 0; face < ptex->m_layout->m_faceCount; face++)
                            {
                                for (int mip = 0; mip < ptex->m_layout->m_mipCount; mip++)
                                {
                                    ...
                                    gtp.m_chunkCount =
                                        (slice->m_xSize * slice->m_ySize * slice->m_zSize) /
                                        (ptex->m_layout->m_format->m_chunkSize * ptex->m_layout->m_format->m_chunkSize);
