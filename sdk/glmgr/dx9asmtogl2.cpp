...
static const char *g_szVecZeros[] = {NULL, "0.0", "vec2( 0.0, 0.0 )", "vec3( 0.0, 0.0, 0.0 )",
                                     "vec4( 0.0, 0.0, 0.0, 0.0 )"};
static const char *g_szVecOnes[] = {NULL, "1.0", "vec2( 1.0, 1.0 )", "vec3( 1.0, 1.0, 1.0 )",
...
void D3DToGL::PrintUsageAndIndexToString(uint32 dwToken, char *strUsageUsageIndexName, int nBufLen, int fSemanticFlags)
{
    ...
    case D3DDECLUSAGE_POSITION:
        ...
        else
        {
            V_snprintf(strUsageUsageIndexName, nBufLen,
                       "vertex.attrib[0]"); //"vertex.position" );			// aka generic [0]
        }
        break;
    ...
    case D3DDECLUSAGE_TEXCOORD:
        ...
        {
            V_snprintf(strUsageUsageIndexName, nBufLen, "vertex.attrib[%d]",
                       8 + dwUsageIndex); // "vertex.texcoord[%d]", dwUsageIndex );		// aka [8] - [15] ?
        }
        break;
    ...
    case D3DDECLUSAGE_COLOR:
        ...
        {
            //				if ( fSemanticFlags & SEMANTIC_OUTPUT )
            //					V_snprintf( strUsageUsageIndexName, nBufLen, dwUsageIndex != 0 ? "gl_BackColor" :
            //"gl_FrontColor" ); 				else
        ...
        {
            V_snprintf(strUsageUsageIndexName, nBufLen, "vertex.attrib[%d]",
                       3 + dwUsageIndex); // dwUsageIndex );		// != 0 ? "vertex.color.secondary" : "vertex.color" );
                                          // // aka [3] / [4] (second)
        }
        break;
    case D3DDECLUSAGE_FOG:
        Assert(!m_bGLSL);
        V_snprintf(
            strUsageUsageIndexName, nBufLen,
            "vertex.attrib[5]"); //"vertex.position" /* "_fog" */ );	//FIXME, evil		// aka [5] / vertex.fogcoord
        break;
                                     ...
                                     bool bForceScalarSource, int *pARLDestReg)
{
    ...
    case D3DSPR_CONST:
        ...
        else // Direct addressing of constant array
        {
            ...
            //// NOGO if (dwRegNum != 255)	// have seen cases where dwRegNum is 0xFF... need to figure out where those
            ///opcodes are coming from
            {
    ...
    case D3DSPR_RASTOUT: // vertex shader oPos
        ...
        case D3DSRO_POSITION:
            strcat_s(pRegisterName, nBufLen,
                     m_bGLSL ? "vTempPos" : "oPos"); // In GLSL, this ends up in gl_Position later on
...
void D3DToGL::Handle_DEF()
{
    ...
    const char float_fmt_commaspace[] =
        "%.12f, "; // %g causes GLSL compile problems around consts like "1e+2.0" - try %f
...
void D3DToGL::Handle_DP2ADD()
{
    ...
    if (m_bGLSL)
    {
        ...
        if (nDestToken & D3DSPDM_SATURATE)
        {
            ...
            PrintToBuf(*m_pBufALUCode, "%s = clamp( %s, %s, %s );\n", pDestReg, pDestReg, g_szVecZeros[nComponents],
                       g_szVecOnes[nComponents]);
        }
    }
    else
    {
...
void D3DToGL::Handle_LRP(uint32 nInstruction)
{
    if (m_bGLSL)
    {
        ...
        PrintToBuf(*m_pBufALUCode, "%s = %s * ( %s - %s ) + %s;\n", sDest.String(), sParam0.String(), sParam1.String(),
                   sParam2.String(), sParam2.String());
...
void D3DToGL::Handle_TEX(uint32 dwToken, bool bIsTexLDL)
{
    ...
    if (m_bGLSL)
    {
        ...
        if (nSamplerType == SAMPLER_TYPE_2D)
        {
            ...
            else if (((int)(dwSrc1Token & D3DSP_REGNUM_MASK)) ==
                     m_nShadowDepthSampler) // Syntax for shadow depth sampler
            {
...
void D3DToGL::Handle_CMP()
{
    ...
    if (m_bGLSL)
    {
        ...
        if (DoParamNamesMatch(pDestReg, pSrc0Reg) && GetNumSwizzleComponents(pDestReg) > 1)
        {
            ...
            //
            // But the first lines changes r0.x and thus screws the atomicity of the CMP instruction for the second
            // line. So we assign r0 to a temporary first and then write to the temporary.
...
void D3DToGL::Handle_UnaryOp(uint32 nInstruction)
{
    ...
    CUtlString sParam1 = GetParameterString(nDestToken, DST_REGISTER, false, NULL);
    CUtlString sParam2 =
        GetParameterString(GetNextToken(), SRC_REGISTER, (nInstruction == D3DSIO_MOVA) && !m_bGLSL, NULL);
    sParam2 = FixGLSLSwizzle(sParam1, sParam2);
...
int D3DToGL::TranslateShader(uint32 *code, CUtlBuffer *pBufDisassembledCode, bool *bVertexShader, uint32 options,
                             int32 nShadowDepthSampler, uint32 nCentroidMask, char *debugLabel)
{
    ...
    // If pixel shader
    char *glslBindableUniformExtText =
    ...
    *bVertexShader = m_bVertexShader;
    ...
    if (m_bGLSL)
    {
        ...
        PrintToBuf(*m_pBufHeaderCode, "\n%suniform vec4 %s[%d];\n", m_bUseBindableUniforms ? "bindable " : "",
    ...
    else
    {
        ...
        PrintToBuf(*m_pBufParamCode, "PARAM %s[%d] = { program.%s[0..%d] };\n", m_bVertexShader ? "vc" : "pc",
                   m_nHighestRegister + 1, m_bUseEnvParams ? "env" : "local", m_nHighestRegister);
    }
    ...
    if (m_bDeclareVSOPos && m_bVertexShader)
    {
        if (m_bGLSL)
        {
            if (m_bDoUserClipPlanes)
            {
                StrcatToALUCode("gl_ClipVertex = vTempPos;\n"); // if user clip is enabled, jam clip space position into
                                                                // gl_ClipVertex
            }
        }
    ...
    // do some annotation at the end of the attrib block
    {
        ...
        V_snprintf(temp, sizeof(temp), "%s trans#%d label:%s\n", m_bGLSL ? "//" : "#", g_translationCounter,
                   debugLabel ? debugLabel : "none");
        StrcatToAttribCode(temp);
