...
bool GLMGenTexels(GLMGenTexelParams *params)
{
    ...
    case D3DFMT_A8R8G8B8: // B G R A
        InsertTexelComponentFixed(
            params->a, 8, temp32); // A is inserted first and winds up at most significant bits after insertions follow
    ...
    case D3DFMT_DXT5: {
        ...
        // so to write a single alpha value, just figure out the value, store it in both the first two bytes then store
        // zeroes.
    ...
    case D3DFMT_A16B16G16R16:
        ...
        InsertTexelComponentFixed(params->r, 16,
                                  (unsigned long *)&chunkbuf[0]); // winds up as LSW of first word, thus first in RAM
    ...
    else
    {
        ...
        for (int mip = 0; mip < mipCount; mip++)
        {
            for (int face = 0; face < faceCount; face++)
            {
                ...
                storage_x =
                    std::max(slicePtr->m_xSize, formatDesc->m_chunkSize); // storage extent can't go below chunk size

                slicePtr->m_ySize = layout->m_key.m_ySize >> mip;
                slicePtr->m_ySize = std::max(slicePtr->m_ySize, 1); // dimension can't go to zero
                storage_y =
                    std::max(slicePtr->m_ySize, formatDesc->m_chunkSize); // storage extent can't go below chunk size
    ...
    case GL_TEXTURE_2D: {
        ...
        if (format->m_chunkSize != 1)
        {
            ...
            glCompressedTexImage2D(target,            // target
                                   desc->m_req.m_mip, // level
                                   intformat, // internalformat - don't use format->m_glIntFormat because we have the
                                              // SRGB select going on above
                                   slice->m_xSize,       // width
        ...
        else
        {
            if (mayUseSubImage)
            {
                ...
                glTexSubImage2D(target,
                                ...
                                glDataFormat,                  // format
                                glDataType,                    // type
                                sliceAddress // data (will be offsetted by the SKIP_PIXELS and SKIP_ROWS - let GL do the
                                             // math to find the first source texel)
                );
                GLMCheckError();
            ...
            else
            {
                ...
                glTexImage2D(target,            // target
                             desc->m_req.m_mip, // level
                             intformat, // internalformat - don't use format->m_glIntFormat because we have the SRGB
                                        // select going on above
                             slice->m_xSize, // width
                             slice->m_ySize, // height
                             0,              // border
