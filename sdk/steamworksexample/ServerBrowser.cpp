...
void CServerBrowser::RefreshInternetServers()
{
    ...
    m_hServerListRequest = SteamMatchmakingServers()->RequestInternetServerList(SteamUtils()->GetAppID(), &pFilter,
                                                                                ARRAYSIZE(pFilters), this);
}
...
void CServerBrowser::ServerResponded(HServerListRequest hReq, int iServer)
{
    ...
    if (pServer)
    {
        // Filter out servers that don't match our appid here (might get these in LAN calls since we can't put more
        // filters on it)
