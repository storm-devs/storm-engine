...
class CGameEngineGL : public IGameEngine
{
  public:
    ...
    // on OSX client with DX9MODE=1, the HGAMEFONT is a texture with the glyphs in a 16x16 grid (see "g_glmDebugFontMap"
    // in glmgrbasics.cpp)
    ...
    HGAMETEXTURE HCreateTexture(byte *pRGBAData, uint32 uWidth, uint32 uHeight,
                                ETEXTUREFORMAT eTextureFormat = eTextureFormat_RGBA);

    // update an existing texture
    bool UpdateTexture(HGAMETEXTURE texture, byte *pRGBAData, uint32 uWidth, uint32 uHeight,
    ...
    bool BDrawTexturedRect(float xPos0, float yPos0, float xPos1, float yPos1, float u0, float v0, float u1, float v1,
                           DWORD dwColor, HGAMETEXTURE hTexture);

    // Draw a textured arbitrary quad
    bool BDrawTexturedQuad(float xPos0, float yPos0, float xPos1, float yPos1, float xPos2, float yPos2, float xPos3,
                           float yPos3, float u0, float v0, float u1, float v1, DWORD dwColor, HGAMETEXTURE hTexture);
    ...
    const char *GetTextStringForControllerOriginDigital(ECONTROLLERACTIONSET dwActionSet,
                                                        ECONTROLLERDIGITALACTION dwDigitalAction);
    const char *GetTextStringForControllerOriginAnalog(ECONTROLLERACTIONSET dwActionSet,
    ...
    void TriggerControllerHaptics(ESteamControllerPad ePad, unsigned short usOnMicroSec, unsigned short usOffMicroSec,
    ...
    //			way to handle that.
    uint64 GetGameTicksFrameDelta()
    {
        return m_ulGameTickCount - m_ulPreviousGameTickCount;
    }
    ...
    bool BUberRenderPrimitive(IDirect3DVertexShader9 *vsh, IDirect3DPixelShader9 *psh,
                              IDirect3DVertexDeclaration9 *decl, uint streamNumber, HGAMEVERTBUF hVertBuf,
                              uint32 uOffset, uint32 uStride, D3DPRIMITIVETYPE primType, uint32 uStartVertex,
                              uint32 uCount);
    ...
    void RenderedSize(uint &width, uint &height,
