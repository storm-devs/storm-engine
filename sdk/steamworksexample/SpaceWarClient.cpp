...
#else
    // When you aren't using Steam auth you can still call AdvertiseGame() so you can communicate presence data to the
    // friends system. Make sure to pass k_steamIDNonSteamGS
    ...
    for (uint32 i = 0; i < MAX_PLAYERS_PER_SERVER; ++i)
    {
        ...
        if (pUpdateData->GetPlayerActive(i))
        {
            ...
            if (!m_rgpShips[i])
            {
                ...
                m_rgpShips[i] = new CShip(m_pGameEngine, false, pShipData->GetXPosition(), pShipData->GetYPosition(),
                                          g_rgPlayerColors[i]);
...
//-----------------------------------------------------------------------------
// Purpose: a Steam URL to launch this app was executed while the game is already running, eg
// steam://run/480//+connect%20127.0.0.1
//      	Anybody can build random Steam URLs	and these extra parameters must be carefully parsed to avoid unintended
//      side-effects
//-----------------------------------------------------------------------------
...
void CSpaceWarClient::OnMenuSelection(LobbyMenuItem_t selection)
{
    ...
    else if (selection.m_eCommand == LobbyMenuItem_t::k_ELobbyMenuItemToggleReadState)
    {
        // update our state
        bool bOldState =
            (1 == atoi(SteamMatchmaking()->GetLobbyMemberData(m_steamIDLobby, SteamUser()->GetSteamID(), "ready")));
    ...
    else if (selection.m_eCommand == LobbyMenuItem_t::k_ELobbyMenuItemStartGame)
    {
        ...
        // before telling all the lobby members to join (so that the NAT traversal code has a path to contact the game
        // server)
...
void CSpaceWarClient::OnGameStateChanged(EClientGameState eGameStateNew)
{
    ...
    else if (m_eGameState == k_EClientCreatingLobby)
    {
        ...
        if (!m_SteamCallResultLobbyCreated.IsActive())
        {
            ...
            SteamAPICall_t hSteamAPICall =
                SteamMatchmaking()->CreateLobby(k_ELobbyTypePublic /* public lobby, anyone can find it */, 4);
    ...
    if (pchSteamRichPresenceDisplay != NULL)
    {
        SteamFriends()->SetRichPresence("steam_display",
                                        bDisplayScoreInRichPresence ? "#StatusWithScore" : "#StatusWithoutScore");
...
void CSpaceWarClient::RunFrame()
{
    ...
    if (m_eConnectedStatus != k_EClientNotConnected &&
        m_pGameEngine->GetGameTickCount() - m_ulLastNetworkDataReceivedTime > MILLISECONDS_CONNECTION_TIMEOUT)
    {
...
void CSpaceWarClient::DrawHUDText()
{
    ...
    for (uint32 i = 0; i < MAX_PLAYERS_PER_SERVER; ++i)
    {
        ...
        case 0:
            ...
            if (hTexture)
            {
                m_pGameEngine->BDrawTexturedRect((float)rect.left, (float)rect.top, (float)rect.left + nAvatarWidth,
                                                 (float)rect.bottom, 0.0f, 0.0f, 1.0, 1.0,
                                                 D3DCOLOR_ARGB(255, 255, 255, 255), hTexture);
            ...
            m_pGameEngine->BDrawString(m_hHUDFont, rect, g_rgPlayerColors[i], TEXTPOS_LEFT | TEXTPOS_VCENTER,
                                       rgchBuffer);
            break;
        case 1:
            ...
            if (hTexture)
            {
                m_pGameEngine->BDrawTexturedRect((float)rect.right - nAvatarWidth, (float)rect.top, (float)rect.right,
                                                 (float)rect.bottom, 0.0f, 0.0f, 1.0, 1.0,
                                                 D3DCOLOR_ARGB(255, 255, 255, 255), hTexture);
                rect.right -= nAvatarWidth + nSpaceBetweenAvatarAndScore;
                rect.left -= nAvatarWidth + nSpaceBetweenAvatarAndScore;
            ...
            m_pGameEngine->BDrawString(m_hHUDFont, rect, g_rgPlayerColors[i], TEXTPOS_RIGHT | TEXTPOS_VCENTER,
                                       rgchBuffer);
            break;
        case 2:
            ...
            sprintf_safe(rgchBuffer, "%s\nScore: %2u %s", rgchPlayerName, m_rguPlayerScores[i], pszVoiceState);
            m_pGameEngine->BDrawString(m_hHUDFont, rect, g_rgPlayerColors[i], TEXTPOS_LEFT | TEXTPOS_BOTTOM,
                                       rgchBuffer);
            break;
        case 3:
            rect.top = height - nHudPaddingVertical - nAvatarHeight;
            ...
            sprintf_safe(rgchBuffer, "%s\nScore: %2u %s", rgchPlayerName, m_rguPlayerScores[i], pszVoiceState);
            m_pGameEngine->BDrawString(m_hHUDFont, rect, g_rgPlayerColors[i], TEXTPOS_RIGHT | TEXTPOS_BOTTOM,
                                       rgchBuffer);
            break;
        default:
    ...
    {
        char rgchHint[128];
        const char *rgchFireOrigin = m_pGameEngine->GetTextStringForControllerOriginDigital(
            eControllerActionSet_ShipControls, eControllerDigitalAction_FireLasers);
        ...
        m_pGameEngine->BDrawString(m_hHUDFont, rect, D3DCOLOR_ARGB(255, 255, 255, 255), TEXTPOS_LEFT | TEXTPOS_TOP,
                                   rgchHint);
    }
}
...
void CSpaceWarClient::DrawInstructions()
{
    ...
    m_pGameEngine->BDrawString(m_hInstructionsFont, rect, D3DCOLOR_ARGB(255, 25, 200, 25),
                               TEXTPOS_CENTER | TEXTPOS_VCENTER, rgchBuffer);
    ...
    m_pGameEngine->BDrawString(m_hInstructionsFont, rect, D3DCOLOR_ARGB(255, 25, 200, 25), TEXTPOS_CENTER | TEXTPOS_TOP,
                               rgchBuffer);
}
...
void CSpaceWarClient::DrawConnectionAttemptText()
{
    ...
    uint32 uSecondsLeft =
        (MILLISECONDS_CONNECTION_TIMEOUT - uint32(m_pGameEngine->GetGameTickCount() - m_ulStateTransitionTime)) / 1000;
...
{
    int nSecondsToRestart =
        ((MILLISECONDS_BETWEEN_ROUNDS - (int)(m_pGameEngine->GetGameTickCount() - m_ulStateTransitionTime)) / 1000) + 1;
    ...
    else if (m_eGameState == k_EClientGameWinner)
    {
        ...
        {
            sprintf_safe(rgchPlayerName, "%s",
                         SteamFriends()->GetFriendPersonaName(m_rgSteamIDPlayers[m_uPlayerWhoWonGame]));
        }
        else
        {
            sprintf_safe(rgchPlayerName, "Unknown Player");
    ...
    // Note: GetLastDroppedItem is the result of an async function, this will not render the reward right away. Could
    // wait for it.
...
//-----------------------------------------------------------------------------
void CSpaceWarClient::OnRequestEncryptedAppTicket(EncryptedAppTicketResponse_t *pEncryptedAppTicketResponse,
                                                  bool bIOFailure)
{
    if (bIOFailure)
        return;
    ...
    if (pEncryptedAppTicketResponse->m_eResult == k_EResultOK)
    {
        ...
        if (!SteamEncryptedAppTicket_BDecryptTicket(rgubTicket, cubTicket, rgubDecrypted, &cubDecrypted, rgubKey,
                                                    sizeof(rgubKey)))
        {
        ...
        uint32 cubData;
        uint32 *punSecretData =
            (uint32 *)SteamEncryptedAppTicket_GetUserVariableData(rgubDecrypted, cubDecrypted, &cubData);
...
bool CSpaceWarClient::LoadWorkshopItem(PublishedFileId_t workshopItemID)
{
    ...
    {
        // szItemFolder just points directly to the item for legacy items that were published with the RemoteStorage
        // API.
...
void CSpaceWarClient::LoadWorkshopItems()
{
    ...
    if (m_nNumWorkshopItems < MAX_WORKSHOP_ITEMS)
    {
        ...
        if (pItem)
        {
            ...
            strncpy(pItem->m_ItemDetails.m_rgchDescription, "This is a local test item for debugging",
...
void CSpaceWarClient::DrawWorkshopItems()
{
    ...
    for (int iSubscribedItem = 0; iSubscribedItem < MAX_WORKSHOP_ITEMS; iSubscribedItem++)
    {
        ...
        sprintf_safe(rgchBuffer, "%u. \"%s\" (%llu) : %s", iSubscribedItem + 1, pItem->m_ItemDetails.m_rgchTitle,
                     pItem->m_ItemDetails.m_nPublishedFileId, pItem->m_ItemDetails.m_rgchDescription);

        m_pGameEngine->BDrawString(m_hInstructionsFont, rect, D3DCOLOR_ARGB(255, 25, 200, 25),
                                   TEXTPOS_LEFT | TEXTPOS_VCENTER, rgchBuffer);
    }
...
void CSpaceWarClient::DrawInGameStore()
{
    ...
    for (uint32 i = 0; i < m_vecPurchaseableItems.size(); ++i)
    {
        ...
        if (!SteamInventory()->GetItemDefinitionProperty(t.m_nItemDefID, "name", buf, &bufSize) &&
            bufSize <= sizeof(buf))
        {
            continue;
        }
        ...
        sprintf_safe(rgchBuffer, "%u. Purchase %-25s    %s %0.2f    (own %u)", i + 1, buf, m_rgchCurrency,
                     float(t.m_ulPrice) / 100, unQuantity);

        m_pGameEngine->BDrawString(m_hInGameStoreFont, rect, D3DCOLOR_ARGB(255, 25, 200, 25),
                                   TEXTPOS_LEFT | TEXTPOS_VCENTER, rgchBuffer);
