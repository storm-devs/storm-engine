...
//  This is OK because it's warning on setting up the Steam callbacks, they won't use this until after construction is
//  done
#pragma warning(disable : 4355)
CStatsAndAchievements::CStatsAndAchievements(IGameEngine *pGameEngine)
    : m_pGameEngine(pGameEngine), m_pSteamUser(NULL), m_pSteamUserStats(NULL), m_GameID(SteamUtils()->GetAppID()),
...
void CStatsAndAchievements::OnPS3TrophiesInstalled(PS3TrophiesInstalled_t *pCallback)
{
    ...
    if (m_GameID.ToUint64() == pCallback->m_nGameID)
    {
        ...
        {
            // this is a fatal error. Usually the PS3 would have already displayed a fatal error to the user and forced
            // the game to exit. If the system could not display that error, you should display the appropriate message
...
void CStatsAndAchievements::Render()
{
    ...
    if (!m_bStatsValid)
    {
        ...
        m_pGameEngine->BDrawString(m_hDisplayFont, rect, D3DCOLOR_ARGB(255, 25, 200, 25),
        ...
        m_pGameEngine->BDrawString(m_hDisplayFont, rect, D3DCOLOR_ARGB(255, 25, 200, 25), TEXTPOS_CENTER | TEXTPOS_TOP,
                                   rgchBuffer);
    }
    else
    {
        ...
        m_pGameEngine->BDrawString(m_hDisplayFont, rect, D3DCOLOR_ARGB(255, 25, 200, 25),
                                   TEXTPOS_LEFT | TEXTPOS_VCENTER, "Inventory");

        std::list<CSpaceWarItem *>::const_iterator iter;
        for (iter = SpaceWarLocalInventory()->GetItemList().begin();
             iter != SpaceWarLocalInventory()->GetItemList().end(); ++iter)
        {
            rect.top = pxVertOffset;
            rect.bottom = rect.top + ACHDISP_FONT_HEIGHT;
            pxVertOffset = rect.bottom + ACHDISP_VERT_SPACING;
...
void CStatsAndAchievements::DrawAchievementInfo(RECT &rect, Achievement_t &ach)
{
    ...
    if (hTexture)
    {
        m_pGameEngine->BDrawTexturedRect((float)rect2.left, (float)rect2.top, (float)rect2.left + ACHDISP_IMG_SIZE,
                                         (float)rect2.bottom, 0.0f, 0.0f, 1.0, 1.0, D3DCOLOR_ARGB(255, 255, 255, 255),
                                         hTexture);
    ...
    char rgchBuffer[256];
    sprintf_safe(rgchBuffer, "%s: %s\n%s", ach.m_rgchName, ach.m_bAchieved ? "Unlocked" : "Locked",
                 ach.m_rgchDescription);

    m_pGameEngine->BDrawString(m_hDisplayFont, rect2, D3DCOLOR_ARGB(255, 25, 200, 25), TEXTPOS_LEFT | TEXTPOS_VCENTER,
                               rgchBuffer);
}
...
void CStatsAndAchievements::DrawStatInfo(RECT &rect, const char *pchName, float flValue)
{
    ...
    m_pGameEngine->BDrawString(m_hDisplayFont, rect, D3DCOLOR_ARGB(255, 25, 200, 25), TEXTPOS_LEFT | TEXTPOS_VCENTER,
                               rgchBuffer);
}
...
{
#ifdef _PS3
    // On PS3, we need to load the user's stats & achievement information from the save container. In this example, we
    // are simply reading the data from a known location on disk
