...
#define VOICE_OUTPUT_SAMPLE_RATE                                                                                       \
    11000 // real sample rate is 11025 but for XAudio2 it must be a multiple of XAUDIO2_QUANTUM_DENOMINATOR
...
class IGameEngine
{
  public:
    ...
    // Create a new texture returning our internal handle value for it (0 means failure), texture type specifies the
    // type of data contained in pData
    virtual HGAMETEXTURE HCreateTexture(byte *pData, uint32 uWidth, uint32 uHeight,
                                        ETEXTUREFORMAT eTextureFormat = eTextureFormat_RGBA) = 0;

    // update an existing texture, texture type specifies the type of data contained in pData
    virtual bool UpdateTexture(HGAMETEXTURE texture, byte *pData, uint32 uWidth, uint32 uHeight,
                               ETEXTUREFORMAT eTextureFormat = eTextureFormat_RGBA) = 0;
    ...
    virtual bool BDrawTexturedRect(float xPos0, float yPos0, float xPos1, float yPos1, float u0, float v0, float u1,
                                   float v1, DWORD dwColor, HGAMETEXTURE hTexture) = 0;

    // Draw a textured arbitrary quad
    virtual bool BDrawTexturedQuad(float xPos0, float yPos0, float xPos1, float yPos1, float xPos2, float yPos2,
    ...
    virtual const char *GetTextStringForControllerOriginDigital(ECONTROLLERACTIONSET dwActionSet,
                                                                ECONTROLLERDIGITALACTION dwDigitalAction) = 0;
    virtual const char *GetTextStringForControllerOriginAnalog(ECONTROLLERACTIONSET dwActionSet,
                                                               ECONTROLLERANALOGACTION dwDigitalAction) = 0;
    ...
    virtual void TriggerControllerHaptics(ESteamControllerPad ePad, unsigned short usOnMicroSec,
                                          unsigned short usOffMicroSec, unsigned short usRepeat) = 0;
