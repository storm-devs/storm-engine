...
void CSpaceWarLocalInventory::DoExchange()
{
    ...
    if (item100 && item101 && item102 && item103)
    {
        ...
        outputQuantity[0] = 1;
        SteamInventory()->ExchangeItems(&m_hExchangeRequestResult, outputItems, outputQuantity, 1, inputItems,
                                        inputQuantities, 4);
    }
}
...
std::string CSpaceWarItem::GetLocalizedDescription() const
{
    ...
    char buf[2048];
    uint32 bufSize = sizeof(buf);
    if (SteamInventory()->GetItemDefinitionProperty(GetDefinition(), "description", buf, &bufSize) &&
        bufSize <= sizeof(buf))
    {
        ret = buf;
    }
...
std::string CSpaceWarItem::GetIconURL() const
{
    ...
    char buf[512];
    uint32 bufSize = sizeof(buf);
    if (SteamInventory()->GetItemDefinitionProperty(GetDefinition(), "icon_url", buf, &bufSize) &&
        bufSize <= sizeof(buf))
    {
        ret = buf;
    }
