...
{
    if (m_pRemoteStorageSync &&
        (m_eState == k_ERemoteStorageStateSyncToClient || m_eState == k_ERemoteStorageStateSyncToServer))
...
void CRemoteStorageScreen::Render()
{
    ...
    while (m_pGameEngine->BGetFirstKeyDown(&dwVKDown))
    {
            ...
            if (!bQuotaExceeded)
            {
                ...
                if (ulCurrentTickCount - 150 > g_ulLastReturnKeyTick)
                {
                    ...
                    // Do it
                    {
                        ...
                        bool bRet = m_pSteamRemoteStorage->FileWrite(MESSAGE_FILE_NAME, m_rgchGreeting,
                                                                     (int)strlen(m_rgchGreeting));
        ...
        else if ((dwVKDown >= 0x30 && dwVKDown <= 0x39) || (dwVKDown >= 0x41 && dwVKDown <= 0x5A) ||
                 dwVKDown == VK_SPACE)
        {
            // Add the key pressed
    ...
    RECT rect;
    {
        ...
        m_pGameEngine->BDrawString(m_hDisplayFont, rect, D3DCOLOR_ARGB(255, 25, 200, 25),
                                   TEXTPOS_CENTER | TEXTPOS_VCENTER, m_rgchGreeting);
        ...
        m_pGameEngine->BDrawString(m_hDisplayFont, rect, D3DCOLOR_ARGB(255, 25, 200, 25),
                                   TEXTPOS_CENTER | TEXTPOS_VCENTER, m_rgchGreetingNext);
