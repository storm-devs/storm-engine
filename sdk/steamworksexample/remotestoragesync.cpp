...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CRemoteStorageSyncMenu::CRemoteStorageSyncMenu(IGameEngine *pGameEngine)
    : CBaseMenu<ERemoteStorageSyncMenuCommand>(pGameEngine)
{
}
...
void CRemoteStorageSyncMenu::ShowSyncProgress(ESyncOperation eSyncOperation, double dPercentComplete)
{
    ...
    const char *pchFormat =
        (eSyncOperation == k_ESyncOperationToClient) ? "Syncing to client (%d%%)" : "Syncing to server(%d%%)";
...
void CRemoteStorageSyncMenu::ShowSyncComplete(ESyncOperation eSyncOperation, bool bSuccess)
{
    ...
    const char *pchFormat =
        (eSyncOperation == k_ESyncOperationToClient) ? "Sync to client %s.. press X" : "Sync to server %s.. press X";
...
void CRemoteStorageSyncMenu::ShowConflict()
{
    ...
    // Description
    AddMenuItem(CRemoteStorageSyncMenu::MenuItem_t(
        "There is a conflict between the local & server versions of your files:", k_EMenuCommandNone));
...
CRemoteStorageSync::CRemoteStorageSync(IGameEngine *pGameEngine)
    : m_eSyncOperation(k_ESyncOperationIdle), m_menu(pGameEngine),
...
void CRemoteStorageSync::OnAppSyncProgress(RemoteStorageAppSyncProgress_t *pParam)
{
    ...
    if (m_eSyncOperation == k_ESyncOperationIdle ||
        (pParam->m_bUploading && m_eSyncOperation != k_ESyncOperationToServer))
        return;
