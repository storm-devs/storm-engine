...
CP2PAuthPlayer::CP2PAuthPlayer(IGameEngine *pGameEngine, CSteamID steamID, HSteamNetConnection hServerConn)
    : m_CallbackBeginAuthResponse(this, &CP2PAuthPlayer::OnBeginAuthResponse), m_steamID(steamID),
...
void CP2PAuthPlayer::StartAuthPlayer()
{
    ...
    {
        m_hAuthTicketIGaveThisUser = SteamUser()->GetAuthSessionTicket(
            m_rgubTicketIGaveThisUser, sizeof(m_rgubTicketIGaveThisUser), &m_cubTicketIGaveThisUser);
    }
    ...
    if (SteamNetworkingSockets()->SendMessageToConnection(
            m_hServerConnection, &msg, sizeof(msg), k_nSteamNetworkingSend_Reliable, &nIgnoreMessageID) == k_EResultOK)
    {
        m_bSentTicket = true;
    }
