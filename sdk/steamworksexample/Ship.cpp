...
//-----------------------------------------------------------------------------
CShipDebris::CShipDebris(IGameEngine *pGameEngine, float xPos, float yPos, DWORD dwDebrisColor)
    : CSpaceWarEntity(pGameEngine, 0, true)
{
...
//  This is OK because the thruster classes won't use the ship object in their constructors (where it may still be only
//  partly constructed)
#pragma warning(disable : 4355)
CShip::CShip(IGameEngine *pGameEngine, bool bIsServerInstance, float xPos, float yPos, DWORD dwShipColor)
    : CSpaceWarEntity(pGameEngine, 11, true), m_ForwardThrusters(pGameEngine, this),
      m_ReverseThrusters(pGameEngine, this)
{
    m_bDisabled = false;
    m_bExploding = false;
    ...
    m_pGameEngine->SetControllerColor(m_dwShipColor >> 16 & 255, m_dwShipColor >> 8 & 255, m_dwShipColor & 255,
...
void CShip::OnReceiveServerUpdate(ServerShipUpdateData_t *pUpdateData)
{
    ...
    SetPosition(pUpdateData->GetXPosition() * m_pGameEngine->GetViewportWidth(),
                pUpdateData->GetYPosition() * m_pGameEngine->GetViewportHeight());
    ...
    for (int i = 0; i < MAX_PHOTON_BEAMS_PER_SHIP; ++i)
    {
        ...
        if (pPhotonUpdate->GetActive())
        {
            if (!m_rgPhotonBeams[i])
            {
                m_rgPhotonBeams[i] = new CPhotonBeam(
                    m_pGameEngine, pPhotonUpdate->GetXPosition(), pPhotonUpdate->GetYPosition(), m_dwShipColor,
                    pPhotonUpdate->GetRotation(), pPhotonUpdate->GetXVelocity(), pPhotonUpdate->GetYVelocity());
            }
            else
            {
...
void CShip::RunFrame()
{
    ...
    if (m_bIsLocalPlayer)
    {
        ...
        else if (m_pGameEngine->BIsKeyDown(0x31) &&
                 SpaceWarLocalInventory()->HasInstanceOf(k_SpaceWarItem_ShipDecoration1))
        {
            m_nShipDecoration = 1;
            BuildGeometry();
        }
        else if (m_pGameEngine->BIsKeyDown(0x32) &&
                 SpaceWarLocalInventory()->HasInstanceOf(k_SpaceWarItem_ShipDecoration2))
        {
            m_nShipDecoration = 2;
            BuildGeometry();
        }
        else if (m_pGameEngine->BIsKeyDown(0x33) &&
                 SpaceWarLocalInventory()->HasInstanceOf(k_SpaceWarItem_ShipDecoration3))
        {
            m_nShipDecoration = 3;
            BuildGeometry();
        }
        else if (m_pGameEngine->BIsKeyDown(0x34) &&
                 SpaceWarLocalInventory()->HasInstanceOf(k_SpaceWarItem_ShipDecoration4))
        {
            m_nShipDecoration = 4;
            BuildGeometry();
        }
        ...
        else if (m_pGameEngine->BIsKeyDown(0x37) &&
                 SpaceWarLocalInventory()->HasInstanceOf(k_SpaceWarItem_ShipSpecial1))
        {
            m_nShipPower = 1;
        }
        else if (m_pGameEngine->BIsKeyDown(0x38) &&
                 SpaceWarLocalInventory()->HasInstanceOf(k_SpaceWarItem_ShipSpecial2))
        {
            m_nShipPower = 2;
        }
    }
    else if (m_bIsServerInstance)
    {
        ...
        if (m_SpaceWarClientUpdateData.GetReverseThrustersPressed() ||
            m_SpaceWarClientUpdateData.GetForwardThrustersPressed())
        {
            float flSign = 1.0f;
            if (m_SpaceWarClientUpdateData.GetReverseThrustersPressed())
            {
    ...
    {
        // server side
        if (nNextAvailablePhotonBeamSlot != -1 && !m_bExploding && m_SpaceWarClientUpdateData.GetFirePressed() &&
            ulCurrentTickCount - PHOTON_BEAM_FIRE_INTERVAL_TICKS > m_ulLastPhotonTickCount)
        {
            ...
            if (m_nShipWeapon == 1) // Item#101
            {
                ...
                m_rgPhotonBeams[nNextAvailablePhotonBeamSlot] = new CPhotonBeam(
                    m_pGameEngine, xPos, yPos, m_dwShipColor, GetAccumulatedRotation(), xVelocity, yVelocity);
    ...
    // Finally, update the thrusters ( we do this after the base class call as they rely on our data being fully
    // up-to-date)
...
void CShip::Render()
{
    ...
    if (m_nShipPower == 2)
    {
        ...
        m_pGameEngine->BDrawTexturedQuad(this->GetXPos() - x1, this->GetYPos() - y1, this->GetXPos() + y1,
                                         this->GetYPos() - x1, this->GetXPos() - y1, this->GetYPos() + x1,
                                         this->GetXPos() + x1, this->GetYPos() + y1, 0, 0, 1, 1, shieldColor,
                                         m_hTextureWhite);

        m_pGameEngine->BDrawTexturedQuad(this->GetXPos() - x2, this->GetYPos() - y2, this->GetXPos() + y2,
                                         this->GetYPos() - x2, this->GetXPos() - y2, this->GetYPos() + x2,
                                         this->GetXPos() + x2, this->GetYPos() + y2, 0, 0, 1, 1, shieldColor,
                                         m_hTextureWhite);
    }
    else
    {
...
void CShip::UpdateVibrationEffects()
{
    if (m_ulExplosionTickCount > 0)
    {
        ...
        {
            m_pGameEngine->TriggerControllerVibration((unsigned short)((1.0f - flVibration) * 48000.0f),
                                                      (unsigned short)((1.0f - flVibration) * 24000.0f));
        }
    }
}
