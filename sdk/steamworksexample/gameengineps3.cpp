...
static const char s_npCommunicationSignature[STEAM_PS3_COMMUNICATION_SIG_MAX] = {
    };
...
bool SetSteamPS3Params(SteamPS3Params_t *pParams)
{
    ...
    // 1) look at where the sysutil_game module is loaded to make sure it still needs to be loaded at this point (maybe
    // you can dump it to save memory) 2) if it's being taken care of somewhere else, we don't need to load the module
    // here.
    ...
    if (bSuccess)
    {
        ...
        bSuccess &=
            CELL_GAME_RET_OK == cellGameGetParamString(CELL_GAME_PARAMID_TITLE_ID, gameTitleID, sizeof(gameTitleID));
        bSuccess &=
            CELL_GAME_RET_OK == cellGameGetParamString(CELL_GAME_PARAMID_APP_VER, gameAppVer, sizeof(gameAppVer));
    }
...
{
    // On PS3, we need to save the user's stats & achievement information into the save container. In this example, we
    // are simply saving the data to a known location on disk.
    ...
    if (snprintf(g_rgchUserDataPath, sizeof(g_rgchUserDataPath), "%s/%u_stats.bin", g_SteamPS3Params.m_rgchGameData,
                 unLocalUserID) > sizeof(g_rgchUserDataPath) - 1)
        return false;
...
int main(int argc, char *argv[])
{
    ...
    SteamPS3OverlayRender()->BHostInitialize(pGameEngine->GetViewportWidth(), pGameEngine->GetViewportHeight(), 60,
                                             pGameEngine, NULL);

    // No +connect support on PS3 since Steam isn't launching us, but we check for PSN boot invites, and this may
    // postback a lobby join requested callback to us.
...
void CGameEnginePS3::EndFrame()
{
    ...
    BDrawLine(GetViewportWidth() - flXSafe, flYSafe, dwColor, GetViewportWidth() - flXSafe,
              GetViewportHeight() - flYSafe, dwColor);
    BDrawLine(flXSafe, GetViewportHeight() - flYSafe, dwColor, GetViewportWidth() - flXSafe,
              GetViewportHeight() - flYSafe, dwColor);
...
//-----------------------------------------------------------------------------
bool CGameEnginePS3::BDrawTexturedQuad(float xPos0, float yPos0, float xPos1, float yPos1, float u0, float v0, float u1,
                                       float v1, DWORD dwColor, HGAMETEXTURE hTexture)
{
    return BDrawTexturedGradientQuad(xPos0, yPos0, xPos1, yPos1, u0, v0, u1, v1, dwColor, dwColor, dwColor, dwColor,
                                     hTexture);
}
...
//-----------------------------------------------------------------------------
bool CGameEnginePS3::BDrawTexturedGradientQuad(float xPos0, float yPos0, float xPos1, float yPos1, float u0, float v0,
                                               float u1, float v1, DWORD dwColorTopLeft, DWORD dwColorTopRight,
                                               DWORD dwColorBottomLeft, DWORD dwColorBottomRight, HGAMETEXTURE hTexture)
{
    ...
    if (!m_bShuttingDown)
    {
        ...
        if (ret == CELL_OK)
        {
            ...
            for (int i = 0; i < CELL_PAD_MAX_PORT_NUM; ++i)
            {
                ...
                if ((padInfo.port_status[i] & CELL_PAD_STATUS_CONNECTED) > 0 &&
                    padInfo.device_type[i] == CELL_PAD_DEV_TYPE_STANDARD)
                {
            ...
            if (!bControllerFound)
            {
                ...
                if (GetGameTickCount() - dwLastSpewTime > 3000 || dwLastSpewTime == 0 ||
                ...
                if (ret == CELL_OK && padData.len)
                {
                    if (!SteamPS3OverlayRender()->BHandleCellPadData(padData))
                    {
                        ...
                        if (padData.button[CELL_PAD_BTN_OFFSET_DIGITAL1] & CELL_PAD_CTRL_UP ||
                            padData.button[CELL_PAD_BTN_OFFSET_ANALOG_LEFT_Y] == 0x00)
                        {
                        ...
                        if (padData.button[CELL_PAD_BTN_OFFSET_DIGITAL1] & CELL_PAD_CTRL_DOWN ||
                            padData.button[CELL_PAD_BTN_OFFSET_ANALOG_LEFT_Y] == 0xFF)
                        {
                        ...
                        if (padData.button[CELL_PAD_BTN_OFFSET_DIGITAL1] & CELL_PAD_CTRL_LEFT ||
                            padData.button[CELL_PAD_BTN_OFFSET_ANALOG_LEFT_X] == 0x00)
                        {
                        ...
                        if (padData.button[CELL_PAD_BTN_OFFSET_DIGITAL1] & CELL_PAD_CTRL_RIGHT ||
                            padData.button[CELL_PAD_BTN_OFFSET_ANALOG_LEFT_X] == 0xFF)
                        {
...
//-----------------------------------------------------------------------------
void CGameEnginePS3::DrawTexturedRect(int x0, int y0, int x1, int y1, float u0, float v0, float u1, float v1,
                                      int32 iTextureID, DWORD colorStart, DWORD colorEnd,
                                      EOverlayGradientDirection eDirection)
{
    ...
    if (iter != m_MapSteamTextures.end())
    {
        if (eDirection == k_EOverlayGradientHorizontal)
            BDrawTexturedGradientQuad(x0, y0, x1, y1, u0, v0, u1, v1, colorStart, colorEnd, colorEnd, colorStart,
                                      iter->second);
        else if (eDirection == k_EOverlayGradientVertical)
            BDrawTexturedGradientQuad(x0, y0, x1, y1, u0, v0, u1, v1, colorStart, colorStart, colorEnd, colorEnd,
                                      iter->second);
        else
            BDrawTexturedGradientQuad(x0, y0, x1, y1, u0, v0, u1, v1, colorStart, colorStart, colorStart, colorStart,
                                      iter->second);
    }
    else
    {
...
// Purpose: Part of the render host interface for Steam overlay to draw through
//-----------------------------------------------------------------------------
void CGameEnginePS3::LoadOrUpdateTexture(int32 iTextureID, bool bIsFullTexture, int x0, int y0, uint32 uWidth,
                                         uint32 uHeight, int32 iBytes, char *pData)
{
    BFlushQuadBuffer();
    m_hLastTexture = 0;
