...
- (id)initWithString:(NSString *)aString
            withFont:(NSFont *)inFont
       withTextColor:(NSColor *)color
               inBox:(NSRect *)box
           withFlags:(uint32_t)inFlags;
...
- (NSColor *)
    textColor;    // get the pre-multiplied default text color (includes alpha) string attributes could override this
...
- (void)drawWithBounds:(NSRect)bounds; // will update the texture if required due to change in settings (note context
                                       // should be setup to be orthographic scaled to per pixel scale)
