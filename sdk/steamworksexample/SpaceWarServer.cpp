...
CSpaceWarServer::CSpaceWarServer(IGameEngine *pGameEngine)
{
    ...
    if (!SteamGameServer_Init(unIP, SPACEWAR_AUTHENTICATION_PORT, SPACEWAR_SERVER_PORT, usMasterServerUpdaterPort,
                              eMode, SPACEWAR_SERVER_VERSION))
    {
...
void CSpaceWarServer::AddPlayerShip(uint32 uShipPosition)
{
    ...
    case 0:
        m_rgpShips[uShipPosition] =
            new CShip(m_pGameEngine, true, flXOffset, flYOffset, g_rgPlayerColors[uShipPosition]);
        m_rgpShips[uShipPosition]->SetInitialRotation(flAngle);
        break;
    case 1:
        m_rgpShips[uShipPosition] =
            new CShip(m_pGameEngine, true, flWidth - flXOffset, flYOffset, g_rgPlayerColors[uShipPosition]);
        m_rgpShips[uShipPosition]->SetInitialRotation(-1.0f * flAngle);
        break;
    case 2:
        m_rgpShips[uShipPosition] =
            new CShip(m_pGameEngine, true, flXOffset, flHeight - flYOffset, g_rgPlayerColors[uShipPosition]);
        m_rgpShips[uShipPosition]->SetInitialRotation(PI_VALUE - flAngle);
        break;
    case 3:
        m_rgpShips[uShipPosition] =
            new CShip(m_pGameEngine, true, flWidth - flXOffset, flHeight - flYOffset, g_rgPlayerColors[uShipPosition]);
...
void CSpaceWarServer::SendUpdatedServerDetailsToSteam()
{
    ...
    for (uint32 i = 0; i < MAX_PLAYERS_PER_SERVER; ++i)
    {
        ...
        {
            SteamGameServer()->BUpdateUserData(m_rgClientData[i].m_SteamIDUser, m_rgpShips[i]->GetPlayerName(),
                                               m_rguPlayerScores[i]);
        }
    }
#endif

    // game type is a special string you can use for your game to differentiate different game play types occurring on
    // the same maps When users search for this parameter they do a sub-string search of this string (i.e if you report
    // "abc" and a client requests "ab" they return your server)
