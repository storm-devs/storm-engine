  ...
  public:
    ...
    void Rebuild(const CSteamID &steamIDLobby)
    {
        ...
        for (int i = 0; i < cLobbyMembers; i++)
        {
            ...
            {
                const char *pchReady =
                    SteamMatchmaking()->GetLobbyMemberData(steamIDLobby, steamIDLobbyMember, "ready");
        ...
        {
            const char *pchReady =
                SteamMatchmaking()->GetLobbyMemberData(steamIDLobby, SteamUser()->GetSteamID(), "ready");
            bool bReady = (pchReady && atoi(pchReady) == 1);
...
CLobby::CLobby(IGameEngine *pGameEngine)
    : m_pGameEngine(pGameEngine), m_CallbackPersonaStateChange(this, &CLobby::OnPersonaStateChange),
...
void CLobby::OnLobbyChatUpdate(LobbyChatUpdate_t *pCallback)
{
    ...
    if (pCallback->m_ulSteamIDUserChanged == SteamUser()->GetSteamID().ConvertToUint64() &&
        (pCallback->m_rgfChatMemberStateChange & (k_EChatMemberStateChangeLeft | k_EChatMemberStateChangeDisconnected |
