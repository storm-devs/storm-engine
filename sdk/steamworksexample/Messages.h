...
struct MsgServerSendInfo_t
{
    ...
    {
        return LittleDWord(m_dwMessageType);
    }

    void SetSteamIDServer(uint64 SteamID)
    {
        m_ulSteamIDServer = LittleQWord(SteamID);
    }
    uint64 GetSteamIDServer()
    {
        return LittleQWord(m_ulSteamIDServer);
    }

    void SetSecure(bool bSecure)
    {
        m_bIsVACSecure = bSecure;
    }
    bool GetSecure()
    {
        return m_bIsVACSecure;
    }

    void SetServerName(const char *pchName)
    {
        strncpy_safe(m_rgchServerName, pchName, sizeof(m_rgchServerName));
    }
    const char *GetServerName()
    {
        return m_rgchServerName;
    }
...
struct MsgServerPassAuthentication_t
{
    ...
    void SetPlayerPosition(uint32 pos)
    {
        m_uPlayerPosition = LittleDWord(pos);
    }
    uint32 GetPlayerPosition()
    {
        return LittleDWord(m_uPlayerPosition);
    }
...
struct MsgServerUpdateWorld_t
{
    ...
    ServerSpaceWarUpdateData_t *AccessUpdateData()
    {
        return &m_ServerUpdateData;
    }
...
struct MsgClientBeginAuthentication_t
{
    ...
    uint32 GetTokenLen()
    {
        return LittleDWord(m_uTokenLen);
    }
    ...
    {
        m_ulSteamID = LittleQWord(ulSteamID);
    }
    uint64 GetSteamID()
    {
        return LittleQWord(m_ulSteamID);
    }
...
struct MsgClientSendLocalUpdate_t
{
    ...
    void SetShipPosition(uint32 uPos)
    {
        m_uShipPosition = LittleDWord(uPos);
    }
    ClientSpaceWarUpdateData_t *AccessUpdateData()
    {
        return &m_ClientUpdateData;
    }
...
struct MsgP2PSendingTicket_t
{
    ...
    uint32 GetTokenLen() const
    {
        return LittleDWord(m_uTokenLen);
    }
    const char *GetTokenPtr() const
    {
        return m_rgchToken;
    }
...
struct MsgVoiceChatData_t
{
    ...
    DWORD GetMessageType() const
    {
        return LittleDWord(m_dwMessageType);
    }

    void SetDataLength(uint32 unLength)
    {
        m_uDataLength = LittleDWord(unLength);
    }
    uint32 GetDataLength() const
    {
        return LittleDWord(m_uDataLength);
    }

    void SetSteamID(CSteamID steamID)
    {
        from_steamID = steamID;
    }
