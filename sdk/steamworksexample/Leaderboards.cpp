...
class CLeaderboardMenu : public CBaseMenu<LeaderboardMenuItem_t>
{
    ...
    LeaderboardEntry_t m_leaderboardEntries[k_nMaxLeaderboardEntries]; // leaderboard entries we received from
    ...
    void ShowLeaderboard(SteamLeaderboard_t hLeaderboard, ELeaderboardDataRequest eLeaderboardData, int offset)
    {
        ...
        if (hLeaderboard)
        {
            ...
            SteamAPICall_t hSteamAPICall = SteamUserStats()->DownloadLeaderboardEntries(
                hLeaderboard, eLeaderboardData, offset, offset + k_nMaxLeaderboardEntries);
    ...
    void Rebuild()
    {
        ...
        else
        {
            ...
            for (int index = 0; index < m_nLeaderboardEntries; index++)
            {
                ...
                sprintf_safe(rgchMenuText, "(%d) %s - %d", m_leaderboardEntries[index].m_nGlobalRank, pchName,
...
void CLeaderboards::FindLeaderboards()
{
    ...
    if (!m_hQuickestWinLeaderboard)
    {
        ...
        hSteamAPICall = SteamUserStats()->FindOrCreateLeaderboard(
            LEADERBOARD_QUICKEST_WIN, k_ELeaderboardSortMethodAscending, k_ELeaderboardDisplayTypeTimeSeconds);
    }
    else if (!m_hFeetTraveledLeaderboard)
    {
        // find/create a leaderboard for the most feet traveled in 1 round
        hSteamAPICall = SteamUserStats()->FindOrCreateLeaderboard(
            LEADERBOARD_FEET_TRAVELED, k_ELeaderboardSortMethodDescending, k_ELeaderboardDisplayTypeNumeric);
    }
...
void CLeaderboards::OnFindLeaderboard(LeaderboardFindResult_t *pFindLeaderboardResult, bool bIOFailure)
{
    ...
    // if the user is currently looking at a leaderboard, it might be one we didn't have a handle for yet. Update the
    // leaderboard.
...
{
    // if the user won, update the leaderboard with the time it took. If the user's previous time was faster, this time
    // will be thrown out.
    if (m_hQuickestWinLeaderboard && SpaceWarClient()->BLocalPlayerWonLastGame())
    {
        SteamAPICall_t hSteamAPICall =
            SteamUserStats()->UploadLeaderboardScore(m_hQuickestWinLeaderboard, k_ELeaderboardUploadScoreMethodKeepBest,
                                                     (int)pStats->GetGameDurationSeconds(), NULL, 0);
        m_SteamCallResultUploadScore.Set(hSteamAPICall, this, &CLeaderboards::OnUploadScore);
    }

    // update the leaderboard for the most feet traveled in 1 round. If the user previously traveled farther in a round
    // than this one, this value will be thrown out
    if (m_hFeetTraveledLeaderboard)
    {
        SteamUserStats()->UploadLeaderboardScore(m_hFeetTraveledLeaderboard, k_ELeaderboardUploadScoreMethodKeepBest,
                                                 (int)pStats->GetGameFeetTraveled(), NULL, 0);
    }
}
