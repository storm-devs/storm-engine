...
static int RealMain(const char *pchCmdLine, HINSTANCE hInstance, int nCmdShow)
{
    ...
    _snprintf(rgchFullPath, sizeof(rgchFullPath), "%s/steamworksexample.app/Contents/Resources/%s", rgchCWD,
              "controller.vdf");
#else
    _snprintf(rgchFullPath, sizeof(rgchFullPath), "%s/%s", rgchCWD, "controller.vdf");
#endif
    ...
    {
        // no connect string on process command line. If app was launched via a Steam URL, the extra command line
        // parameters in that URL get be retrieved with GetLaunchCommandLine. This way an attacker can't put malicious
        // parameters in the process command line which might allow much more functionality then indented.
