...
class PARTICLE_SYSTEM_INTERFACE : VPARTICLE_SYSTEM
{
  ...
  public:
    ...
    {
        if (pClass)
            pClass->InterfaceClassDestructor(this);
    }
    ...
    {
        if (pClass)
            pClass->SetEmitter(p, a);
    }
    void LinkToObject(ENTITY_ID id, CVECTOR _LinkPos)
    {
        if (pClass)
            pClass->LinkToObject(id, _LinkPos);
    }
    void SetDelay(long _delay)
    {
        if (pClass)
            pClass->SetDelay(_delay);
    }
    void SetLifeTime(dword time)
    {
        if (pClass)
            pClass->SetLifeTime(time);
    }
    void StopEmitter()
    {
        if (pClass)
