...
void WdmLocations::IslandInfo::Update(float dltTime, WdmLocations *loc)
{
    ...
    for (long i = 0; i < WDM_LOCATIONS_MAXLOC; i++)
    {
        ...
        if (locations[i].model)
            locations[i].model->alpha = locations[i].alpha;
    }
}
...
WdmLocations::~WdmLocations()
{
    ...
    if (iconTexture >= 0)
        wdmObjects.rs->TextureRelease(iconTexture);
}
...
void WdmLocations::SetIslandsData(ATTRIBUTES *apnt)
{
    ...
    for (long i = 0; i < numIslands; i++)
    {
        ...
        if (a)
        {
            ...
            if (islandsInfo[i].kRadius < 0.2f)
                islandsInfo[i].kRadius = 0.2f;
            if (islandsInfo[i].kRadius > 0.99f)
                islandsInfo[i].kRadius = 0.99f;
        }
...
void WdmLocations::InitLabel(Label &label, ATTRIBUTES *apnt, bool isIsl, bool isSet)
{
    ...
    if (isSet)
        label.SetVisible(vf ? stricmp(vf, "true") == 0 : false);
    else
        label.isVisible = vf ? stricmp(vf, "true") == 0 : false;
}
...
void WdmLocations::UpdateLabelPriority(IslandInfo &ii)
{
    ...
    //Острова
    if (ii.label[0].IsHave())
        ii.labelPrt[ii.numLabels++] = 0;
    //Локаций
    for (i = 0; i < WDM_LOCATIONS_MAXLOC; i++)
    {
        float maxRad;
        ...
        if (k >= 0)
        {
            ...
            if (ii.label[k + 1].IsHave())
                ii.labelPrt[ii.numLabels++] = k + 1;
            rad[k] = -1;
        }
        else
            break;
...
void WdmLocations::Update(float dltTime)
{
    ...
    for (long i = 0; i < numIslands; i++)
    {
        ...
        if (wdmObjects.isDebug)
        {
            ...
            for (long j = 0; j < WDM_LOCATIONS_MAXLOC; j++)
            {
                if (!islandsInfo[i].locations[j].model)
                    continue;
                if (!islandsInfo[i].locations[j].isDebugMove)
                    continue;
                ...
                else if (GetAsyncKeyState(VK_PRIOR) < 0)
                {
                    ...
                    islandsInfo[i].locations[j].model->mtx.BuildMatrix(
                        CVECTOR(0.0f, islandsInfo[i].locations[j].ay, 0.0f),
                        CVECTOR(islandsInfo[i].locations[j].model->mtx.Pos()));
                }
                ...
                {
                    api->Trace("i:%i l:%i  (%f, %f, %f) ay:%f", i, j, islandsInfo[i].locations[j].model->mtx.Pos().x,
                               islandsInfo[i].locations[j].model->mtx.Pos().y,
                               islandsInfo[i].locations[j].model->mtx.Pos().z, islandsInfo[i].locations[j].ay);
                }
            }
        }
#endif
...
void WdmLocations::LRender(VDX8RENDER *rs)
{
    ...
    if (islandsInfo && wdmObjects.isDebug)
    {
        for (long i = 0; i < numIslands; i++)
        {
            wdmObjects.DrawCircle(CVECTOR(islandsInfo[i].x, 0.1f, islandsInfo[i].z), sqrtf(islandsInfo[i].radius2),
                                  0x4fff0000);
            wdmObjects.DrawCircle(CVECTOR(islandsInfo[i].x, 0.1f, islandsInfo[i].z),
                                  sqrtf(islandsInfo[i].radius2) * islandsInfo[i].kRadius, 0x4f00ff00);
        }
    }
    //Рисуем метки
    DrawLabels(rs);
...
void WdmLocations::DrawLabels(VDX8RENDER *rs)
{
    ...
    for (long i = 0; i < numIslands; i++)
    {
        ...
        if (island.radius2 < 0.0001f)
            continue;
        if (island.numLabels <= 0)
            continue;
        ...
        {
            if (!island.label[vrt[j].label].IsVisible())
                continue;
            ...
            for (long k = 0; k < numRects; k++)
            {
                ...
                float ddy = 0.0f;
                if (dx == 0)
                    dx = ((lbr[k].r + lbr[k].l) < (r + l)) ? 1 : -1;
                if (dy == 0)
                    dy = ((lbr[k].b + lbr[k].t) < (b + t)) ? 1 : -1;
                if (dx > 0)
                    ddx = lbr[k].r - l;
                else
                    ddx = lbr[k].l - r;
...
bool WdmLocations::FindGoodIsland(float shipX, float shipZ, float &x, float &z, float &r, bool testInZone,
                                  bool retPivot)
{
    ...
    for (long i = 0, j = -1; i < numIslands; i++)
    {
        ...
        float d = dx * dx + dz * dz;
        if (testInZone && d >= islandsInfo[i].radius2)
            continue;
        if (j < 0)
        {
            j = i;
...
void WdmLocations::DrawIcon(VDX8RENDER *rs, float x, float y, Label &label)
{
    ...
    dword color;
    if (label.t[0].icon < 0 && label.t[1].icon < 0)
        return;
    ...
    if (label.t[1].icon >= 0 && label.t[1].alpha > 0.0f)
    {
        ...
        if (color & 0xff000000)
        {
            ...
            drawbuf[4] = drawbuf[2];
            drawbuf[5] = drawbuf[1];
            rs->DrawPrimitiveUP(D3DPT_TRIANGLELIST, D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_TEX1, 2, drawbuf,
