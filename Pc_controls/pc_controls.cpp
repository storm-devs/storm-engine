...
void PC_CONTROLS::AppState(bool state)
{
    ...
    if (pDevices)
    {
        for (n = 0; n < nDevicesNum; n++)
            pDevices[n]->SetAppState(state);
    }
}
...
void PC_CONTROLS::AddSystemControl(SYSTEM_CONTROL_DESC &_control_desc_struct)
{
    ...
    nSystemControlsNum++;
    pSystemControlsTable =
        (SYSTEM_CONTROL_ELEMENT *)RESIZE(pSystemControlsTable, nSystemControlsNum * sizeof(SYSTEM_CONTROL_ELEMENT));
