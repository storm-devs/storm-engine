...
#ifndef _XBOX
#define CACHED_BUFFER_CAPS                                                                                             \
    DSBCAPS_CTRL3D | DSBCAPS_CTRLVOLUME | DSBCAPS_CTRLPOSITIONNOTIFY | DSBCAPS_GETCURRENTPOSITION2 |                   \
        DSBCAPS_MUTE3DATMAXDISTANCE /*| DSBCAPS_LOCSOFTWARE*/
#else
#define CACHED_BUFFER_CAPS                                                                                             \
    DSBCAPS_CTRL3D | DSBCAPS_CTRLVOLUME | DSBCAPS_CTRLPOSITIONNOTIFY | DSBCAPS_MUTE3DATMAXDISTANCE
#endif

//--------------------------------------------------------------------
PCM3DSound::PCM3DSound(LPDIRECTSOUND8 _ds8, bool _looped, bool _cached, eVolumeType _volumeType) : VSound(_volumeType)
{
...
//--------------------------------------------------------------------
bool PCM3DSound::Play(const char *_name, bool _simpleCache /* = false*/, long _time /* = 0*/,
                      const CVECTOR *_startPosition /* = 0*/, float _minDistance /*= -1.0f*/,
                      float _maxDistance /*= -1.0f*/, long _loopPauseTime /*= 0*/, float _volume /*= 1.0f*/)
{
    ...
    else
    {
        ...
        if (!CreateBuffer(CACHED_BUFFER_CAPS, dataSize, waveFormat, cachedData))
            return false;
...
//--------------------------------------------------------------------
PCM3DSound::PCM3DSound(const PCM3DSound &_source) : VSound(_source.volumeType)
{
