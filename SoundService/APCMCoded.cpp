...
bool ReadWAVHeader(HANDLE _file, WAVEFORMATEX *_waveFormat, DWORD *_size)
{
    ...
    {
        if (pcv[n] == 'd' && pcv[n + 1] == 'a' && pcv[n + 2] == 't' && pcv[n + 3] == 'a')
            break;
    }
...
//--------------------------------------------------------------------
APCMCoded::APCMCoded() : cachedI(INVALID_I)
{
}
...
// *** READ WAV DATA : PC ***
    ...
    Assert(_cachedData->size == buffer1Size) memcpy(buffer1, (void *)_cachedData->pcmData, _cachedData->size);
    /*
...
//--------------------------------------------------------------------
bool APCMCoded::OpenWAVFile(const char *_filename, HANDLE *_inFile, WAVEFORMATEX *_waveFormat, DWORD *_dataSize,
