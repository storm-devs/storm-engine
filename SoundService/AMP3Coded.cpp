...
//--------------------------------------------------------------------
AMP3Coded::AMP3Coded()
    : soundNotify(0), inFile(INVALID_HANDLE_VALUE), validMP3(false), decodedData(0), codedData(0), notifyEvent(0),
      notifyEvent2(0), flushFirst(true), decodedStream(0), firstEventSignaled(0), newDecodeStage(true),
      bytesDoneOnStage(0), finishedDecoding(false), bytesPlayed(0), bytesAll(1), leftFlushes(-1)
{
...
bool AMP3Coded::LoadDataToBuffer(LPDIRECTSOUNDBUFFER _buffer, MMCKINFO &_info)
{
    ...
    DSBPOSITIONNOTIFY notifyPositions[2] = {{0, notifyEvent}, {MP3_BUFFER_SIZE, notifyEvent2}}; //{, endEvent}};
...
bool AMP3Coded::AddAndDecodeMP3Data(bool _looped)
{
    ...
    Assert(inFile)
        ...
        if (!validMP3 || !decodedStream) return false;
...
bool AMP3Coded::FlushDecodedToSoundBuffer(LPDIRECTSOUNDBUFFER _buffer, bool _first, bool _looped)
{
    ...
    if (leftFlushes != -1)
    {
        if (!leftFlushes)
        {
            if (core->looped)
            {
                ...
                outDSCode = _buffer->Lock(0, 2 * MP3_BUFFER_SIZE, &buffer1, &buffer1Size, &buffer2, &buffer2Size, 0);
                if (outDSCode == DS_OK)
                {
    ...
    flushFirst = _first;
    if (flushFirst)
        outDSCode = _buffer->Lock(0, MP3_BUFFER_SIZE, &buffer1, &buffer1Size, &buffer2, &buffer2Size, 0);
    else
        outDSCode = _buffer->Lock(MP3_BUFFER_SIZE, MP3_BUFFER_SIZE, &buffer1, &buffer1Size, &buffer2, &buffer2Size, 0);
    ...
    if (outDSCode != DS_OK)
    {
        ...
        if (flushFirst)
            ...
            outDSCode =
                _buffer->Lock(MP3_BUFFER_SIZE, MP3_BUFFER_SIZE, &buffer1, &buffer1Size, &buffer2, &buffer2Size, 0);
    }
...
float AMP3Coded::GetPosition()
{
    ...
    DWORD playPosition = 0, writePosition;
    if (core->dsBuffer)
