...
bool AIShipCannonController::Fire2Position(dword dwBort, CVECTOR &vFirePos, float fFireHeight)
{
    ...
    api->Event(SHIP_BORT_FIRE, "lisffffff", GetIndex(GetAIShip()->GetACharacter()), GetAIShip()->GetShipEID(),
               (char *)aShipBorts[dwBort].sName.GetBuffer(), vTempFirePos.x, vTempFirePos.y, vTempFirePos.z,
               vTempFireDir.x, vTempFireDir.y, vTempFireDir.z);
...
void AIShipCannonController::Execute(float fDeltaTime)
{
    ...
    for (i = 0; i < aShipBorts.Size(); i++)
        if (aShipBorts[i].aCannons.Size())
        {
            ...
            if (dwReadyCannonsBefore != dwReadyCannonsAfter &&
                dwReadyCannonsAfter == GetBortIntactCannonsNum(i)) // pBort->fChargePercent >= 0.999f)
            {
            ...
            // if (GetAIShip()->isMainCharacter())
            {
                ...
                pACurBort->SetAttributeUseFloat(
                    "DamageRatio", 1.0f - ((float)GetBortIntactCannonsNum(i) + (float)GetBortDisabledCannonsNum(i)) /
        ...
        for (i = 0; i < dword(info.nlabels); i++)
        {
            ...
            for (j = 0; j < aShipBorts.Size(); j++)
            {
                ...
                // if (aShipBorts[j] == label.group_name) /// Акелла , блять ну кто так пишет код ??????
                {
                    ...
                    if (pADamages != NULL)
                        fDamage = pADamages->GetAttributeAsFloat(str, 0.0f);
                    else
                        fDamage = 0.0f;
                    if (pADamages != NULL)
...
bool AIShipCannonController::isHaveEnoughtBallsForBort(dword dwBortIdx)
{
    ...
    Assert(pvData);
    if ((long)GetBortIntactCannonsNum(dwBortIdx) > pvData->GetLong())
        return false;
    return true;
}
