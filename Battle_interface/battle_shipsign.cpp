    ...
    for (long n = 0; n < m_nShipQuantity; n++)
        if (m_Ship[n].pASailorQuantity)
            m_pRS->ExtPrint(m_idSailorFont, m_dwSailorFontColor, 0, ALIGN_CENTER, false, m_fSailorFontScale, 0, 0,
                            ...
                            (long)m_Ship[n].pntPos.y + m_SailorFontOffset.y, "%s",
                            m_Ship[n].pASailorQuantity->GetThisAttr());
...
void BIShipIcon::UpdateBuffers(long nShipQ)
{
        ...
        (m_nBackSquareQ + m_nShipStateSquareQ + m_nShipClassSquareQ + m_nShipSquareQ))
    {
        ...
        VERTEX_BUFFER_RELEASE(m_pRS, m_nVBufID);
        m_nVBufID = m_pRS->CreateVertexBufferMananged(
    ...
    if (pV)
    {
        ...
        for (n = 0; n < m_nShipQuantity; n++)
        {
            vn += WriteSquareToVBuffWithProgress(&pV[vn], m_rShipHPUV, m_dwShipStateColor,
                                                 ...
                                                 1.f - GetRelativeShipHP(n), 0.f, 0.f, 0.f);
            vn += WriteSquareToVBuffWithProgress(&pV[vn], m_rShipSPUV, m_dwShipStateColor,
                                                 ...
                                                 1.f - GetRelativeShipSP(n), 0.f, 0.f, 0.f);
        }

        for (n = 0; n < m_nShipQuantity; n++)
            vn += WriteSquareToVBuff(&pV[vn], m_rShipClassUV, m_dwShipClassColor,
                                     m_Ship[n].pntPos + m_pntShipClassOffset, m_pntShipClassIconSize);

        // m_nShipQuantity = 0;
        /*for( n=0; n<m_nShipQuantity; n++ )
            vn += WriteSquareToVBuff( &pV[vn], m_rShipUV, m_dwShipColor, m_Ship[n].pntPos+m_pntShipOffset,
           m_pntShipIconSize );*/
