...
BIShipCommandList::BIShipCommandList(ENTITY_ID &eid, ATTRIBUTES *pA, VDX8RENDER *rs)
    : BICommandList(eid, pA, rs), m_aChargeQuantity(_FL)
{
    Init();
}
    ...
    for (; sd != null; sd = sd->next)
    {
        if (sd != selShip && sd != main_sd)
        {
                ...
                !bMyShip && bNeutral && !sd->isMyShip && sd->relation == BI_RELATION_NEUTRAL)
            {
                ...
                n = AddToIconList(sd->textureNum, sd->pictureNum, sd->selectPictureNum, -1, sd->characterIndex, null,
                                  -1, null, sd->pAttr->GetAttribute("name"));
                if (n > 0)
                {
                    retVal += n;
...
long BIShipCommandList::FortAdding(bool allLabel, bool bFriend, bool bNeutral, bool bEnemy)
{
    ...
    do
    {
            ...
            bEnemy && pL->relation == BI_RELATION_ENEMY)
        {
            ...
            retVal += AddToIconList(pL->texIdx, pL->picIdx, pL->selPicIdx, -1, pL->characterIndex, null, -1,
...
long BIShipCommandList::LandAdding(bool allLabel)
{
    ...
    do
    {
        ...
        if (pL->pA != null)
            pLocName = pL->pA->GetAttribute("name");
        retVal += AddToIconList(pL->texIdx, pL->picIdx, pL->selPicIdx, -1, pL->characterIndex, null, -1, pLocName,
                                pL->pchr_note);
    } while ((pL = g_IslandDescr.GetNext()) != null);
    return retVal;
}
...
long BIShipCommandList::CommandAdding()
{
    ...
    for (long i = 0; i < attrQuant; i++)
    {
        ...
        retVal += AddToIconList(texNum, pictureNum, selPictureNum, cooldownPictureNum, -1, eventName, -1, null,
...
long BIShipCommandList::UserIconsAdding()
{
    ...
    for (long i = 0; i < attrQuant; i++)
    {
        ...
        retVal += AddToIconList(textureNum, pictureNum, selPictureNum, -1, -1, null, i + 1, pA->GetAttribute("name"),
                                pA->GetAttribute("note"));
    }
...
long BIShipCommandList::AbilityAdding()
{
    ...
    for (long i = 0; i < attrQuant; i++)
    {
        ...
        long cooldownPictureNum = pA->GetAttributeAsDword("cooldownPicNum", -1);
        char *eventName = pA->GetAttribute("event");
        retVal += AddToIconList(textureNum, pictureNum, selPictureNum, cooldownPictureNum, -1, eventName, -1, null,
                                pA->GetAttribute("note"));
    }
...
long BIShipCommandList::AddCancelIcon()
{
    ...
    long selPictureNum = pA->GetAttributeAsDword("selPicNum", 0);
    long textureNum = pA->GetAttributeAsDword("texNum", -1);
    return AddToIconList(textureNum, pictureNum, selPictureNum, -1, -1, pA->GetAttribute("event"), -1, null,
                         pA->GetAttribute("note"));
}
