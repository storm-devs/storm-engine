...
class BIImageRender
{
  public:
    ...
    IBIImage *CreateImage(BIImageType type, const char *pcTextureName, dword color, FRECT &uv, long nLeft, long nTop,
                          long nRight, long nBottom, long nPrior = ImagePrioritet_DefaultValue,
                          const char *pcTechniqueName = 0);
    IBIImage *CreateImage(BIImageType type, const char *pcTextureName, dword color, FRECT &uv, RECT &pos,
                          long nPrior = ImagePrioritet_DefaultValue, const char *pcTechniqueName = 0);
    ...
    IBIString *CreateString(const char *text, const char *font_name, float font_scale, dword font_color, long valign,
