...
void BATTLE_NAVIGATOR::Draw()
{
    ...
    rs->ExtPrint(m_speedFont, 0xFFFFFFFF, 0, ALIGN_CENTER, true, m_fFontScale, 0, 0, m_xWindSpeed, m_ySpeedShow, "%.1f",
                 m_fWindStrength);
    rs->ExtPrint(m_speedFont, 0xFFFFFFFF, 0, ALIGN_CENTER, true, m_fFontScale, 0, 0, m_xShipSpeed, m_ySpeedShow, "%.1f",
                 m_fShipSpeed);
...
void BATTLE_NAVIGATOR::Update()
{
    ...
    if (m_wb == NULL)
    {
        ...
        fWindAngle = m_wb->GetFloat(whf_wind_angle);
    ...
    if (pV != NULL)
    {
        SetRectangleVertexPos(&pV[8], (float)m_XNavigator, (float)m_YNavigator, (float)m_NavigationWidth,
                              (float)m_NavigationWidth, -m_fAngle);
        SetRectangleVertexPos(&pV[12], (float)m_XNavigator, (float)m_YNavigator, (float)m_windWidth,
                              (float)m_windHeight, fWindAngle - m_fAngle);
...
{
    if (pv == NULL || pShip == NULL || pChar == NULL || pstr == NULL)
        return;
...
void BATTLE_NAVIGATOR::Init(VDX8RENDER *RenderService)
{
    ...
    if (tmpstr == null)
        ...
        m_idChargeBackTexture = rs->TextureCreate(tmpstr);
    ...
    // create buffers
    m_idEmptyVBuf = rs->CreateVertexBufferManaged(BI_ONETEX_VERTEX_FORMAT,
                                                  (4 + 4 + 4 + 4) * sizeof(BI_ONETEXTURE_VERTEX), D3DUSAGE_WRITEONLY);
    m_idCannonVBuf = rs->CreateVertexBufferManaged(BI_ONETEX_VERTEX_FORMAT, 7 * 4 * sizeof(BI_ONETEXTURE_VERTEX),
                                                   D3DUSAGE_WRITEONLY);
    m_idSpeedVBuf = rs->CreateVertexBufferManaged(BI_ONETEX_VERTEX_FORMAT, 7 * 2 * sizeof(BI_ONETEXTURE_VERTEX),
                                                  D3DUSAGE_WRITEONLY);
    m_idMapVBuf = rs->CreateVertexBufferManaged(
        BI_ONETEX_VERTEX_FORMAT, (RADIAL_QUANTITY + 2) * sizeof(BI_ONETEXTURE_VERTEX), D3DUSAGE_WRITEONLY);
    m_idFireZoneVBuf = rs->CreateVertexBufferManaged(
        BI_NOTEX_VERTEX_FORMAT, FIRERANGE_QUANTITY * sizeof(BI_NOTEXTURE_VERTEX), D3DUSAGE_WRITEONLY);
    m_idShipsVBuf = rs->CreateVertexBufferManaged(
        BI_COLORONLY_VERTEX_FORMAT, MAX_ENEMY_SHIP_QUANTITY * 3 * sizeof(BI_COLORONLY_VERTEX), D3DUSAGE_WRITEONLY);
    m_idGradBackVBuf =
        rs->CreateVertexBufferManaged(BI_COLORONLY_VERTEX_FORMAT, 3 * sizeof(BI_COLORONLY_VERTEX), D3DUSAGE_WRITEONLY);
    m_idCurChargeVBuf =
        rs->CreateVertexBufferManaged(BI_ONETEX_VERTEX_FORMAT, 4 * sizeof(BI_ONETEXTURE_VERTEX), D3DUSAGE_WRITEONLY);
    if (m_idEmptyVBuf == -1 || m_idCannonVBuf == -1 || m_idSpeedVBuf == -1 || m_idMapVBuf == -1 ||
    ...
    if (pV != NULL)
    {
        ...
        SetRectangleVertexPos(&pV[4], (float)m_ChargeBackOffset.x, (float)m_ChargeBackOffset.y,
                              (float)m_ChargeBackSize.x, (float)m_ChargeBackSize.y);
    ...
    if (pV != NULL)
    {
        ...
        SetRectangleVertexPos(pV, (float)m_ChargePos.x, (float)m_ChargePos.y, (float)m_ChargeSize.x,
                              (float)m_ChargeSize.y);
...
{
    rs->CreateTexture(MAP_TEXTURE_WIDTH, MAP_TEXTURE_HEIGHT, 1, D3DUSAGE_RENDERTARGET, D3DFMT_R5G6B5, D3DPOOL_DEFAULT,
                      &m_pIslandTexture);
}
