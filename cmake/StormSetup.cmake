...
macro(_get_relative_path_folder out path)
    ...
    file(RELATIVE_PATH PROJECT_RELATIVE_PATH "${CMAKE_CURRENT_SOURCE_DIR}" "${dir}")

    set(base_folders "${SRC_DIRS};${PUBLIC_INCLUDE_DIRS};TESTSUITE_DIRS")
    list(JOIN base_folders "|" folders_match)

    string(REGEX REPLACE "((${folders_match})\/?)" "" ${out} "${PROJECT_RELATIVE_PATH}")
endmacro()
...
macro(_collect_sources)
    ...
    message(STATUS "[StormSetup] Collected sources for \"${_SETUP_TARGET_NAME}\": ${SRCS} (testsuite: ${TEST_SRCS})")
endmacro()
...
macro(STORM_SETUP)
    ...
    if(_SETUP_HAS_TESTSUITE)
	    ...
	    target_link_libraries(${test_target} PRIVATE ${_SETUP_TEST_DEPENDENCIES} ${_SETUP_TARGET_NAME})
