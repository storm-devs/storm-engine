...
bool CXI_TITLE::Init(INIFILE *ini1, char *name1, INIFILE *ini2, char *name2, VDX8RENDER *rs, XYRECT &hostRect,
                     XYPOINT &ScreenSize)
{
...
void CXI_TITLE::LoadIni(INIFILE *ini1, char *name1, INIFILE *ini2, char *name2)
{
    ...
    if (ReadIniString(ini1, name1, ini2, name2, "titleCenter", param, sizeof(param), ""))
        ...
        ZeroMemory(&centerRect, sizeof(centerRect));
    ...
    if (pIndex == NULL)
        SE_THROW_MSG("index buffer not create")
    ...
    if (pVert == NULL)
        SE_THROW_MSG("vertex buffer not create")
    for (i = 0; i < m_nVert; i++)
    {
