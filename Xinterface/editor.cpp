...
bool GIEditor::MakeControl()
{
    ...
    if (!m_bShowMode)
    {
        if (GetAsyncKeyState(VK_CONTROL) < 0 && GetAsyncKeyState(VK_SHIFT) < 0 && GetAsyncKeyState('E') < 0)
        {
            m_bShowMode = true;
            return true;
        }
    ...
    if (m_bShowMode)
    {
        if (GetAsyncKeyState(VK_CONTROL) < 0 && GetAsyncKeyState(VK_SHIFT) < 0 && GetAsyncKeyState('Q') < 0)
            m_bShowMode = false;
        return true;
    }
    ...
    if (GetAsyncKeyState(VK_CONTROL) < 0 && GetAsyncKeyState(VK_SHIFT) < 0 && GetAsyncKeyState('S') < 0)
    {
    ...
    if (bMove || bSize)
    {
        ...
        if (nHorz != 0 || nVert != 0)
        {
            ...
            if (nHorz != 0)
            {
                ...
                if (rectNew.right < 0)
                    rectNew.right = 0;
                if (rectNew.left > (long)m_pGIOwner->GetScreenWidth())
                    rectNew.left = m_pGIOwner->GetScreenWidth();
            ...
            if (nVert != 0)
            {
                ...
                if (rectNew.bottom < 0)
                    rectNew.bottom = 0;
                if (rectNew.top > (long)m_pGIOwner->GetScreenHeight())
                    rectNew.top = m_pGIOwner->GetScreenHeight();
