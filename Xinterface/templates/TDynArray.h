...
template <class T> class TDynamicArray
{
  ...
  public:
    ...
    void ResizeBuffer(dword dwNewSize)
    {
        ...
        if (dwNeedReserv == m_dwReservSize)
            return; // same size
        T *pOldBuf = m_pBuffer;
    ...
    {
        if (idx < 0 || idx >= (int)GetSize())
            return; // out of rang
        ResizeBuffer(m_dwSize + 1);
        for (int n = m_dwSize; n > idx; n--)
    ...
    void SimpleDelete(T one)
    {
        Delete(Find(one));
    }
    void Delete(int n, int q = 1)
    {
        if (n < 0 || n >= (int)m_dwSize)
            return; // out of range
        m_dwSize -= q;
        for (int i = n; i < (int)m_dwSize; i++)
            m_pBuffer[i] = m_pBuffer[i + q];
        ResizeBuffer(m_dwSize);
    }
    void Replace(int idx, T &one)
    {
        if (idx < 0 || idx >= (int)m_dwSize)
            return; // out of range
        m_pBuffer[i] = one;
    }

    // find function
    int SimpleFind(T one)
    {
        return Find(one);
    }
    int Find(T &one)
    ...
    dword GetIndex(T *pT)
    {
        if ((dword)pT < (dword)m_pBuffer || (dword)pT > (dword)&m_pBuffer[m_dwSize - 1])
            return -1;
