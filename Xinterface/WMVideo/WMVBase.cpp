...
bool WMVBase::Initialize(IDirect3DDevice8 *pd3dDev, const char *cWMVfileName)
{
    ...
    */
    ...
    for (int i = 0; i < 2; i++)
    {
        m_pd3dDevice->CreateTexture(m_wmvVideoInfo.Width, m_wmvVideoInfo.Height, 1, 0, D3DFMT_YUY2, 0, &m_pOverlay[i]);
...
bool WMVBase::FileOpen(const char *cWMVfileName)
{
    ...
    hr = XMVDecoder_CreateDecoderForFile(0, cWMVfileName, &m_pWMVDecoder);
...
HRESULT WMVBase::DecodeNext()
{
    ...
    if (wmvResult == XMV_FAIL || wmvResult == XMV_ENDOFFILE)
        return S_FALSE;
