...
bool SEA::CalculateBumpMapsGF2MX()
{
    char str[1024];
    long iTexIndex;
    HANDLE hFile;
    ...
    // delete old bumps
    for (dword i = 0; i < aGF2BumpMaps.Size(); i++)
        pRS->TextureRelease(aGF2BumpMaps[i]);
    aGF2BumpMaps.DelAll();

    // load new bumps
    do
    {
        sprintf(str, "resource\\textures\\%sSeaBump%.4d.tga.tx", (const char *)SeaData.GF2MX.sBumpDir,
                aGF2BumpMaps.Size());
...
bool SEA::CalculateBumpMapsGF3()
{
    ...
    // delete old bumps
    for (dword i = 0; i < aBumpMaps.Size(); i++)
        pRS->Release(aBumpMaps[i]);
    aBumpMaps.DelAll();
    ...
    while (true)
    {
        ...
        else if (dwIdx == dwNumBumps)
            break;

        IDirect3DTexture9 **pBumpMap = &aBumpMaps[aBumpMaps.Add()];

        hr = pRS->CreateTexture(SeaData.dwBumpSize, SeaData.dwBumpSize, SeaData.dwBumpMipLevels, 0, D3DFMT_A1R5G5B5,
                                D3DPOOL_MANAGED, pBumpMap);
...
#define GET_MASSIVE(dx, dy)                                                                                            \
    float(                                                                                                             \
        pMassive[((x + dx) & (SeaData.dwBumpSize - 1)) + ((y + dy) & (SeaData.dwBumpSize - 1)) * SeaData.dwBumpSize] & \
        0xFF) /                                                                                                        \
        255.0f
                    ...
                    for (x = 0; x < d3dsd.Width; x++)
                    {
                        ...
                        for (y1 = y * dwMult; y1 < (y + 1) * dwMult; y1++)
                            for (x1 = x * dwMult; x1 < (x + 1) * dwMult; x1++)
                                vTmp += pVV[(x1 & (SeaData.dwBumpSize - 1)) +
                                            (y1 & (SeaData.dwBumpSize - 1)) * SeaData.dwBumpSize];
...
void SEA::SetBumpTransformConstants()
{
    float _fFov;
    CVECTOR _vPos, _vAng;
    ...
    pRS->SetVertexShaderConstant(CV_FOG, &CVECTOR4(SeaData.fFogSeaDensity, SeaData.fFogStartDistance, 255.0f, 255.0f),
                                 1);
