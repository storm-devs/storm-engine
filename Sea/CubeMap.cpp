...
void SEA::CubeMapGetSideMatrix(long iSide, void *matView)
{
...
void SEA::CubeMapSaveMatricesAndSetProjection()
{
    HRESULT hr;
    D3DXMATRIX matProj;
...
void SEA::CubeMapRestoreMatrices()
{
...
void SEA::CubeMapCalculateReflectionDynamic()
{
    ...
    for (long i = 0; i < 6; i++) // if (i==m_iCubeMapSide)
    {
        ...
        if (Techniques[dwCurTechnique].dwCode == G2MX_SEA)
            D3DXMatrixMultiply(&matView, &matCamera, &matView);
        ...
        NewViewport.X = 0;
        NewViewport.Width = SeaData.dwCubeMapSize;
        NewViewport.Y = 0;
        NewViewport.Height = SeaData.dwCubeMapSize;
        ...
        if (hr == D3D_OK && api->SetEntityScanLayer("sea_reflection") && api->GetEntity(&ent_id))
            do
            {
                ...
                if (bUnderWater && dwCCode == dwShipCode)
                    continue;
...
void SEA::CubeMapExecute(dword DeltaTime)
{
    CubeMapCalculateReflectionDynamic();
    CubeMapCalculateSunroadDynamic();
}
