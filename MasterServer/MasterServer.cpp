...
MasterServer::MasterServer() : aClients(_FL_), aServers(_FL_)
{
    _timeb _tb;
    _ftime(&_tb);
...
void MasterServer::Execute()
{
    ...
    for (i = 0; i < aServers.Size(); i++)
        if (dwCurrentTime - aServers[i].dwLastServerTime > 60 * 5)
        {
            Trace("delete server\n");
...
{
    for (dword i = 0; i < aServers.Size(); i++)
        if (aServers[i].dwIP == dwIP && aServers[i].wPort == wPort)
        {
...
void MasterServer::OnDisconnect(dword dwIP, word wPort, NMRecv &nmRecv)
{
    ...
    for (i = 0; i < aClients.Size(); i++)
        if (aClients[i]->dwIP == dwIP && aClients[i]->wPort == wPort)
        {
            Trace("delete client\n");
            delete aClients[i];
            aClients.ExtractNoShift(i);
...
void MasterServer::OnConnect(dword dwIP, word wPort, NMRecv &nmRecv)
{
    ...
    for (i = 0; i < aServers.Size(); i++)
        if (aServers[i].byVersion == byClientVersion)
        {
