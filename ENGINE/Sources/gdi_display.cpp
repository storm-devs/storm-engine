...
#define HEADER_TEXT "Storm Engine 2.9.0    BMS Edition (10.01.20)"
...
void GDI_DISPLAY::Print(char *text_PTR, ...)
{
    ...
    if (MS_Count >= MS_Lines)
    {
        ...
        copy_PTR = ms_PTR + MS_Count * BUFFER_SIZE;

    size = strlen(buffer);
    memset(copy_PTR, 0, BUFFER_SIZE);
    if (size >= BUFFER_SIZE)
        memcpy(copy_PTR, buffer, (BUFFER_SIZE - 1));
    else
