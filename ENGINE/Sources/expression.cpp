    ...
    case UNKNOWN:
        ...
        case CALL_FUNCTION:
            ...
            if(sttFuncCallType != CALL)	// skip argument checking for dynamic function call
            {
                if(func_args != fi.arguments)
                {
                    ...
                    else
                    {
                        ...
                        {
                            SetError("function '%s(args:%d)' doesnt accept %d
            arguments",fi.name,fi.arguments,func_args); return false;
                        }
            ...
            if (sttFuncCallType == CALL_FUNCTION)
                CompileToken(Segment, CALL_FUNCTION, 1, (char *)&dwRCode, sizeof(DWORD));
            else
            {
