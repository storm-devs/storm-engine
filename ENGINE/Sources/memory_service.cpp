...
void *MEMORY_SERVICE::Allocate(long size)
{
    ...
    if (data_PTR == null)
    {
        if (Exceptions_Mask & _X_NO_MEM)
            SE_THROW_MSG(_X_NO_MEM);
        return null;
    }
    ...
    if (bCollectInfo)
        MemStat.Allocate(CodeSource.pFileName, CodeSource.line, size);
    ...
    if (Minimal_Pointer > result)
        Minimal_Pointer = result;
    if (Minimal_Pointer == 0)
        Minimal_Pointer = result;
    if (Maximal_Pointer < result)
        Maximal_Pointer = result;
...
long MEMORY_SERVICE::PoolFree(void *block_PTR, DWORD nBlockSize)
{
    ...
    {
        if (nBlockSize >= nMPoolClassesNum)
            return false;
        if (pMPool[nBlockSize] == 0)
            return false;
        if (pMPool[nBlockSize]->FreeMemory((void *)block_PTR))
        {
            if (nBlockSize < SBCNUM)
            {
                if (SBCounter[nBlockSize] > 0)
                    SBCounter[nBlockSize]--;
            }
            return true;
        }
...
void MEMORY_SERVICE::Free(void *block_PTR)
{
    ...
    if ((dword)block_PTR < Minimal_Pointer || (dword)block_PTR > Maximal_Pointer || Blocks <= 0)
        throw "MEMORY_SERVICE::Free : Invalid pointer";
    ...
    if (link_L)
        link_L->link_R = (char *)link_R;
    if (link_R)
        link_R->link_L = (char *)link_L;
...
void *MEMORY_SERVICE::Reallocate(void *block_PTR, long size)
{
    ...
    if (bPoolOn)
    {
        ...
        if (nBlockSize != 0xffffffff)
        {
            ...
            if (pTemp == 0)
                SE_THROW("no mem");
            if ((DWORD)size > nBlockSize)
                memcpy(pTemp, block_PTR, nBlockSize);
            else
                memcpy(pTemp, block_PTR, size);
    ...
    if (bCollectInfo)
    {
        ...
        char *pFileName;
        pFileName = cMemAddress.GetSource(mb_PTR->address, &dwLine);
        MemStat.Resize(CodeSource.pFileName, CodeSource.line, size, pFileName, dwLine, mb_PTR->data_size);
    }
    ...
    if (old_data_PTR != data_PTR)
    {
        ...
        if (link_L)
            link_L->link_R = (char *)data_PTR;
        if (link_R)
            link_R->link_L = (char *)data_PTR;
        if (Mem_link == old_mb_PTR)
            Mem_link = mb_PTR;
    }
    ...
    while (Mem_link)
    {
        ...
        catch (...)
        {
            ...
            try
            {
                ...
                {
                    if (data[i])
                        sprintf(buf + strlen(buf), "%.2x('%c') ", data[i], data[i]);
                    else
                        sprintf(buf + strlen(buf), "%.2x(   ) ", data[i]);
                    if (j >= 7)
                    {
                        trace("       %s", buf);
                        j = 0;
        ...
        if (Blocks <= 0 && Mem_link != null)
            throw "MBLFREE_ERROR"; // ... ; fatal, close programm
    }
    Minimal_Pointer = null;
    Maximal_Pointer = null;
}
...
dword MEMORY_SERVICE::GetBlockSize(void *block_PTR)
{
    ...
    MEM_BLOCK *mb_PTR;
    if ((dword)block_PTR < Minimal_Pointer || (dword)block_PTR > Maximal_Pointer || Blocks <= 0)
        throw "MEMORY_SERVICE::GetBlockSize : Invalid pointer";
...
void *MEMORY_SERVICE::GetNextPointer()
{
    ...
    MEM_BLOCK *mb_PTR;
    if (!ValidateBlock(Search_Pointer /*,0*/))
        return null;
    mb_PTR = (MEM_BLOCK *)((char *)Search_Pointer - sizeof(MEM_BLOCK));
    if (mb_PTR->link_L == null)
        Search_Pointer = null;
    else
        Search_Pointer = (void *)((char *)mb_PTR->link_L + sizeof(MEM_BLOCK));
    return Search_Pointer;
}
...
void MEMORY_SERVICE::DumpMemoryState()
{
    ...
    if (bPoolOn)
    {
        ...
        for (n = 0; n < nMPoolClassesNum; n++)
        {
            if (!pMPool[n])
                continue;
            if (pMPool[n]->nBlocksNum == 0)
                continue;
            trace("Pool : [%d] bytes", pMPool[n]->nBlockSize);
            trace("     : blocks  [%d of %d]   memory  [%.3fkb of %.3fkb]", pMPool[n]->nUsedBlocks,
                  pMPool[n]->nBlocksNum, pMPool[n]->nUsedBlocks * (pMPool[n]->nBlockSize + POOLHEADERSIZE) / 1024.f,
        ...
        trace("Total:");
        trace("     : blocks  [%d of %d]   memory  [%.3fkb of %.3fkb]", dwTotalPoolUsedBlocks, dwTotalPoolBlocks,
              dwTotalPoolUsedMem / 1024.f, dwTotalPoolMem / 1024.f);

        if (dwTotalPoolMem)
            trace("     : usage %.2f%%   missed %d", dwTotalPoolUsedMem * 100.f / dwTotalPoolMem, dwTotalMissed);
    }
#endif
    ...
    for (n = 0; n < cMemAddress.dwRecordsNum; n++)
    {
        ...
        if (dwBlocks)
        {
            trace("MEMUSER [%s, %d] : \n memory  %.3f kb in %d blocks", cMemAddress.pData[n].pFileName,
                  cMemAddress.pData[n].dwLine, dwMem / 1024.f, dwBlocks);
        }
    }
