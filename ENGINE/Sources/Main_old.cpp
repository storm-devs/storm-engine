...
char AClass[] = "Legend is Back"; //"Age of pirates";  boal
...
bool WriteDirs(INIFILE *ini, char *pRootName)
{
    ...
    do
    {
        if (findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
        {
            ...
            if (pSubDirName[strlen(pSubDirName) - 1] != '\\')
                strcat(pSubDirName, "\\");
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    try
    {
        ...
        case WM_DESTROY:
            ...
            // trace("System exit and cleanup:");
            // trace("Mem state: User memory: %d  MSSystem: %d  Blocks:
            // %d",Memory_Service.Allocated_memory_user,Memory_Service.Allocated_memory_system,Memory_Service.Blocks);
