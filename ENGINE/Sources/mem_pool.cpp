...
bool BITLEVEL::SetBit(DWORD nIndex, bool &bDwordFull)
{
    ...
    nMask = 1;
    if (nDwordBitIndex)
        nMask = (nMask << nDwordBitIndex);
    pBits[nOffset] = pBits[nOffset] | nMask;

    if (pBits[nOffset] == 0xffffffff)
        bDwordFull = true;
    else
        bDwordFull = false;
...
{
    if ((DWORD)pMem - POOLHEADERSIZE < (DWORD)pPoolData)
        return false;
    if ((DWORD)pMem - POOLHEADERSIZE > (DWORD)pPoolData + (nBlockSize + POOLHEADERSIZE) * (nBlocksNum - 1))
        return false;
    return true;
}