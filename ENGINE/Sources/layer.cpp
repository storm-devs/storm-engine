...
bool LAYER::Add(ENTITY_ID eid, dword priority)
{
    ...
    while (scanln_PTR)
    {
        if (scanln_PTR->priority >= priority)
        {
            ...
            if (ln_PTR->link_R)
                (ln_PTR->link_R)->link_L = ln_PTR;
            if (scanln_PTR == Base_Link)
                Base_Link = ln_PTR;
            Count++;
            return true;
        }
    ...
    do
    {
        if (ln_PTR)
        {
            if (memcmp(&ln_PTR->id, &eid, sizeof(ENTITY_ID)) == 0)
            {
                ...
                if (link_L)
                    link_L->link_R = link_R;
                if (link_R)
                    link_R->link_L = link_L;
                if (ln_PTR == Base_Link)
                    Base_Link = link_L;

                delete ln_PTR;
                Count--;
                if (Search_Link == ln_PTR)
                    Search_Link = link_L;
                ...
                for (dword n = 0; n < walkers_num; n++)
                {
                    if (walkers[n])
                    {
                        if (walkers[n]->Search_Link == (LPBYTE)ln_PTR)
                            walkers[n]->Search_Link = (LPBYTE)link_L;
                    }
                }
                return true;
            }
...
ENTITY_ID *LAYER::GetID(dword *priority_PTR)
{
    ...
    if (Base_Link)
    {
        Search_Link = Base_Link;
        if (priority_PTR)
            *priority_PTR = Base_Link->priority;
        return &Base_Link->id;
    }
    Search_Link = null;
    return null;
...
ENTITY_ID *LAYER::GetNextID(dword *priority_PTR)
{
    ...
    if (Search_Link)
    {
        ...
        if (Search_Link == null)
            return null;
        if (priority_PTR)
            *priority_PTR = Search_Link->priority;
        return &Search_Link->id;
    }
    return null;
}
...
VIDWALKER *LAYER::GetWalker()
{
    ...
    if (walkers == null)
        walkers = (IDWALKER **)NEW char[sizeof(IDWALKER *)];
    else
        walkers = (IDWALKER **)RESIZE(walkers, (walkers_num + 1) * sizeof(IDWALKER *));
...
{
    if (pLayer)
        ((LAYER *)pLayer)->DelWalker((void *)this);
}

ENTITY_ID *IDWALKER::GetID()
{
    if (pLayer)
        return ((LAYER *)pLayer)->WalkerGetID(Search_Link);
    return null;
}
...
{
    if (pLayer)
        return ((LAYER *)pLayer)->WalkerGetNextID(Search_Link);
    return null;
}
