...
AnimationServiceImp::~AnimationServiceImp()
{
    if (animation)
    {
        for (long i = 0; i < numAnimations; i++)
            if (animation[i])
            {
                _CORE_API->Trace("No release Animation pnt:0x%x for %s.ani", animation[i],
                                 animation[i]->GetAnimationInfo()->GetName());
                delete animation[i];
            }
        delete animation;
    }
...
Animation *AnimationServiceImp::CreateAnimation(const char *animationName)
{
    ...
    if (i == numAnimations)
    {
        ...
        for (long j = i; j < numAnimations; j++)
            animation[j] = null;
    }
...
long AnimationServiceImp::LoadAnimation(const char *animationName)
{
    ...
    //Зачитаем действия
    for (bool isHaveSection = ani->GetSectionName(path, 63); isHaveSection;
         isHaveSection = ani->GetSectionNameNext(path, 63))
    {
        ...
        {
            if (stricmp(key, ASKWAT_NORMAL) == 0)
                type = at_normal;
            else if (stricmp(key, ASKWAT_REVERSE) == 0)
                type = at_reverse;
            else if (stricmp(key, ASKWAT_PINGPONG) == 0)
                type = at_pingpong;
            else if (stricmp(key, ASKWAT_RPINGPONG) == 0)
                type = at_rpingpong;
            else
            {
        ...
        {
            if (stricmp(key, ASKWAL_TRUE) == 0)
                isLoop = true;
            else if (stricmp(key, ASKWAL_FALSE) == 0)
                isLoop = false;
            else
            {
                _CORE_API->Trace("Incorrect %s in action [%s] of animation file %s.ani\nThis parameter (%s) use is "
                                 "default value %s\n",
        ...
        if (ani->ReadString(path, ASKW_EVENT, key, 256, ""))
        {
            do
            {
                ...
                if (key[p])
                {
                    ...
                    else if (stricmp(em, ASKWAE_ALWAYS) == 0)
                    {
                        ev = eae_always;
                    }
                    else if (stricmp(em, ASKWAE_NORMAL) == 0)
                    {
                        ev = eae_normal;
                    }
                    else if (stricmp(em, ASKWAE_REVERSE) == 0)
                    {
                        ev = eae_reverse;
                    }
                    else
                    {
                        _CORE_API->Trace("Warning: Incorrect %s <%s> in action [%s] of animation file %s.ani,\nunknow "
                                         "event type <%s> -> set is default value\n",
                                         ASKW_EVENT, key + 257, path, animationName, em);
                    }
                }
                //_CORE_API->Trace("Add event %s, time = %f to action %s", key + 1, (tm - stime)/float(etime - stime),
                // path); Добавляем событие
...
bool AnimationServiceImp::LoadAN(const char *fname, AnimationInfo *info)
{
    ...
    try
    {
        ...
        if (!_CORE_API->fio->_ReadFile(fl, &header, sizeof(ANFILE::HEADER), 0) || header.nFrames <= 0 ||
