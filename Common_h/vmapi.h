...
#define API_MODULE_START(a)                                                                                            \
    ...
    class MODULE_API : public VMODULE_API                                                                              \
    {                                                                                                                  \
      public:                                                                                                          \
        long Build_Version()                                                                                           \
        {                                                                                                              \
        ...
        void *CreateClass(long code, bool get_class_name, VSYSTEM_API *sysapi_PTR)                                     \
        {                                                                                                              \
            _VSYSTEM_API = sysapi_PTR;                                                                                 \
            switch (code)                                                                                              \
            {
#define API_MODULE_END                                                                                                 \
    default:                                                                                                           \
        ...
        ;                                                                                                              \
        DLL_MODULE_API_FUNCTION DMAInterface();
...
#define INTERFACE_FUNCTION                                                                                             \
    ...
    void *_SERVICE_PTR = null;                                                                                         \
    extern "C" DLL_MODULE_API_FUNCTION DMAInterface(VAPI *api_PTR)                                                     \
    {                                                                                                                  \
        _CORE_API = api_PTR;                                                                                           \
        return new MODULE_API;                                                                                         \
    }                                                                                                                  \
    void *operator new(size_t size)                                                                                    \
    {                                                                                                                  \
        return _CORE_API->MemAllocate(size);                                                                           \
    }                                                                                                                  \
    void operator delete(void *block_ptr)                                                                              \
    {                                                                                                                  \
        _CORE_API->MemFree(block_ptr);                                                                                 \
    }                                                                                                                  \
    void *resize(void *block_ptr, size_t size)                                                                         \
    {                                                                                                                  \
...
#define INTERFACE_FUNCTION                                                                                             \
    extern VAPI *_CORE_API;                                                                                            \
    extern VSYSTEM_API *_VSYSTEM_API;                                                                                  \
    extern void *_SERVICE_PTR;
// void * operator new(size_t size); {return _CORE_API->MemAllocate(size);} void operator delete(void * block_ptr) {
// _CORE_API->MemFree(block_ptr); } void * resize(void * block_ptr,size_t size){ return
// _CORE_API->MemReallocate(block_ptr,size); }
