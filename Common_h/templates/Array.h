...
template <class _Ty> class array
{
  ...
  public:
    ...
    void SetFileLine(const char *pFileName, long iFileLine)
    {
        _Al.SetFileLine(pFileName, iFileLine);
    }
    ...
    _Myt &SetAddElements(dword dwReserveCount)
    {
        _Al.SetAddElements(dwReserveCount);
        return *this;
    }
    ...
    __forceinline dword GetDataSize() const
    {
        return _Al.Size() * sizeof(_Ty);
    }
    ...
    // Return number of elements
    __forceinline long Len() const
    {
        return (long)_Al.Size();
    }
    // Return last element index
    __forceinline dword Last() const
    {
        return _Al.Size() - 1;
    }
    // Return first element index
    __forceinline dword First() const
    {
        return 0;
    }
    ...
    // Operator return number of elements
    __forceinline dword operator()(void) const
    {
        return _Al.Size();
    }
    ...
    __forceinline _Ty *GetBuffer()
    {
        return (Size()) ? &_Al.pE[0] : null;
    }
    // Return first element
    __forceinline _Ty &FirstE()
    {
        Assert(Size());
        return _Al.pE[0];
    }
    // Return last element
    __forceinline _Ty &LastE()
    {
        Assert(Size());
        return _Al.pE[Last()];
    }
    ...
    _Myt &Sort(bool bGreatest = true)
    {
        ...
        for (dword i = 0; i < Size(); i++)
            for (dword j = i + 1; j < Size(); j++)
            {
                ...
                if (GetElement(ii) < GetElement(jj))
                    Swap(ii, jj);
            }
    ...
    _Myt &QSort(bool (*compare_func)(const _Ty &, const _Ty &), long iFirst, long iLast)
    {
        if (!Size() || !compare_func)
            return *this;
        if (iFirst < 0 || iFirst >= Len() || iLast < 0 || iLast >= Len())
            return *this;
  ...
  private:
    ...
    begin:
        ...
        do
        {
            while (compare_func(GetElement(j), str) && (j < h))
                j++;
            while (compare_func(str, GetElement(i)) && (i > d))
                i--;
