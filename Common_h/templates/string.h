...
class string
{
  ...
  public:
    ...
    inline dword GetDataSize()
    {
        return dwCurSize;
    };
    // set new string size
    inline void SetDataSize(dword dwNewSize)
    {
        Assert(dwNewSize < dwCurSize);
        dwSize = dwNewSize;
    };

    // Operator return string length
    inline dword operator()(void) const
    {
        return Size();
    }
    ...
    {
        return (pString) ? stricmp(GetBuffer(), pString) == 0 : false;
    }
    inline bool operator==(const string &s1) const
    {
        return *this == s1.GetBuffer();
    }
    ...
    string &Insert(dword dwIndex, const string &sInsString)
    {
        ...
        if (dwShiftSize)
            memmove(&pBuffer[dwIndex + sInsString.Len()], &pBuffer[dwIndex], dwShiftSize);
    ...
    string &CheckPath()
    {
        ...
        while (dwCurr < Len())
        {
            ...
            if (pBuffer[dwCurr] == '\\')
                while (pBuffer[dwCurr + 1] == '\\' && dwCurr < Len() - 1)
                    dwCurr++;
            dwReal++;
            dwCurr++;
        }
    ...
    string &TrimRight()
    {
        ...
        while ((*pTemp == ' ' || *pTemp == '\t') && (pTemp >= pBuffer))
            pTemp--;

        // do nothing if string is already trimed
        if (pTemp == pBuffer + Len() - 1)
            return *this;
    ...
    {
        if (!pSkip || dwShift >= Len() - 1)
            return false;
        for (dword i = dwShift; i < Len(); i++)
            if (GetBuffer()[i] == pSkip[0])
            {
    ...
    inline static bool EqualPostfix(const char *str, const char *postfix)
    {
        ...
        if (slen < plen)
            return false;
        if (stricmp(str + slen - plen, postfix) == 0)
            return true;
        return false;
    }
